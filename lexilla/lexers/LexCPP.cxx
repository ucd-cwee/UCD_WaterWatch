...
bool followsReturnKeyword(const StyleContext &sc, LexAccessor &styler) {
	...
	const char *retBack = "return"; // nruter
...
struct OptionsCPP {
	...
	bool foldExplicitAnywhere = true;
	bool foldPreprocessor = true;
	bool foldPreprocessorAtElse = true;
...
class LexerCPP : public ILexer5 {
	...
	WordList keywords; // SCE_C_WORD
	WordList keywords2; // SCE_C_WORD2
	WordList keywords3; // comment dockey words?
	WordList keywords4; // globals
	WordList keywords5; // function names
	WordList ppDefinitions; // preprocessor definitions
	WordList markerList; // comment markers? 
	struct SymbolValue {
		std::string value;
		std::string arguments;
...
void SCI_METHOD LexerCPP::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
	...
	bool currentlyProcessingFunction = false;
	...
	for (; sc.More();) {
		...
		if (sc.atLineStart) {
			currentlyProcessingFunction = false;
		...
		switch (MaskActive(sc.state)) {
			case SCE_C_OPERATOR:
				...
				if (sc.chPrev == ')' && sc.ch == '.' && !sc.atLineEnd) {
					// the upcoming variable will be a function or accessor of some type
					currentlyProcessingFunction = true;
				}
			...
			case SCE_C_IDENTIFIER:
				if (sc.atLineStart || sc.atLineEnd || !setWord.Contains(sc.ch) || (sc.ch == '.')) {
					...
					if (keywords.InList(currentText)) {
						...
						sc.ChangeState(SCE_C_WORD|activitySet); // built-in types
					} else if (keywords2.InList(currentText)) {
						sc.ChangeState(SCE_C_WORD2|activitySet); // language features
					} else if (keywords4.InList(currentText)) {
						sc.ChangeState(SCE_C_GLOBALCLASS|activitySet); // user-defined types
					} else if (keywords5.InList(currentText) || sc.ch == '(' || (sc.ch == '.' && currentlyProcessingFunction)) {
						sc.ChangeState(SCE_C_USERLITERAL | activitySet); // function names
					} else if (currentlyProcessingFunction){
						currentlyProcessingFunction = false;
						sc.ChangeState(SCE_C_USERLITERAL | activitySet); // function names
					} else {
					...
					if (sc.ch == '.') {
						// the upcoming (NOT PREVIOUS, WHICH IS BELOW) variable will be a function or accessor of some type
						currentlyProcessingFunction = true;
					}
