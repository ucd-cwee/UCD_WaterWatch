...
#define FINALLY_CONCAT(a, b) FINALLY_CONCAT_(a, b)
	...
	//      printf("hello ");
	// }
#define defer(x) decltype(auto) FINALLY_CONCAT(defer_, __LINE__) { GoodLang::make_finally([&] { x; }) }
	...
	public:
		...
		virtual bool IsBuiltInType() const { return true; };
	...
	public:
		...
		virtual bool IsBuiltInType() const override { return false; };
...
namespace GoodLang {
	namespace details {
		...
		public:
			using ReturnType = typename utilities::function_traits< typename decltype(std::function(std::declval<Callable>())) >::result_type;
			using InputType = typename /*std::decay_t<*/std::tuple_element_t<0, typename utilities::function_traits< typename decltype(std::function(std::declval<Callable>())) >::arguments>/*>*/;
		...
		template<class Callable> __forceinline std::shared_ptr<Type_Conversion_Base> MakeConversionFunc(Callable func) {
			...
			using CallableArguments = typename utilities::function_traits< CallableTypeAsStdFunc >::arguments;
			if constexpr (std::tuple_size_v< CallableArguments > != 1) {
				return nullptr;
			}
			else {
				using From = typename /*std::decay_t<*/std::tuple_element_t<0, CallableArguments>/*>*/;
				using To = typename utilities::function_traits< CallableTypeAsStdFunc >::result_type;
	...
	public:
		...
		template<class Callable> void AddConverter(Callable Func) {
			...
			if constexpr (std::tuple_size_v< CallableArguments > != 1) {
				...
				//using To_t = typename details::get_type<typename utilities::function_traits< CallableTypeAsStdFunc >::result_type>::type;
