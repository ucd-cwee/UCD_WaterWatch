	...
	namespace details {
		...
		public:
			...
			Any convert_down(const Any&) const override {
				throw fibers::exception::bad_boxed_cast("Custom_Type_Conversion_Impl is not bidirectional.");
			};
		...
		public:
			...
			Any convert_down(const Any& t_to) const override {
				if constexpr (is_bidir) {
					...
					throw fibers::exception::bad_boxed_cast("Static_Type_Conversion_Impl was not bidirectional.");
				}
			};
		...
		public:
			...
			// BaseType -> ChildType
			Any convert_down(const Any& t_to) const override {
				throw fibers::exception::bad_boxed_cast("Dynamic_Type_Conversion_Impl is never bidirectional (Base -> Child). Only may cast from (Child -> Base).");
			};
		...
		public:
			constexpr Function_Params(Any* t_begin, Any* t_end)
				: m_begin(t_begin)
				, m_end(t_end) {
			};
...
namespace fibers {
	namespace details {
		...
		public:
			...
			const Any& operator[](const std::string& t_attr_name) const { return get_attr(t_attr_name); }

			Any& operator[](const std::string& t_attr_name) { return get_attr(t_attr_name); }

			const Any& get_attr(const std::string& t_attr_name) const {
			...
			Any& get_attr(const std::string& t_attr_name) { return m_attrs[t_attr_name]; }

			Any& method_missing(const std::string& t_method_name) {
				if (m_option_explicit && m_attrs.find(t_method_name) == m_attrs.end()) {
					throw fibers::exception::option_explicit_set(t_method_name);
				}

				return get_attr(t_method_name);
			}

			const Any& method_missing(const std::string& t_method_name) const {
				if (m_option_explicit && m_attrs.find(t_method_name) == m_attrs.end()) {
					throw fibers::exception::option_explicit_set(t_method_name);
				}
			...
			std::map<std::string, Any> get_attrs() const { return m_attrs; }
...
namespace fibers {
	namespace details {
		...
		public:
			...
			std::vector<Any> convert(Function_Params t_params, const Type_Converter_Tree& t_conversions) const {
				...
				for (size_t i = 0; i < vals.size(); ++i) {
					...
					if (bv.Type() != dynamic_object_type_info) {
		...
		*/
		...
		public:
			...
			Any operator()(const Function_Params& params, const Type_Converter_Tree& t_conversions) const {
				...
				throw fibers::exception::arity_error(static_cast<int>(params.size()), m_types.size());
			};
		...
		*/
		...
		protected:
			static Param_Types Get_Arg_Type() {
				...
				return fibers::details::Param_Types{ t_types };
			};
		...
		*/
		...
		template<typename Func> Proxy_Function make_callable(Func&& func) {
			using function_header = decltype(detail::function_signature(func));
			...
			if constexpr (is_object) {
				// function objects
				auto* function_impl = new fibers::details::Explicit_Function_Impl(std::forward<Func>(func));
				auto ptr{ std::static_pointer_cast<fibers::details::Proxy_Function_Base>(std::shared_ptr<typename std::remove_pointer<decltype(function_impl)>::type>(function_impl)) };
				return ptr;
			}
			else if constexpr (is_member_object) {
				// member objects
				auto* function_impl = new fibers::details::Attribute_Access_Impl(std::forward<Func>(func));
				auto ptr{ std::static_pointer_cast<fibers::details::Proxy_Function_Base>(std::shared_ptr<typename std::remove_pointer<decltype(function_impl)>::type>(function_impl)) };
				return ptr;
			}
			else if constexpr (is_member && !is_member_object) {
				// member functions
				return fibers::details::Member_Function_Impl(std::forward<Func>(func));
			}
			else if constexpr (is_static_member_function) {
				// static function pointers
				auto* function_impl = new fibers::details::Static_Function_Impl(std::forward<Func>(func));
				auto ptr{ std::static_pointer_cast<fibers::details::Proxy_Function_Base>(std::shared_ptr<typename std::remove_pointer<decltype(function_impl)>::type>(function_impl)) };
				return ptr;
			}
			else {
		...
		__forceinline Any call(Proxy_Function callable, std::vector<Any> const& inputs, fibers::details::Type_Converter_Tree const& conversionTree) {
			return callable->operator()(fibers::details::Function_Params{ const_cast<std::vector<Any>&>(inputs) }, conversionTree);
		};
	...
	namespace details {
		...
		class ScriptedClass {
			std::string
				m_name; // classt name. May not be empty.
			concurrency::concurrent_unordered_map<std::string, Type_Info>
				m_parameters; // parameters of the Class. Duplicate names are not allowed.
			concurrency::concurrent_unordered_multimap<std::string, Proxy_Function>
				m_functions; // functions in this Class. Duplicate names are allowed.

			//void AddFunction(std::vector<Type_Info> const& inputs, Type_Info ExpectedOutcome) {
			//	// TO-Do, make a version of "Explicit_Function_Impl" that is designed to take as input the std::vector<Any> and helps guarrantee the expected outcome
		...
		private:
			Namespace*
				m_parent_scope; // parent scope -- may be empty.
			concurrency::concurrent_unordered_set<Namespace*>
				m_children_scopes; // children that are within this scope. 
			std::string
		...
		public:
			Namespace() = default;
			Namespace(std::string const& p_name) : m_name(p_name) {};
			Namespace(std::string p_name, Namespace* p_parentScope) : m_name(std::move(p_name)), m_parent_scope(std::move(p_parentScope)) {};

		public:
			// fibers::utilities::GarbageCollectedAllocator< Namespace > allocator;
		};
