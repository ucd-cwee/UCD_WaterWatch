...
int main() {
	...
	if (1) {
		print(GoodLang::ToString(GoodLang::Engine::Compiler::Interpreter::Parser().Parse(R"(1_ft > 1_in)").first->eval(StartScope(StartScope()))));
	}
	...
	if (1) {
		...
		auto parsed_result = GoodLang::Engine::Compiler::Interpreter::Parser().Parse(final_script);
		if (1) {
			print(final_script);
			Stopwatch sw;
			sw.Start();
	...
	if (1) {
		...
		if (1) {
			...
			auto parse{ GoodLang::Engine::Compiler::Interpreter::Parser() };
