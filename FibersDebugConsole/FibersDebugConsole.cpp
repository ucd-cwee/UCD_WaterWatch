...
namespace GoodLang {
	...
	namespace Engine {
		...
		public:
								...
								if (words[word_index].word == macro_function.first) { // print
									...
									if (Code[start_pos] == '(') {
										...
										while ((parenCount > 0) && (++end_pos < Code.size())) {
											...
											if ((macro_function.second.second.size() > functionParams.size()) && Code[end_pos] == ',') {
			...
			// To-Do: Allow the eval function to evaluate the user-provided compile-time logic. Recursion with overall engine, perhaps? 
		...
		// The interpreter should take in source code and convert it to an abstract syntax tree (AST) that can be evaluated at runtime. 
		class Interpreter {
		public:
		...
		struct Id_AST_Node final : AST_Node_Impl {
		...
		*/
		...
		std::string
		*/
		struct TypeID_Decl_AST_Node final : AST_Node_Impl {
			TypeID_Decl_AST_Node(const std::shared_ptr<Scope>& currentScope, std::string t_ast_node_text, Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
				: AST_Node_Impl(std::move(t_ast_node_text), AST_Node_Type::TypeId, std::move(t_loc), std::move(t_children))
			{
				assert(this->children.size() >= 1);
		...
		*/
		...
		struct Reference_AST_Node final : AST_Node_Impl {
			Reference_AST_Node(const std::shared_ptr<Scope>& currentScope, std::string t_ast_node_text, Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
				: AST_Node_Impl(std::move(t_ast_node_text), AST_Node_Type::Reference, std::move(t_loc), std::move(t_children)) {
				assert(this->children.size() == 1);
			}
		...
		struct Compiled_AST_Node : AST_Node_Impl {
			Compiled_AST_Node(const std::shared_ptr<Scope>& currentScope, AST_Node_Impl_Ptr t_original_node,
				std::vector<AST_Node_Impl_Ptr> t_children,
				std::function<Any(const std::vector<AST_Node_Impl_Ptr>&, const std::shared_ptr<Scope>& currentScope)> t_func
			)
				: AST_Node_Impl(t_original_node->text, AST_Node_Type::Compiled, t_original_node->location, std::move(t_children))
			...
			std::function<Any(const std::vector<AST_Node_Impl_Ptr>&, const std::shared_ptr<Scope>& currentScope)> m_func;
			AST_Node_Impl_Ptr m_original_node;
		};
		...
		namespace parser {
			namespace optimizer {
				template<typename... T> struct Optimizer : T... {
					...
					auto optimize(AST_Node_Impl_Ptr p, const std::shared_ptr<Scope>& currentScope) {
				...
				AST_Node_Impl& child_at(AST_Node_Impl& node, const size_t offset) noexcept {
					if (node.children[offset]->identifier == AST_Node_Type::Compiled) {
						return *(dynamic_cast<Compiled_AST_Node &>(*node.children[offset]).m_original_node);
					}
					else {
						return *node.children[offset];
					}
				};
				const AST_Node_Impl& child_at(const AST_Node_Impl& node, const size_t offset) noexcept {
					if (node.children[offset]->identifier == AST_Node_Type::Compiled) {
						return *(dynamic_cast<const Compiled_AST_Node &>(*node.children[offset]).m_original_node);
					}
					else {
						return *node.children[offset];
					}
				};
				size_t child_count(const AST_Node_Impl& node) noexcept {
					if (node.identifier == AST_Node_Type::Compiled) {
						return dynamic_cast<const Compiled_AST_Node &>(node).m_original_node->children.size();
					}
					else {
						return node.children.size();
					}
				};
				template<typename Callable>	std::shared_ptr<AST_Node_Impl> make_compiled_node(const std::shared_ptr<Scope>& currentScope, AST_Node_Impl_Ptr original_node, std::vector<AST_Node_Impl_Ptr> children, Callable callable) {
					return std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Compiled_AST_Node>(currentScope, std::move(original_node), std::move(children), std::move(callable)));
				}
				bool contains_var_decl_in_scope(const AST_Node_Impl& node) noexcept {
					if (
					...
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if ((node->identifier == AST_Node_Type::Block) || (node->identifier == AST_Node_Type::Scopeless_Block)) {
							...
							if (keepers.size() == num_children) {
								...
								node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Block_AST_Node>(currentScope, node->text, node->location, new_children()));
							...
							if (last_child->identifier == AST_Node_Type::Block || last_child->identifier == AST_Node_Type::Scopeless_Block) {
								...
								if (block_last_child->identifier == AST_Node_Type::Return) {
									if (block_last_child->children.size() == 1) {
										...
										block_last_child = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Noop_AST_Node>());
										return true;
									}
								}
							}
							...
							if (last_child->identifier == AST_Node_Type::Block || last_child->identifier == AST_Node_Type::Scopeless_Block) {
								...
								if (block_last_child->identifier == AST_Node_Type::Return) {
									if (block_last_child->children.size() == 0) {
										last_child->children.back() = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Noop_AST_Node>());
										return true;
									}
						...
						if (node->identifier == AST_Node_Type::Block) {
							if (!contains_var_decl_in_scope(*node)) {
								if (node->children.size() == 1) {
									...
									node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Scopeless_Block_AST_Node>(
										currentScope,
										node->text,
										node->location,
						...
						if ((node->identifier == AST_Node_Type::Block || node->identifier == AST_Node_Type::Scopeless_Block) && !node->children.empty()) {
							for (size_t i = 0; i < node->children.size() - 1; ++i) {
								...
								if (child->identifier == AST_Node_Type::Fun_Call) {
									node->children[i] = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Unused_Return_Fun_Call_AST_Node>(
										currentScope,
										child->text,
										child->location,
							...
							if (child.identifier == AST_Node_Type::Block || child.identifier == AST_Node_Type::Scopeless_Block) {
								...
								for (size_t i = 0; i < num_sub_children; ++i) {
									...
									if (sub_child.identifier == AST_Node_Type::Fun_Call) {
										child.children[i] = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Unused_Return_Fun_Call_AST_Node>(
											currentScope,
											sub_child.text,
											sub_child.location,
				...
				struct If {
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if ((node->identifier == AST_Node_Type::If) && node->children.size() >= 2 && node->children[0]->identifier == AST_Node_Type::Constant) {
							try {
								if (currentScope->Cast<bool>(dynamic_cast<Constant_AST_Node *>(node->children[0].get())->m_value)) {
									...
									// do nothing?
									node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Noop_AST_Node>());
									return true;
								}
							}
							catch (...) {
				...
				struct PrefixFold {
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if (node->identifier == AST_Node_Type::Prefix
							...
							const Any& rhs = dynamic_cast<Constant_AST_Node *>(node->children[0].get())->m_value;
							try {
								node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Constant_AST_Node>(
				...
				struct BinaryFold {
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if (node->identifier == AST_Node_Type::Binary
							...
							const Any& lhs = dynamic_cast<Constant_AST_Node *>(node->children[0].get())->m_value;
							const Any& rhs = dynamic_cast<Constant_AST_Node *>(node->children[1].get())->m_value;
						...
						if (node->identifier == AST_Node_Type::Binary
							...
							try {
								...
								if (parsed != Operators::Opers::invalid) {
									const auto rhs = dynamic_cast<Constant_AST_Node *>(node->children[1].get())->m_value;
									node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Fold_Right_Binary_Operator_AST_Node>(
					...
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						...
						if (node->identifier == AST_Node_Type::Inline_Array
							...
							if (allItemsAreConst) {
								...
								for (int childIndex = 0; childIndex < argList.children.size(); childIndex++) {
									Any rhs = dynamic_cast<Constant_AST_Node*>(argList.children[childIndex].get())->m_value;
						...
						if (node->identifier == AST_Node_Type::Inline_Map
							...
							for (int childIndex = 0; childIndex < argList.children.size(); childIndex++) {
								if (argList.children[childIndex]->identifier == AST_Node_Type::Map_Pair) {
									auto& map_pair = *dynamic_cast<Map_Pair_AST_Node*>(argList.children[childIndex].get());
									if (
				...
				struct ToStringFunctionCallWithConstant {
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if (node->identifier == AST_Node_Type::Fun_Call
							...
							const Any& rhs = dynamic_cast<Constant_AST_Node *>(node->children[1]->children[0].get())->m_value;
							try {
								node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Constant_AST_Node>(
				...
				struct VarDeclEquation_To_RetroactiveAssignment {
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if (node->identifier == AST_Node_Type::Equation
							...
							node = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Assign_Retroactively_AST_Node>(
					...
					bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scope>& currentScope) {
						if (node->identifier == AST_Node_Type::For
							...
							if (node->children[2]->identifier == AST_Node_Type::Postfix) { // x++
								...
								case hash("++"):
									node->children[2] = std::dynamic_pointer_cast<AST_Node_Impl>(std::make_shared<Prefix_AST_Node>(
		...
		namespace parser {
			...
			private:
				std::array<std::array<bool, lengthof_alphabet>, max_alphabet> m_alphabet = build_alphabet();
				std::array<Operator_Precedence, 12> m_operators = create_operators();
								...
								if (cparser.saw_interpolation_marker) {
									if (*s == '{') {
										...
										if (*s == '}') {
											...
											build_match<Fun_Call_AST_Node>(currentScope, tostr_stack_top);
											build_match<Binary_Operator_AST_Node>(currentScope, prev_stack_top, "+");
										}
										else {
			...
			private:
				...
				bool Arg(const std::shared_ptr<Scope>& currentScope, const bool t_type_allowed = true) {
					...
					build_match<Arg_AST_Node>(currentScope, prev_stack_top);
				...
				bool TypeCastOperation(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Char('(') && TypeName(currentScope, typeName_r) && Char(')')) {
						if (Operator(currentScope)) {
							retval = true;
							build_match<Arg_List_AST_Node>(currentScope, prev_stack_top + 1);
							build_match<Fun_Call_AST_Node>(currentScope, prev_stack_top); // Id(fun name), Arg_List()
						}
					}
				...
				bool SpecifiedType_Var_Decl(const std::shared_ptr<Scope>& currentScope) {
					...
					if (TypeName(currentScope, typeName_r)) { // typename was specified and found
						build_match<Arg_List_AST_Node>(currentScope, prev_stack_top + 1); // {no_params}
						build_match<Fun_Call_AST_Node>(currentScope, prev_stack_top); // collapse all into a function call (i.e. int({no_params}))
						...
						if (Reference(currentScope)) { // this builds a reference node (var& i) 
							...
							retval = true;
							build_match<Var_Decl_AST_Node>(currentScope, prev_stack_top + 1);  // var i;                           
						}

						if (retval) {
							// Fun_Call ("Typename()") , Id or Ref ("Variable name");
							build_match < Assign_Retroactively_AST_Node>(currentScope, prev_stack_top);
						}
					}
					if (!retval) {
						while (m_match_stack.size() != prev_stack_top) m_match_stack.pop_back();
				...
				bool Postfix(const std::shared_ptr<Scope>& currentScope, bool gotValueAlready) {
					...
					if (gotValueAlready) {
						if (Symbol("++")) {
							build_match<Postfix_AST_Node>(currentScope, prev_stack_top - 1, "++");
							return true;
						}
						else if (Symbol("--")) {
							build_match<Postfix_AST_Node>(currentScope, prev_stack_top - 1, "--");
							...
							if (prev_stack_top > 0 && m_match_stack[prev_stack_top - 1].first->text != "" && m_match_stack[prev_stack_top - 1].first->identifier == AST_Node_Type::Constant) {
								...
								for (auto& abbreviation_length_to_category : customOperators) {
									for (auto& unit_type : abbreviation_length_to_category.second) {
										...
										if (Symbol(abbreviation)) {
											auto& rhs = std::dynamic_pointer_cast<Constant_AST_Node>(m_match_stack[prev_stack_top - 1].first)->m_value;
											Any lhs;
											if (auto Class = currentScope->FindClass(unit_type.first)) {
											...
											m_match_stack[prev_stack_top - 1].first =
												std::dynamic_pointer_cast<AST_Node_Impl>(
													std::make_shared<Constant_AST_Node>(currentScope, temp, loc, lhs)
													);
								...
								for (auto& unit_type : Units::value::GetValueTypes()) {
									...
									if (Symbol(abbreviation)) {
										...
										m_match_stack[prev_stack_top - 1].first =
											std::dynamic_pointer_cast<AST_Node_Impl>(
												std::make_shared<Equation_AST_Node>(currentScope, "=", loc, std::vector<AST_Node_Impl_Ptr>{
											std::dynamic_pointer_cast<AST_Node_Impl>(
												std::make_shared<Constant_AST_Node>(currentScope, GoodLang::ToString(lhs), loc, lhs)
												),
						...
						if (Id(true, currentScope)) {
							if (Symbol("++")) {
								build_match<Postfix_AST_Node>(currentScope, prev_stack_top, "++");
								return true;
							}
							else if (Symbol("--")) {
								build_match<Postfix_AST_Node>(currentScope, prev_stack_top, "--");
						...
						if (Operator(currentScope, t_precedence + 1)) {
							...
							while (Operator_Helper(t_precedence, oper)) {
								...
								case (Operator_Precedence::Ternary_Cond):
									if (oper == "?=") {
										build_match<Equation_AST_Node>(currentScope, prev_stack_top, oper);
									}
									else {
										if (Symbol(":")) {
											if (!Operator(currentScope, t_precedence + 1)) {
								...
								case (Operator_Precedence::Comparison):
									build_match<Binary_Operator_AST_Node>(currentScope, prev_stack_top, oper);
									break;

								case (Operator_Precedence::Logical_And):
									build_match<Logical_And_AST_Node>(currentScope, prev_stack_top, oper);
									break;
								case (Operator_Precedence::Logical_Or):
									build_match<Logical_Or_AST_Node>(currentScope, prev_stack_top, oper);
									break;
					...
					if (Char('[')) {
						...
						if ((prev_stack_top != m_match_stack.size()) && (!m_match_stack.back().first->children.empty())) {
							if (m_match_stack.back().first->children[0]->identifier == AST_Node_Type::Map_Pair) {
								build_match<Inline_Map_AST_Node>(thisScope, prev_stack_top);
							}
							else {
								build_match<Inline_Array_AST_Node>(thisScope, prev_stack_top);
							}
						}
						else {
							build_match<Inline_Array_AST_Node>(thisScope, prev_stack_top);
				...
				bool Lambda(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Char('[')) {
						...
						build_match<Arg_List_AST_Node>(currentScope, prev_stack_top);
					}
					...
					if (Symbol("->")) {
						...
						// make sure we always have the same number of nodes
						m_match_stack.push_back(ParseNode{ std::make_shared<Noop_AST_Node>(), nullptr });
					}
				...
				bool For_Guards(const std::shared_ptr<Scope>& currentScope) {
					...
					if (!(Equation(currentScope) && Eol())) {
						if (!Eol()) {
							...
							m_match_stack.push_back(ParseNode{ std::make_shared<Constant_AST_Node>(Any(true)), currentScope });
						}
					}
				...
				bool Break(const std::shared_ptr<Scope>& currentScope) {
					const auto prev_stack_top = m_match_stack.size();
					if (Keyword("break")) {
						build_match<Break_AST_Node>(currentScope, prev_stack_top);
						return true;
					}
					else {
						return false;
				...
				bool Continue(const std::shared_ptr<Scope>& currentScope) {
					const auto prev_stack_top = m_match_stack.size();
					if (Keyword("continue")) {
						build_match<Continue_AST_Node>(currentScope, prev_stack_top);
						return true;
					}
					else {
						return false;
				...
				bool Try(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Keyword("try")) {
						...
						build_match<Try_AST_Node>(currentScope, prev_stack_top);
					}
					else if (Keyword("do")) {
						retval = true;
						...
						build_match<Do_AST_Node>(currentScope, prev_stack_top);
					}
					return retval;
				}
				...
				bool DeclFunction(const std::shared_ptr<Scope>& currentScope) {
					...
					build_match<FunctionDecl_AST_Node>(currentScope, prev_stack_top);
			...
			private:
				...
				ParseNode parse_internal(const std::string& t_input, const std::shared_ptr<Scope>& currentScope) {
					...
					if (Statements(currentScope)) {
						if (m_position.has_more()) {
							...
							build_match<File_AST_Node>(currentScope, 0);
						}
					}
					else {
						m_match_stack.push_back({ std::make_shared<AST_Node_Impl, Noop_AST_Node>(Noop_AST_Node()), nullptr });
					}
				...
				bool Block(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Char('{')) {
						...
						build_match<Block_AST_Node>(thisScope, prev_stack_top);
					}
				...
				bool DeclarationsBlock(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Char('{')) {
						...
						build_match<Declaration_Block_AST_Node>(currentScope, prev_stack_top);
					}
				...
				bool FunctionBlock(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Char('{')) {
						...
						build_match<Function_Block_AST_Node>(thisScope, prev_stack_top);
					}
				...
				bool Return(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Keyword("return")) {
						...
						build_match<Return_AST_Node>(currentScope, prev_stack_top);
						return true;
					}
					else {
						return false;
				...
				bool If(const std::shared_ptr<Scope>& currentScope) {
					...
					if (Keyword("if")) {
						...
						if ((is_if_init && num_children == 3) || (!is_if_init && num_children == 2)) {
							m_match_stack.push_back(ParseNode{ std::make_shared<Noop_AST_Node>(), currentScope });
						}
						...
						if (!is_if_init) {
							...
							build_match<If_AST_Node>(currentScope, prev_stack_top + 1);
							build_match<Block_AST_Node>(currentScope, prev_stack_top);
						}
					}
...
namespace GoodLang {
	namespace Impl {
		__forceinline void ToString(Tag< GoodLang::Engine::parser::Parser2::ParseNode >, GoodLang::Engine::parser::Parser2::ParseNode const& r, std::string& out) {
			out = r.first->to_string();
		};
		__forceinline void GetChildren(Tag< GoodLang::Engine::parser::Parser2::ParseNode >, GoodLang::Engine::parser::Parser2::ParseNode const& r, std::vector< NodeCache >& out) {
...
int main() {
	...
	if (1) {
		if (1) {
			...
			auto Interpretted = GoodLang::Engine::Preprocessor().Parse(R"(
				print(ONE_HUNDRED);
		...
		if (1) {
			GoodLang::Engine::Preprocessor::PreprocessorState state;
			auto Interpretted = GoodLang::Engine::Preprocessor().Parse(R"(
				#error "This error was thrown on purpose from the compiler"
			)");
			try {
				Interpretted->GenerateExpandedCode(state);
				print("ERROR WAS NOT THROWN!");
			}
			catch (GoodLang::Engine::exception::eval_error const& e) {
				print(e.what());
			}
		}
	...
	if (1) {
		if (1) {
			...
			print(ToString(Units::second(sw.Stop_s())) + " @ insert w/ Functions");
		}
		if (1) {
			Stopwatch sw;
			sw.Start();
		...
		if (1) {
			...
			for (auto x = funcs.find("int"), y = funcs.end(); x != y; ++x) {
				auto& func_name = x->get<0>();
				print(GoodLang::ToString(func_name) + ": " + GoodLang::ToString(x->get<2>()));
			}
			for (auto x = funcs.find("double"), y = funcs.end(); x != y; ++x) {
				auto& func_name = x->get<0>();
				print(GoodLang::ToString(func_name) + ": " + GoodLang::ToString(x->get<2>()));
			}
			// should be nothing in this list...
			for (auto x = funcs.find("to_string"), y = funcs.end(); x != y; ++x) {
				auto& func_name = x->get<0>();
				print(GoodLang::ToString(func_name) + ": " + GoodLang::ToString(x->get<2>()));
			}

			funcs.emplace("obj", std::make_shared<Any>(100.0f));
			if (auto* p = funcs.at("obj")) {
				if (!p->get<4>()) {
					print(ToString(p->get<3>()));
				}
			}
	...
	if (1) {
		...
		if (1) {
			...
			auto parse{ GoodLang::Engine::parser::Parser2() };
			...
			if (1) {
				...
				print(ToString(Units::second(sw.Stop_s())) + " @ for 60 basic game logic loops. Targetting < 1_s"); // 8 - 10 seconds
			}
