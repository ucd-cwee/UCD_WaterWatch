...
namespace GoodLang {
	...
	namespace Engine {
		...
		public:
			...
			template<typename T>
			static std::string_view GetText(T const& r) {
				std::string_view out;
			...
			*/
			...
			public:
				...
				public:
					...
					template<typename Callable>	static std::shared_ptr<AST_Node_Impl> make_compiled_node(const std::shared_ptr<Scope>& currentScope, AST_Node_Impl_Ptr original_node, std::vector<AST_Node_Impl_Ptr> children, Callable callable) {
...
namespace GoodLang {
	namespace Impl {
		__forceinline void ToString(Tag< GoodLang::Engine::Interpreter::parser::Parser2::ParseNode >, GoodLang::Engine::Interpreter::parser::Parser2::ParseNode const& r, std::string& out) {
			out = r.first->to_string();
		};
		__forceinline void GetChildren(Tag< GoodLang::Engine::Interpreter::parser::Parser2::ParseNode >, GoodLang::Engine::Interpreter::parser::Parser2::ParseNode const& r, std::vector< NodeCache >& out) {
...
int main() {
	...
	if (1) {
		...
		auto parsed_result = GoodLang::Engine::Interpreter::parser::Parser2().Parse(final_script);
		if (1) {
			print(final_script);
			Stopwatch sw;
			sw.Start();
	...
	if (1) {
		...
		if (1) {
			...
			auto parse{ GoodLang::Engine::Interpreter::parser::Parser2() };
