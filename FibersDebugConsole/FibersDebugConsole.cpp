...
#define push_back_if_not_at_end(vec, item) if (vec.size() <= 0) { vec.push_back(item); } else if (vec[vec.size()-1] != item) { vec.push_back(item); }
	...
	public:
		compound_string_view(std::string_view A = "", std::string_view B = "", std::string_view C = "") : a{ A }, b{ B }, c{ C } {};
		...
		compound_string_view substr(size_t offset = 0, size_t count = std::string::npos) const {
			compound_string_view copy = *this;
	...
	public:
		...
		class Callback {
		private:
			GoodLang::details::flat_map<BasicScope*, std::pair<std::weak_ptr<BasicScope>, std::function<void(BasicScope*)>>>
				listeners;

		public:
			// add a listener to the list
			void add_listener(std::shared_ptr<BasicScope> const& p, std::function<void(BasicScope*)>&& listener) {
				if (p) listeners.insert(p.get(), { p, std::move(listener) });
			};
			// callback performed on all listeners
			void speak() {
				std::vector< BasicScope* > deleteMe;
				for (auto& x : listeners) {
					if (auto L = x.second->first.lock()) {
						x.second->second(&*L);
					}
					else {
						deleteMe.push_back(*x.first);
					}
				}
				for (auto& x : deleteMe) listeners.erase(x);
			};
		...
		public:
			...
			static size_t			rFind(compound_string_view const& WITHIN, std::string_view const& FIND) {
				if (WITHIN.length() == 0 || FIND.length() == 0) return std::string::npos;
				if (FIND.length() > WITHIN.length()) return std::string::npos;
				return rFindString(WITHIN, FIND);
			};
			...
			static void RemoveLeadingAndTrailing(compound_string_view& x, char what) {
				while (
					(x.length() > 0)
					&& (x[0] == what)
					) {
			...
			static void RemoveTrailing(compound_string_view& x, char what) {
				while (
					(x.length() > 0)
					&& ((x[x.length() - 1] == what))
					) {
			...
			static void RemoveLeading(compound_string_view& x, char what) {
				while (
					(x.length() > 0)
					&& (x[0] == what)
					) {
		...
		public:
			...
			std::atomic<size_t>*
				child_versions;
			using cache_map = GoodLang::details::flat_map<size_t, Breadcrumb*>;
				...
				, check_flag{}
			{
				...
				if (this_m->scope_name != "") {
					if (parent_m) current_namespace = (std::string)CleanUpScopeName(parent_m->current_namespace + std::string(this_m->scope_name));
					else current_namespace = (std::string)CleanUpScopeName(this_m->scope_name);
				}
				else {
		...
		protected:			
			...
			concurrency::concurrent_unordered_map< size_t, ObjectWrapper >
		...
		public:
			...
			virtual void SetSelf(std::shared_ptr<BasicScope> const& Self) {
				...
				if (this->breadcrumb_m.parent_m) {
					if (this->breadcrumb_m.parent_m->this_m->scope_ptr) {
						this->breadcrumb_m.parent_m->this_m->scope_ptr->children_listeners.add_listener(Self, [](BasicScope* p) {
							p->UpdateObjectFunctionVersion();
						});
					}
				}
			};
			...
			virtual Breadcrumb* FindNearestScopeWhere(
				...
				if (using_m.size() > 0ull) {
					for (auto& childNamespace : using_m) {
						if (childNamespace) {
							...
							if (auto* ptr = childNamespace->this_m->scope_ptr) {
				...
				if (!(searchState & SkipParent)) {
					...
					while (thisParent = thisParent->parent_m) {
						...
						if (thisParent->has_usings > 0) {
							if (auto* ptr = thisParent->this_m->scope_ptr) {
				...
				if ((depth == 0) && SecondaryPriortyScope) {
					...
					if (thisParent) {
						...
						// test my personal "using" namespaces completely
						if (thisParent->has_usings > 0) {
							auto* thisParentPtr = thisParent->this_m->scope_ptr;
				...
				if (!(searchState & SkipParent)) {
					if (selfPtr.parent_m) {
						...
						if (!flag.second) {
							if (auto* ptr = selfPtr.parent_m->this_m->scope_ptr) {
			...
			static Breadcrumb* FindNamespace(compound_string_view const& Name, Breadcrumb* start) {
			...
			static Breadcrumb* FindClass(std::shared_ptr<Type_Info> const& type, Breadcrumb* start) {
			...
			std::shared_ptr<Any> FindObject(std::string_view Name) const {
				...
				if (auto f = this->objects_m.find(GetHash(Name)), e = this->objects_m.end(); f != e) {
					...
					Proxy_Function out2;
					(void)FindObjectOrFunction(Name, {}, out, out2, &*converter, SearchState::SkipChildren);
					return out;
				}
			};
			...
			Proxy_Function FindFunction(std::string_view Name, ParamTypes const& params) const {
				...
				Proxy_Function out2 = nullptr;
				// Only namespaces can have functions, so jump to it.
				if (auto* p = this->breadcrumb_m.namespace_m->this_m->scope_ptr) {
					(void)p->FindObjectOrFunction(Name, params, out, out2, &*converter);
				}
				else {
					(void)FindObjectOrFunction(Name, params, out, out2, &*converter);
				}
				return out2;
			};
			...
			bool FindObjectOrFunction(std::string_view Name, ParamTypes const& params, std::shared_ptr<Any>& out1, Proxy_Function& out2, TypeConverter* converter, int search_state = 0) const {
				...
				if (auto f = Breadcrumb::rFind(temp_name, "::"); f != std::string::npos) {
					...
					namespaceName_hash = namespaceName.hash();
				}
				else {
					objName = std::string(temp_name);
					namespaceName = compound_string_view("::");
					namespaceName_hash = default_namespace_hash;
				}
				
				size_t objNameHash{ GetHash(objName) };
				...
				if (1) {
					std::array<std::pair<double, Breadcrumb*>, 3> sort{ 
						{ {details::TypeConversionWorstCaseCost, nullptr}, {details::TypeConversionWorstCaseCost, nullptr}, {details::TypeConversionWorstCaseCost, nullptr} }
					};
					...
					if (1) {
						...
						if (FindNearestScopeWhere([&objNameHash, namespaceName_hash, namespaceName, &out1, &out2, &objName, &params, &searchDepth, &sort, converter](Breadcrumb* namespacePtr, int search_state) -> int {
							...
							if ((F != std::string::npos) && (F == (QualifiedNameLen - namespaceName.length()))) {
								...
								if (auto* scope_ptr = namespacePtr->this_m->scope_ptr) {
									if (!out1) {
										if (scope_ptr->objects_m.size() > 0) {
											auto f = scope_ptr->objects_m.find(objNameHash);
									...
									if (namespacePtr->this_m->is_namespace()) {
										if (auto* ptr = dynamic_cast<NamespaceScope*>(scope_ptr)) {
											if (!sort[0].second) {
												double cost{ details::TypeConversionWorstCaseCost };
												auto matched_pair = ptr->functions_m.BuildMatches(objName, const_cast<ParamTypes&>(params), *converter, &cost);
												if (matched_pair.first) {
													if (matched_pair.first->operator*().GetSignature().IsTemplate()) {
														if ((cost + searchDepth) < sort[1].first) {
															sort[1].first = cost + searchDepth++;
												...
												if (matched_pair.second) {
													if (matched_pair.second->operator*().GetSignature().IsTemplate()) {
														if ((cost + searchDepth) < sort[1].first) {
															sort[1].first = cost + searchDepth++;
															sort[1].second = &ptr->breadcrumb_m;
														}
												...
												double cost{ details::TypeConversionWorstCaseCost };
												if (auto& out = ptr->functions_m.BuildMatch(objName, const_cast<ParamTypes&>(params), *converter, false, true, &cost)) {
													if (cost == 0) {
														out1 = nullptr;
														out2 = out;
														return SearchResult::Success;
					...
					// Perhaps the user may have meant to call a constructor...
					if (auto* BC = this->FindClass(Name)) {
						if (BC->this_m->is_class()) {
							if (auto NS_ptr = dynamic_cast<NamespaceScope*>(BC->this_m->scope_ptr)) {
								double cost{ details::TypeConversionWorstCaseCost };
								if (auto& out = NS_ptr->functions_m.BuildMatch(NS_ptr->Name(), const_cast<ParamTypes&>(params), *converter, true, true, &cost)) {
			...
			std::shared_ptr<Any> EmplaceObject(std::string_view Name, Any const& object, int objectState = ObjectWrapper::ObjectState::Normal)  {
				auto sv = Breadcrumb::CleanUpScopeName(Name); // "npos" -> "::npos::"   OR    "std::string::npos" -> "::std::string::npos::"    
				Breadcrumb::RemoveTrailing(sv, ':'); // "::npos::" -> "::npos"   OR    "::std::string::npos::" -> "::std::string::npos"
				std::string_view objName;
				compound_string_view namespaceName;
				if (auto f = Breadcrumb::rFind(sv, "::"); f != std::string::npos) {
					objName = std::string(sv.substr(f + 2, sv.length() - f));
					namespaceName = sv.substr(0, f + 2);
				}
				auto objNameHash{ GetHash(objName) };
				std::shared_ptr<Any> out{ nullptr };
				if (auto BC = FindNearestScopeWhere([&](Breadcrumb* namespacePtr, int search_state)->int {
					...
					if (namespaceName.length() > 0) {
						...
						if ((F != std::string::npos) && (F == (QualifiedNameLen - namespaceName.length()))) {
							// the namespace is technically a candidate
							if (auto* ptr = namespacePtr->this_m->scope_ptr) {
								if (namespacePtr->this_m->is_class()) {
									if (objectState & ObjectWrapper::ObjectState::Static) {
										out = ptr->objects_m.insert({ objNameHash, ObjectWrapper(object, objectState) }).first->second.object;
									}
									else { // the user is adding a member object, NOT a static object. 
										if (auto* classPtr = dynamic_cast<ClassScope*>(ptr)) {
											if (classPtr->ClassType->IsBuiltInType()) {
												...
												classPtr->DeclareMemberObject(std::string(objName), object.Type(), std::make_shared<Any>(object));
											}
										}										
									}
								}
									...
									if (namespacePtr->this_m->is_namespace() && !namespacePtr->this_m->is_root()) {
										out = ptr->objects_m.insert({ objNameHash, ObjectWrapper(object, objectState | ObjectWrapper::ObjectState::Static) }).first->second.object;
									}
									else {
										out = ptr->objects_m.insert({ objNameHash, ObjectWrapper(object, objectState) }).first->second.object;
									}
			...
			bool EmplaceFunction(std::string_view Name, Function const& object)  {
				...
				if (auto BC = FindNearestScopeWhere([&namespaceName, &objName, &sv, &object](Breadcrumb* namespacePtr, int search_state)->int {
					...
					if (namespaceName.length() > 0) {
						...
						if ((F != std::string::npos) && (F == (QualifiedNameLen - namespaceName.length()))) {
							// the namespace is technically a candidate
							if (auto* ptr = dynamic_cast<NamespaceScope*>(namespacePtr->this_m->scope_ptr)) {
			...
			Any Call(std::string_view const& functionName, std::vector<Any> params) const {
				ParamTypes Params{ params };
				std::shared_ptr<Any> out1;
				Proxy_Function out2;
				
				auto converter = this->GetRoot()->GetTypeConverterTree();
				if (FindObjectOrFunction(functionName, Params, out1, out2, &*converter)) {
					if (out1) return out1;
				...
				for (auto& p : Params) {
					...
					if (auto ptr = this->FindClass(p.lock())) {
						className = std::string(ptr->this_m->scope_name);
					}
			...
			Any Call(Proxy_Function const& function, std::vector<Any> params) const {
				ParamTypes Params{ params };
				auto converter = this->GetRoot()->GetTypeConverterTree();
				return function->operator()(std::move(params), *converter);
			};
			...
			Any Cast(Any const& from, std::weak_ptr<Type_Info> const& To, bool requireExplicitReferenceToPreventInstancing = false) const {
				...
				if (ToClass) {
					...
					if (auto Tree2 = dynamic_cast<RootScope*>(ToClass->root_m->this_m->scope_ptr)->GetTypeConverterTree()) {
					...
					if (1) {
						...
						try {
							return this->Call(ToClass->this_m->scope_name, { from });
						}
						catch (exception::not_found_error) {}

						// call a functor from their scope
						try {
							if (auto* ptr = ToClass->this_m->scope_ptr) {
								return ptr->Call(ToClass->this_m->scope_name, { from });
							}							
						}
						catch (exception::not_found_error) {}
					}
			...
			std::string GetTypeName(std::weak_ptr<Type_Info> const& ti) const {
				if (auto p = ti.lock()) {
					if (auto ptr = this->FindClass(p)) {
						if (auto c = ptr->this_m->scope_ptr) {
							if (p->is_const()) {
								if (p->is_ref()) {
								...
								if (!type_ptr->is_any()) {
									if (type_ptr = function.m_function->GetSignature().Returns().lock()) {
										if (!type_ptr->is_any()) {
											...
											if (auto ptr = dynamic_cast<ClassScope*>(this->self_id_m.scope_ptr)) {
												if (name == ptr->Name()) {
		...
		public:
			...
			{
				if (this->breadcrumb_m.root_m) {
					if (this->breadcrumb_m.root_m->this_m) {
						if (auto ptr = std::dynamic_pointer_cast<RootScope>(this->breadcrumb_m.root_m->this_m->scope.lock())) {
							++ptr->child_versions;
						}
					}
				}
			...
			virtual Breadcrumb* FindNearestScopeWhere(
				...
				// test my personal "using" namespaces completely
				if (this->breadcrumb_m.has_usings > 0l) {
					if (using_m.size() > 0ull) {
						for (auto& childNamespace : using_m) {
							if (childNamespace) {
								auto& flag = childNamespace->check_flag[threadIndex];
				...
				if ((depth == 0) && SecondaryPriortyScope) {
					thisParent = SecondaryPriortyScope;
					if (thisParent) {
						auto& flag1 = thisParent->check_flag[threadIndex];
						flag1.first = flag1.second = false;
				...
				if (!RequestedSkipChildren && (!(searchState & SkipChildren)) && children_m.size() > 0ull) {
					...
					while (child_bc) {
						...
						if (auto ptr = child_bc->this_m->scope_ptr) {
			...
			std::shared_ptr<ClassScope> MakeChildClass(std::string_view name, std::vector<std::weak_ptr<ClassScope>> inheritance = {}) {
				...
				ptr->breadcrumb_m.class_type_hash = ptr->ClassType->uniqueHash;
				...
				for (auto& member_obj : p_declared_member_objects) {
					...
					if (auto memberObjectType = member_obj.second->first.lock()) {
						if (auto memberObjectClassType = this->FindClass(memberObjectType)) {
							...
							if (auto memberObjectClassTypePtr = memberObjectClassType->this_m->scope_ptr) {
...
#undef as_string
		...
		protected:
			...
			std::atomic<size_t>
				child_versions{ 0 }; // incremented whenever a child namespace (or class) is made
		...
		public:
			RootScope()
				: NamespaceScope(
					...
					nullptr)
			{
				this->breadcrumb_m.child_versions = &child_versions;
			};
						...
						// the "std" namespace imports the "string" namespace...
						{
							...
							// which has the following types groups... 
							{
								...
								if (auto p = std_namespace->FindClass(user_type_shared_ptr<double>())) {
									if (auto ptr = dynamic_cast<ClassScope*>(p->this_m->scope_ptr)) {
							...
							for (auto& unit_type : Units::value::GetValueTypes()) {
								...
								if (auto ClassBC = std_namespace->FindClass(type_info)) {
									if (auto Class = dynamic_cast<ClassScope*>(ClassBC->this_m->scope_ptr)) {
...
#undef Do
	...
	namespace Engine2 {		
		...
		public:
			...
			virtual void compile(std::shared_ptr<Scopes::BasicScope> const& currentScope) const = 0;
			virtual Any eval(std::shared_ptr<Scopes::BasicScope> const& currentScope) const = 0;
			virtual std::weak_ptr<Type_Info> return_type() const = 0;

			/// Prints the contents of an AST node, including its children, recursively
			std::string to_string(const std::string& t_prepend = "") const {
				std::ostringstream oss;
				std::string Text{ text };
				std::string str = GoodLang::ToString(std::string(identifier.ToString()));
				std::string returnType = GoodLang::ToString(return_type());
				std::string TimeSpentEvaling = GoodLang::ToString(TimeSpent_Total());
				std::string locationStr = location.to_string();
				oss << GoodLang::printf("%s(%s) [%s] %s : %s -> %s\n", t_prepend.c_str(), str.c_str(), TimeSpentEvaling.c_str(), Text.c_str(), locationStr.c_str(), returnType.c_str());
				for (auto& elem : get_children()) { oss << elem.get().to_string(t_prepend + "  "); }
				return oss.str();
			};
		...
		protected:
			AST_Node(std::string t_ast_node_text, Engine::AST_Node_Type t_id, Engine::Parse_Location t_loc)
				: identifier(t_id)
		...
		public:
			...
			struct eval_error : std::runtime_error {
				...
				eval_error(const std::string& t_why, const Engine::Position& t_where, const std::string& t_fname = "__EVAL__") noexcept
			...
			private:
				template<typename T>
				static Engine::AST_Node_Type id(const T& t) noexcept {
					return t.identifier;
				};
				...
				static std::string format_location(const Engine::Position& t_where) {
					std::stringstream ss;
					ss << "at (" << t_where.line << ", " << t_where.col << ")";
					return ss.str();
				};

				static std::string format(const std::string& t_why, const Engine::Position& t_where, const std::string& t_fname) {
					std::stringstream ss;
		...
		//struct ScriptTextNode {
		//	Engine::Parse_Location
		//		original_position; // e.g. 0 to 9
		//	std::string_view 
		//		original_text; // e.g. print(x)
		//	std::string 
		//		expanded_text; // e.g. std::cout << x << std::endl;
		//};
		//class ScriptText {
		//public:		
		//	std::string_view
		//		original_text; // e.g. print(x)
		//	std::vector< ScriptTextNode > 
		//		text_nodes;
		//	std::string_view get_original() const {
		//		return original_text;
		//	};
		//	std::string get_expanded() const {
		//		std::string expanded;
		//		for (auto& node : text_nodes) expanded += node.expanded_text;
		//		return expanded;
		//	};
		//	char get_expanded(long long index) const {
		//		for (auto& node : text_nodes) {
		//			if (node.expanded_text.length() > index) {
		//				return node.expanded_text[index];
		//			}
		//			else {
		//				index -= node.expanded_text.length();
		//			}
		//		}
		//		return '\0';
		//	};
		//	char get_original(long long index) const {
		//		for (auto& node : text_nodes) {
		//			if (node.original_text.length() > index) {
		//				return node.original_text[index];
		//			}
		//			else {
		//				index -= node.original_text.length();
		//			}
		//		}
		//		return '\0';
		//	};
		//	Engine::Parse_Location OriginalLocation_From_ExpandedIndex(long long index) const {
		//		for (auto& node : text_nodes) {
		//			if (node.expanded_text.length() > index) {
		//				return node.original_position;
		//			}
		//			else {
		//				index -= node.expanded_text.length();
		//			}
		...
		//	};
		//	ScriptText(std::string_view from) : original_text(from) {
		//		text_nodes.push_back({ Engine::Parse_Location(&from[0], &from[from.length()-1]), from, std::string(from) });
		//	}
		//};
		...
		public:
			...
			public:
				...
				public:
					std::vector<std::pair<std::string, Engine::Parse_Location>>
						Final_Script;
					std::map<
				...
				protected:
					PreprocessorToken(std::string_view t_ast_node_text, Engine::PreprocessorDirectives t_id, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
						: identifier(t_id)
						, text()
						, location(std::move(t_loc))
				...
				public:
					CompletedPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					NonePreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					DefinePreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					IncludePreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					UndefinePreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					IfChainPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
					...
					static bool Evaluate(PreprocessorState& state, Engine::PreprocessorDirectives const& NodeType, std::string_view Text) {
						...
						case Engine::PreprocessorDirectives::ElseIf: {
							...
							auto temp_master_scope = std::make_shared<Scopes::RootScope>();
				...
				public:
					IfPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					ElseIfPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					ElsePreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					EndIfPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					IfDefinedPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					IfNotDefinedPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					ErrorPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					WarningPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
				...
				public:
					PragmaPreprocessor(std::string_view t_ast_node_text, Engine::Parse_Location t_loc, std::vector<std::shared_ptr<PreprocessorToken>> t_children)
			...
			private: // TO-DO, reimpliment the optimization sequence inside of "build_match"
				...
				PreprocessorTokenPtr make_node(std::string_view t_match, Engine::Position t_prev, Param &&...param) {
			...
			public:
				...
				struct AST_Node_Impl : public AST_Node {
					...
					Any eval(std::shared_ptr<Scopes::BasicScope> const& currentScope) const override final {
					...
					void compile(std::shared_ptr<Scopes::BasicScope> const& currentScope) const override final {
						try {
					...
					std::weak_ptr<Type_Info> return_type() const override final {
						try {
				...
				protected:
					virtual Any eval_internal(std::shared_ptr<Scopes::BasicScope> const&) const {
						return Any();
						// throw std::runtime_error("Undispatched ast_node (internal error)");
					};
					virtual void compile_internal(std::shared_ptr<Scopes::BasicScope> const& currentScope) const {
						for (auto& child : children) child->compile(currentScope);
					};
					virtual std::weak_ptr<Type_Info> return_type_internal() const {
						return user_type_shared<void>();
					};
				};
				class AST_Nodes {
				public:
					...
					struct File_AST_Node final : AST_Node_Impl {
						File_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Noop_AST_Node final : AST_Node_Impl {
						Noop_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc)
					...
					struct Return_AST_Node final : AST_Node_Impl {
						Return_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Constant_AST_Node final : public AST_Node_Impl {
						Constant_AST_Node(std::string t_ast_node_text, Engine::Parse_Location t_loc, Any t_value)
						...
						Any eval_internal(const std::shared_ptr<Scopes::BasicScope>&) const override {
							return m_value;
						}
					...
					struct Binary_Operator_AST_Node : AST_Node_Impl {
						Binary_Operator_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, const std::string& t_oper, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Id_AST_Node : AST_Node_Impl {
						Id_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, const std::string& t_ast_node_text, Engine::Parse_Location t_loc)
					...
					struct FunctionName_AST_Node final : Id_AST_Node {
						FunctionName_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, const std::string& t_ast_node_text, Engine::Parse_Location t_loc)
					...
					struct VariableName_AST_Node final : Id_AST_Node {
						VariableName_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, const std::string& t_ast_node_text, Engine::Parse_Location t_loc)
							: Id_AST_Node(currentScope, t_ast_node_text, std::move(t_loc))
						{
					...
					struct ClassName_AST_Node final : Id_AST_Node {
						ClassName_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, const std::string& t_ast_node_text, Engine::Parse_Location t_loc)
							: Id_AST_Node(currentScope, t_ast_node_text, std::move(t_loc))
						{
							type = IdType::Class;
						}
					...
					struct Arg_AST_Node final : AST_Node_Impl {
						Arg_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Arg_List_AST_Node final : AST_Node_Impl {
						Arg_List_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
							: AST_Node_Impl(std::move(t_ast_node_text), Engine::AST_Node_Type::Arg_List, std::move(t_loc), std::move(t_children))
						{};

						static std::string_view get_arg_name(const AST_Node_Impl& t_node) {
						...
						static std::vector<std::string_view> get_arg_names(const AST_Node_Impl& t_node) {
					...
					struct Fun_Call_AST_Node : AST_Node_Impl {
						Fun_Call_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
						...
						Any do_eval_internal(const std::shared_ptr<Scopes::BasicScope>& currentScope) const {
					...
					struct Unused_Return_Fun_Call_AST_Node final : Fun_Call_AST_Node {
						Unused_Return_Fun_Call_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Equation_AST_Node final : AST_Node_Impl {
						Equation_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Logical_And_AST_Node final : AST_Node_Impl {
						Logical_And_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Logical_Or_AST_Node final : AST_Node_Impl {
						Logical_Or_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					*/
					struct Var_Decl_AST_Node final : AST_Node_Impl {
						Var_Decl_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					*/
					struct Assign_Retroactively_AST_Node final : AST_Node_Impl {
						Assign_Retroactively_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					*/
					struct Assign_Decl_AST_Node final : AST_Node_Impl {
						Assign_Decl_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Prefix_AST_Node final : AST_Node_Impl {
						Prefix_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Postfix_AST_Node final : AST_Node_Impl {
						Postfix_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct If_AST_Node final : AST_Node_Impl {
						If_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct While_AST_Node final : AST_Node_Impl {
						While_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct For_AST_Node final : AST_Node_Impl {
						For_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Ranged_For_AST_Node final : AST_Node_Impl {
						Ranged_For_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Array_Call_AST_Node final : AST_Node_Impl {
						Array_Call_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Dot_Access_AST_Node final : AST_Node_Impl {
						Dot_Access_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Lambda_AST_Node final : AST_Node_Impl {
						Lambda_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
						...
						Any eval_internal(const std::shared_ptr<Scopes::BasicScope>& currentScope) const override {
							...
							std::map<std::string_view, std::shared_ptr<Any>>
								captures;
					...
					private:
						...
						const std::vector<std::string_view> m_param_names;
						const std::shared_ptr<AST_Node_Impl> m_lambda_node;
					};
					// [0, 1, 2, 3]
					struct Inline_Array_AST_Node final : AST_Node_Impl {
						Inline_Array_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Map_Pair_AST_Node final : AST_Node_Impl {
						Map_Pair_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Inline_Map_AST_Node final : AST_Node_Impl {
						Inline_Map_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Parallel_For_AST_Node final : AST_Node_Impl {
						Parallel_For_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Parallel_Ranged_For_AST_Node final : AST_Node_Impl {
						Parallel_Ranged_For_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Break_AST_Node final : AST_Node_Impl {
						Break_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
							: AST_Node_Impl(std::move(t_ast_node_text), Engine::AST_Node_Type::Break, std::move(t_loc), std::move(t_children)) {
						}

						Any eval_internal(const std::shared_ptr<Scopes::BasicScope>& currentScope) const override { throw detail::Break_Loop(); }
					};

					struct Continue_AST_Node final : AST_Node_Impl {
						Continue_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
							: AST_Node_Impl(std::move(t_ast_node_text), Engine::AST_Node_Type::Continue, std::move(t_loc), std::move(t_children)) {
						}

						Any eval_internal(const std::shared_ptr<Scopes::BasicScope>& currentScope) const override { throw detail::Continue_Loop(); }
					};

					struct Case_AST_Node final : AST_Node_Impl {
						Case_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Switch_AST_Node final : AST_Node_Impl {
						Switch_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Default_AST_Node final : AST_Node_Impl {
						Default_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Do_AST_Node final : AST_Node_Impl {
						Do_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Try_AST_Node final : AST_Node_Impl {
						Try_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
							: AST_Node_Impl(std::move(t_ast_node_text), Engine::AST_Node_Type::Try, std::move(t_loc), std::move(t_children))
						{}

						Any handle_exception(const std::shared_ptr<Scopes::BasicScope>& currentScope, const Any& t_except, bool& rethrow) const {
							Any retval;
					...
					struct Catch_AST_Node final : AST_Node_Impl {
						Catch_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Finally_AST_Node final : AST_Node_Impl {
						Finally_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct JustInTimeCompilation_AST_Node final : AST_Node_Impl {
						JustInTimeCompilation_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					private:
						...
						> TLS;
						void Compile(std::string const& SCRIPT, const std::shared_ptr<Scopes::BasicScope>& currentScope, bool UpdateAll = false) const {
							if (UpdateAll) {
					...
					struct Scopeless_Block_AST_Node final : AST_Node_Impl {
						Scopeless_Block_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Block_AST_Node final : AST_Node_Impl {
						Block_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Function_Block_AST_Node final : AST_Node_Impl {
						Function_Block_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Fold_Right_Binary_Operator_AST_Node : AST_Node_Impl {
						Fold_Right_Binary_Operator_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, const std::string& t_oper, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children, Any t_rhs)
					...
					struct Namespace_AST_Node final : AST_Node_Impl {
						Namespace_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct Class_AST_Node final : AST_Node_Impl {
						Class_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
							: AST_Node_Impl(std::move(t_ast_node_text), Engine::AST_Node_Type::Namespace, std::move(t_loc), std::move(t_children))
						{
							(void)this->children.back()->eval(currentScope);
						}
					...
					struct Declaration_Block_AST_Node final : AST_Node_Impl {
						Declaration_Block_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
					...
					struct FunctionDecl_AST_Node final : AST_Node_Impl {
						FunctionDecl_AST_Node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string t_ast_node_text, Engine::Parse_Location t_loc, std::vector<AST_Node_Impl_Ptr> t_children)
						...
						std::vector<std::string_view> inputArgNames;
						std::weak_ptr<Type_Info> returnArgType;
					};
				};
				...
				private:
					template<typename... T> struct Optimizer : T... {
						...
						AST_Node_Impl_Ptr optimize(AST_Node_Impl_Ptr p, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
					...
					struct Example {
						bool optimize(AST_Node_Impl_Ptr& p, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
					...
					struct ArgListFileConstant {
						bool optimize(AST_Node_Impl_Ptr& node, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
				...
				public:
					static AST_Node_Impl_Ptr optimize(AST_Node_Impl_Ptr p, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
				...
				public:
					using ParseNode = std::pair< std::shared_ptr<AST_Node_Impl>, std::shared_ptr<Scopes::BasicScope> >;
				private:
					Engine::Position m_position{};
				...
				private:
					...
					static void validate_object_name(std::string_view const& name, Engine::Position const& m_position) {
				...
				private:
					/// Helper function that collects ast_nodes from a starting position to the top of the stack into a new AST node
					template<typename NodeType> std::shared_ptr<NodeType> build_match(const std::shared_ptr<Scopes::BasicScope>& currentScope, size_t t_match_start, std::string t_text = "") {
						bool is_deep = false;
					...
					/// create a node
					template<typename T, typename... Param> std::shared_ptr<AST_Node_Impl> make_node(const std::shared_ptr<Scopes::BasicScope>& currentScope, std::string_view t_match, Engine::Position t_prev, Param &&...param) {
						auto out = std::make_shared<T>(
							currentScope,
							std::string(t_match),
							Engine::Parse_Location(t_prev, m_position),
					...
					/// create a node
					template<typename... Param> std::shared_ptr<AST_Nodes::Constant_AST_Node> make_const(std::string_view t_match, Engine::Position t_prev, Param &&...param) {
				...
				private:
					...
					bool SkipComment() {
						...
						if (Symbol_(m_multiline_comment_begin)) {
							...
							std::string_view comment = Engine::Position::str(start, m_position);
					...
					bool Quoted_String(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS();
					...
					bool Id(const bool validate, const std::shared_ptr<Scopes::BasicScope>& currentScope, AST_Nodes::IdType T) {
						SkipWS();
						const auto prev_stack_top = m_match_stack.size();
					...
					bool TypeName(const std::shared_ptr<Scopes::BasicScope>& currentScope, bool allowAuto = false) {
					...
					bool Arg(const std::shared_ptr<Scopes::BasicScope>& currentScope, const bool t_type_allowed = true) {
						const auto prev_stack_top = m_match_stack.size();
						const auto prev_pos = m_position;
					...
					bool Id_Arg_List(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS(true);
					...
					bool Decl_Arg_List(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS(true);
					...
					bool Arg_List(const std::shared_ptr<Scopes::BasicScope>& currentScope, int maxNumArgs = std::numeric_limits<int>::max()) {
						SkipWS(true);
						bool retval = false;
					...
					bool TypeCastOperation(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool Equation(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
						using SS = utility::Static_String;
					...
					// int x;
					// int const& x;
					bool SpecifiedType_Var_Decl(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool Var_Decl(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
					...
					bool Prefix(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
						using SS = utility::Static_String;
					...
					bool Postfix(const std::shared_ptr<Scopes::BasicScope>& currentScope, bool gotValueAlready) {
						const auto prev_stack_top = m_match_stack.size();
						const auto prev_pos = m_position;
					...
					bool Map_Pair(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS(true);
						bool retval = false;
					...
					bool Container_Arg_List(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS(true);
						bool retval = false;
					...
					bool Inline_Container(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
					...
					bool Lambda(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
						const auto prev_pos = m_position;
					...
					bool Dot_Fun_Array(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
					...
					bool Value(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
					...
					bool Operator(const std::shared_ptr<Scopes::BasicScope>& currentScope, const size_t t_precedence = 0) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
					...
					bool Paren_Expression(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						if (Char('(')) {
							SkipWS(true);
							if (!Operator(currentScope)) {
					...
					bool While(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool For_Guards(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
					...
					/// Reads the C-style `for` conditions from input
					bool Parallel_For_Guards(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						if (!(Equation(currentScope) && Eol())) {
							if (!Eol()) {
								return false;
							}
					...
					bool Range_Expression(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
					...
					bool For(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
					...
					bool Break(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
						if (Keyword("break")) {
					...
					bool Continue(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
						if (Keyword("continue")) {
					...
					bool Case(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool Switch(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
					...
					bool Try(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
						if (Keyword("try")) {
							retval = true;
					...
					bool Eval(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
						if (Keyword("evaluate")) {
							retval = true;
					...
					/// namespace Thing{ ... };
					bool DeclClass(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
					...
					/// Reads a namespace block from input
					/// namespace Thing{ ... };
					bool DeclNamespace(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
					...
					/// Type Foo(...){ ... };
					bool DeclFunction(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
						const auto prev_stack_top = m_match_stack.size();
						const auto prev_pos = m_position;
					...
					/// Top level parser, starts parsing of all known parses
					bool Declarations(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS();
					...
					/// Top level parser, starts parsing of all known parses
					bool Statements(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						SkipWS();
					...
					bool Block(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool DeclarationsBlock(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool FunctionBlock(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
					...
					bool Return(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						const auto prev_stack_top = m_match_stack.size();
						if (Keyword("return")) {
					...
					bool If(const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						bool retval = false;
				...
				public:
					...
					ParseNode Parse(const std::string& t_input, std::shared_ptr<Scopes::BasicScope> parentScope = nullptr) {
						if (!parentScope) {
				...
				private:
					ParseNode parse(const std::string& t_input, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						return parse_internal(t_input, currentScope);
					};
					ParseNode parse_internal(const std::string& t_input, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
						auto this_scope = currentScope->MakeChildScope();
					...
					ParseNode parse_instr_eval(const std::string& t_input, const std::shared_ptr<Scopes::BasicScope>& currentScope) {
...
int main() {
	...
	while (1) {
		...
		if (1) {
			...
			Engine2::Compiler::Preprocessor::PreprocessorState state;
			if (auto preprocessor_result = Engine2::Compiler::Preprocessor().Parse(Script)) {
		...
		if (1) {
			if (1) {
				...
				for (auto& script : DebugScripts) {
					...
					Engine2::Compiler::Preprocessor::PreprocessorTokenPtr Interpretted;
					Engine2::Compiler::Interpreter::Parser::ParseNode parsed_result;
					try {
						Interpretted = GoodLang::Engine2::Compiler::Preprocessor().Parse(script);
						...
						parsed_result = GoodLang::Engine2::Compiler::Interpreter::Parser().Parse(expandedScript, this_scope);
						Stopwatch sw;
						sw.Start();
					...
					catch (GoodLang::Engine2::exception::eval_error& e) {
						// print(ToString(parsed_result));
						print(expandedScript);

						if (e.start_position.pos >= 0 && e.start_position.pos < expandedScript.length()) {
							print("####");
							print(expandedScript.substr(0, e.start_position.pos));
							print(GoodLang::ToString(" __ ||| __ ") + e.reason);
							print(expandedScript.substr(e.start_position.pos));
							print("####");
						}
						else {
							print("####");
