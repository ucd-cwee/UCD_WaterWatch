...
namespace epanet {
    class epanet_postcompiled { public:
            ...
            */
        {
            ...
            for (auto& N : pr->network->Node) { if (N) N->Zone = nullptr; } pr->network->Node.Clear();
            ...
            */
        {
            ...
            if (!errcode)
            {
                ...
                for (auto& N : pr->network->Node) { if (N) N->Zone = nullptr; } pr->network->Node.ClearedResize(n); for (auto& x : pr->network->Node) { if (!x) { x = make_cwee_shared<Snode>(); } } net->Node[0] = nullptr;
            ...
            //--------------------------------------------------------------
        {
            ...
            // Already freed
            if (pr->network->Rule.Num() == 0) return;
        ...
        static void     parseZone_Basic(EN_Project const& pr, Pzone const& zone, cweeList<Pnode>& nodes, int nodeIndex, u64 const& startTime) {
                ...
                while (adj) {
                    if (links[adj->link]) {                      
                        if (links[adj->link]->N1 == nodeIndex) {
                            zone->Boundary_Link.Append(cweeUnion<Plink, direction_t>(pr->network->Link[adj->link], direction_t::FLOW_OUT_DMA));
                        }
                        else if (links[adj->link]->N2 == nodeIndex) {
                            zone->Boundary_Link.Append(cweeUnion<Plink, direction_t>(pr->network->Link[adj->link], direction_t::FLOW_IN_DMA));
                        }
                        else {
        ...
        static void     combineZonesActual(cweeList<Pzone>& zones, Pzone const& first, Pzone const& second) {            
            ...
            for (auto& L1 : second->Boundary_Link) {
                ...
                if (isUnique) {
                    ...
                    if (i >= 0) {
                        ...
                        first->Within_Link.Append(L1.get<0>());
                    }
                }
            }
        ...
        static int      getAllSharedLinks(Pzone const& zone_1, Pzone const& zone_2, cweeList<Plink>& out, std::unordered_map<Slink*, bool>& map) {
#if 1
            auto& zone1Boundary = zone_1->Boundary_Link; 
            int N = 0;         
#if 0
            map.clear();
            map.reserve(zone1Boundary.Num());
            out.SetGranularity(cweeMath::max(zone1Boundary.Num(), out.GetGranularity()));
            for (auto& boundary_link : zone1Boundary) {
                auto& p = boundary_link.get<0>();
                map.emplace(p.Get(), 0);
            }
            for (auto& boundary_link : zone_2->Boundary_Link) {
                auto& p = boundary_link.get<0>();
                if (map.count(p.Get()) > 0) {
                    out.Emplace(p, N);
                    N++;
                }
            }    
            ...
            return N;
#else
            auto& zone2Boundary = zone_2->Boundary_Link; int i; int N = 0;
            out.SetGranularity(cweeMath::max(zone2Boundary.Num(), out.GetGranularity()));
            cweeList<int> indexes; indexes.AssureSize(zone2Boundary.Num());
            for (i = indexes.Num() - 1; i >= 0; i--) indexes[i] = i;

            for (auto& boundary_link1 : zone_1->Boundary_Link) {
                auto* bLp = boundary_link1.get<0>().Get();
                for (i = indexes.Num() - 1; i >= 0; i--) {
                    auto& boundary_link2 = zone2Boundary[indexes[i]];
                    if (boundary_link2.get<0>().Get() == bLp) { // SHARED!                        
                        out.Emplace(boundary_link2.get<0>(), N);
                        N++;
                        indexes.RemoveIndexFast(i);
                        break;
                    }
                }
            }
        ...
        static bool     combineZones(EN_Project const& pr, cweeList<Pzone>& zones, cweeList<Plink>& sharedLinks, std::unordered_map<Slink*, bool>& sharedLinksMap, std::unordered_map<Slink*, StatusType>& sharedLinksStatusMap, std::unordered_map<Slink*, bool>& sharedLinksContolledMap) {
            // outter loop
        ...
        static bool     isZoneWaterFlowPossible(EN_Project const& pr, Pzone& zone, std::unordered_map<Slink*, bool>& sharedLinksContolledMap, std::unordered_map<Slink*, StatusType>& sharedLinksStatusMap) {
            AUTO startTime = (u64)pr->times.GetSimStartTime();
            ...
            for (auto& boundary_link : zone->Boundary_Link) {
                ...
                if (link) {
                    if (link->IsBiDirectionalPipe()) { // pipe or TCV
                        ...
                        if (boundary_link.get<1>() == direction_t::FLOW_IN_DMA) {
                            if (sharedLinksContolledMap[link.Get()]) { return true; }
                            else {
                                // is it closed? 
                                if (sharedLinksStatusMap[link.Get()] == StatusType::CLOSED) {
        ...
        static int      parseNetwork(EN_Project pr) {
            ...
            // Evaluate all links and nodes and generate a list of zones
            {
                    ...
                    if (node->Type_p == asset_t::RESERVOIR) {                        
                        if (AUTO resP = node.CastReference<Stank>()) {
                            if (resP->Diameter != 0_ft) {
                                ...
                                parseZone_Basic(pr, zone, nodes, i, startTime);
                ...
                for (auto& link : links) if (link) sharedLinksContolledMap.emplace(link.Get(), IsLinkControlled(pr, link));
                ...
                while (combineZones(pr, zones, sharedLinks, sharedLinksMap, sharedLinksStatusMap, sharedLinksContolledMap)) {}
                ...
                for (auto& zone : zones) {  
                    ...
                    for (auto& L : zone->Boundary_Link) {
                        if (!done && L.get<1>() == direction_t::FLOW_IN_DMA) {
                ...
                for (auto& zone : zones) {
                    if (isZoneWaterFlowPossible(pr, zone, sharedLinksContolledMap, sharedLinksStatusMap)) {
            ...
            */
        {
            ...
            else
            {
                ...
                curve = net->Curve[i];
                if (curve->Comment.IsEmpty() && parser->Comment[0])
                {
