    ...
    class epanet_postcompiled { public:
        static void		initrules(EN_Project pr)
                ...
                //--------------------------------------------------------------
            {
                ...
                pr->rules.LastPremise = nullptr;
                pr->rules.LastThenAction = nullptr;
                pr->rules.LastElseAction = nullptr;
                pr->rules.ActionList = nullptr;
        ...
        static void		initpointers(EN_Project pr)
            ...
            **  Input:   none
            **  Output:  none
            **  Purpose: initializes data array pointers to nullptr
            **----------------------------------------------------------------
            */
        {
            ...
            pr->network->NodeHashTable.Clear();
            pr->network->LinkHashTable.Clear();
            ...
            */
        {
            ...
            if (!errcode)
            {
                ...
                pr->network->Node.ClearedResize(n);
            ...
            if (!errcode)
            {
                pr->network->Tank.ClearedResize(pr->parser.MaxTanks + 1);
                pr->network->Pump.ClearedResize(pr->parser.MaxPumps + 1);
                pr->network->Valve.ClearedResize(pr->parser.MaxValves + 1);
            ...
            if (!errcode)
            {
                ...
                {
                    pr->network->Node[n].D = nullptr;    // node demand
        ...
        static void		freedemands(Snode* node)
            /*----------------------------------------------------------------
        ...
        static void		freelinkvertices(Slink* link)
            /*----------------------------------------------------------------
            ...
            */
        {
            ...
            if (pr->network->Pattern.Num() > 0)
            {
                for (j = 0; j <= pr->network->Npats; j++)
                {
                    pr->network->Pattern[j].F.Clear();
                }
                pr->network->Pattern.Clear();
            }

            // Free memory for curves
            if (pr->network->Curve.Num() > 0)
            {
                // There is no Curve[0]
                for (j = 1; j <= pr->network->Ncurves; j++)
                {
                    pr->network->Curve[j].X.Clear();
                    pr->network->Curve[j].Y.Clear();
                }
                pr->network->Curve.Clear();
            }
            ...
            if (pr->network->NodeHashTable.Num() > 0)
            {
                hashtable_t::hashtable_free(pr->network->NodeHashTable);
            }
            if (pr->network->LinkHashTable.Num() > 0)
            {
        ...
        static int		openfiles(EN_Project pr, const char* f1, const char* f2, const char* f3)
            ...
            */
        {
            // Initialize file pointers to nullptr
            pr->parser.InFile = nullptr;
            pr->report.RptFile = nullptr;
            pr->outfile.OutFile = nullptr;
            pr->outfile.HydFile = nullptr;
            pr->outfile.TmpOutFile = nullptr;
            ...
            if (strlen(f1) > 0)
            {
                if ((pr->parser.InFile = fopen(f1, "rt")) == nullptr) return 302;
            }
            if (strlen(f2) == 0) pr->report.RptFile = stdout;
            else
            {
                pr->report.RptFile = fopen(f2, "wt");
                if (pr->report.RptFile == nullptr) return 303;
            }
            writelogo(pr);
            return 0;
        };
            ...
            */
        {
            ...
            if (pr->outfile.HydFile == nullptr) return 305;
            ...
            */
        {
            ...
            {
                if (pr->outfile.TmpOutFile != nullptr)
                {
                    fclose(pr->outfile.TmpOutFile);
                    pr->outfile.TmpOutFile = nullptr;
                }
            }
            if (pr->outfile.OutFile != nullptr)
            {
            ...
            */
        {
            ...
            if (pr->outfile.OutFile == nullptr) return 304;
            ...
            if (!errcode)
            {
                if (pr->report.Tstatflag != SERIES)
                {
                    ...
                    if (pr->outfile.TmpOutFile == nullptr) errcode = 304;
                }
            ...
            */
        {
            ...
            for (i = 1; i <= net->Nrules; i++)
            {
                ...
                auto premise = net->Rule[i].Premises;
                ...
                if (objType == LINK)
                {
                    ...
                    {
                        auto action = net->Rule[i].ThenActions;
                    ...
                    }
                    {
                        ...
                        auto action = net->Rule[i].ElseActions;
                        while (action != nullptr)
                        {
                            if (action->link == index) return 1;
                            action = action->next;
            ...
            */
        {
            ...
            for (j = 1; j <= network->Nnodes; j++)
            {
                ...
                for (demand = network->Node[j].D; demand != nullptr; demand = demand->next)
                {
            ...
            //----------------------------------------------------------------
        {
            ...
            case NODE:
                ...
                currentcomment = network->Node[index].Comment.c_str();
                break;
            case LINK:
                if (index < 1 || index > network->Nlinks) return 251;
                currentcomment = network->Link[index].Comment.c_str();
                break;
            case TIMEPAT:
                if (index < 1 || index > network->Npats) return 251;
                currentcomment = network->Pattern[index].Comment.c_str();
                break;
            case CURVE:
                if (index < 1 || index > network->Ncurves) return 251;
                currentcomment = network->Curve[index].Comment.c_str();
                break;
            default:
                strcpy(comment, "");
                return 251;
            ...
            if (currentcomment.Length() > 0) strcpy(comment, currentcomment.c_str());
            else comment[0] = '\0';
            return 0;
        };
            ...
            //----------------------------------------------------------------
        {
            ...
            case NODE:
                if (index < 1 || index > network->Nnodes) return 251;
                network->Node[index].Comment = newcomment;
                return 0;

            case LINK:
                if (index < 1 || index > network->Nlinks) return 251;
                network->Link[index].Comment = newcomment;
                return 0;
        ...
        static int      addpattern(EN_Network network, cweeStr const& id)
            /*
            **-------------------------------------------------------------
            ...
            */
        {
            ...
            if (n > 0)
            {
                if (network->Pattern[n].ID == id) return 0;
        ...
        static int      addcurve(EN_Network network, const char* id)
            /*
            **-------------------------------------------------------------
            ...
            */
        {
            ...
            if (n > 0)
            {
                if (network->Curve[n].ID == id) return 0;
            ...
            */
        {
            ...
            pattern->Length = 1;
            pattern[0].F.ClearedResize(1);
            pattern[0].F[0] = 1.0;
            parser->MaxPats = pr->network->Npats;

            // Make a pass through input file counting number of each object
            if (parser->InFile == nullptr) return 0;
            while (fgets(line, MAXLINE, parser->InFile) != nullptr)
            {
            ...
            */
        {
            ...
            parser->X.ClearedResize(MAXTOKS);
            if (errcode) return errcode;
            ...
            */
        {
            ...
            if (!epanet_shared::adddemand(node, y, p, nullptr)) return 101;
            ...
            */
        {
            ...
            node->S = nullptr;
            node->Ke = 0.0;
            node->Type = (diam == (decltype(diam))0) ? RESERVOIR : TANK;
            node->Comment = parser->Comment;
            tank->Node = i;
            tank->H0 = initlevel;
            tank->Hmin = minlevel;
            ...
            */
        {
            ...
            n1 = pattern->Length;
            pattern->Length += n;
            pattern->F.AssureSize(pattern->Length);
            ...
            */
        {
            ...
            if (parser->PrevCurve && strcmp(parser->Tok[0], parser->PrevCurve->ID.c_str()) == 0)
            {
            ...
            */
        {
            ...
            else
            {
                ...
                else
                {
                    for (j = 1; j <= net->Nnodes; j++)
                    {
                        if ((strcmp(parser->Tok[0], Node[j].ID.c_str()) <= 0) &&
                            (strcmp(parser->Tok[1], Node[j].ID.c_str()) >= 0)
                            ) Node[j].C0 = c0;
                    }
                }
            }
            ...
            */
        {
            ...
            if (item == 3)
            {
                ...
                {
                    if ((strcmp(parser->Tok[1], net->Node[j].ID.c_str()) <= 0) &&
                        (strcmp(parser->Tok[2], net->Node[j].ID.c_str()) >= 0)
                        ) net->Tank[j - net->Njuncs].Kb = y;
                }
            }
            ...
            */
        {
            ...
            {
                if ((strcmp(parser->Tok[0], net->Link[j].ID.c_str()) <= 0) &&
                    (strcmp(parser->Tok[1], net->Link[j].ID.c_str()) >= 0)
                    ) changestatus(net, j, (StatusType)status, y);
            }
            return 0;
        };
            ...
            */
        {
            ...
            else if (match(parser->Tok[0], w_QUALITY))
            {
                ...
                if (qual->Qualflag == TRACE_QUAL)
                {
                    ...
                    qual->TraceNode = epanet_shared::findnode(net, parser->Tok[2]);
            ...
            //----------------------------------------------------------
        {
            ...
            rule->ThenActions = nullptr;
            rule->ElseActions = nullptr;
            rule->priority = 0.0;
            pr->rules.LastPremise = nullptr;
            pr->rules.LastThenAction = nullptr;
            pr->rules.LastElseAction = nullptr;
            ...
            //---------------------------------------------------------------------
        {
            ...
            AUTO p = make_cwee_shared<Spremise>();
            if (p == nullptr) return 101;
            p->object = i;
            p->index = j;
            p->variable = v;
            p->relop = r;
            ...
            p->next = nullptr;
            if (rules->LastPremise == nullptr) net->Rule[net->Nrules].Premises = p;
            ...
            //----------------------------------------------------------
        {
            ...
            AUTO a = make_cwee_shared<Saction>();
            if (a == nullptr) return 101;
            a->link = j;
            a->status = s;
            a->setting = x;
            ...
            {
                a->next = nullptr;
                if (rules->LastThenAction == nullptr)
                {
            ...
            {
                a->next = nullptr;
                if (rules->LastElseAction == nullptr)
                {
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                if (errcode)
                {
                    ...
                    sprintf(pr->Msg, "Error %d: %s %s",
                        errcode, geterrmsg(errcode, errmsg), net->Link[k].ID.c_str());
                    epanet_shared::writeline(pr, pr->Msg);
                    return 200;
                }
            }
            ...
            */
        {
            ...
            net->Node[n].ID = id;
            hashtable_t::hashtable_insert(net->NodeHashTable, net->Node[n].ID.c_str(), n);
            return 0;
        };
            ...
            */
        {
            ...
            net->Link[n].ID = id;
            hashtable_t::hashtable_insert(net->LinkHashTable, net->Link[n].ID.c_str(), n);
            return 0;
        };
            ...
            //-----------------------------------------------------------
        {
            ...
            lastRule->Premises = nullptr;
            lastRule->ThenActions = nullptr;
            lastRule->ElseActions = nullptr;
        ...
        static cweeSharedPtr<Spremise> getpremise(cweeSharedPtr<Spremise> premises, int i)
            //----------------------------------------------------------
        ...
        static cweeSharedPtr<Saction> getaction(cweeSharedPtr<Saction> actions, int i)
            //----------------------------------------------------------
        ...
        {
            Report* rpt = &pr->report;
            if (rpt->RptFile == nullptr) return 0;
            if (freopen(rpt->Rpt1Fname, "w", rpt->RptFile) == nullptr) return 303;
            //writelogo(pr);
            return 0;
        };
            ...
            */
        {
            ...
            if (rpt->RptFile == nullptr) return 303;
            return 0;
        };
            ...
            */
        {
            ...
            if (type == NODEHDR)
            {
                ...
                for (i = ELEV; i < QUALITY; i++)
                {
                    ...
                    {
                        n += 10;
                        sprintf(s, "%10s", rpt->Field[i].Name.c_str());
                        strcat(s2, s);
            ...
            */
        {
            ...
            else if (qual->Qualflag == TRACE_QUAL) sprintf(s, FMT31, net->Node[qual->TraceNode].ID.c_str());
            ...
            */
        {
            ...
            if (outFile == nullptr) outFile = fopen(pr->outfile.OutFname, "rb");
            if (outFile == nullptr) return 106;
            ...
            */
        {
            ...
            rpt->Fprinterr = FALSE;
            if (rpt->Rptflag && strlen(rpt->Rpt2Fname) == 0 && rpt->RptFile != nullptr)
            {
            ...
            else if (strlen(rpt->Rpt2Fname) > 0)
            {
                ...
                else
                {
                    ...
                    tfile = rpt->RptFile;
                    tflag = rpt->Rptflag;
                    if ((rpt->RptFile = fopen(rpt->Rpt2Fname, "wt")) == nullptr)
                    {
                        rpt->RptFile = tfile;
                        rpt->Rptflag = tflag;
                        errcode = 303;
