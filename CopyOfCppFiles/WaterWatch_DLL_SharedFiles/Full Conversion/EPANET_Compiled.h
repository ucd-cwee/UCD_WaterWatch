...
#pragma region Shared Functions
        ...
        static int		findpattern(EN_Network network, const char* id)
            /*----------------------------------------------------------------
        ...
        static void     writeline(EN_Project pr, const char* s)
            /*
            **--------------------------------------------------------------
            **   Input:   *s = text string
            **   Output:  none
        ...
        static int		findnode(EN_Network network, const char* id)
            /*----------------------------------------------------------------
            **  Input:   id = node ID
            **  Output:  none
        ...
        static int		findlink(EN_Network network, const char* id)
            /*----------------------------------------------------------------
            **  Input:   id = link ID
            **  Output:  none
        ...
        static int		findcurve(EN_Network network, const char* id)
            /*----------------------------------------------------------------
            ...
            */
        {
            ...
            {
                if (strcmp(id, network->Curve[i].ID.c_str()) == 0) return i;
            }
            return 0;
        };
        static int		adddemand(Snode* node, SCALER dbase, int dpat, const char* dname)
            /*----------------------------------------------------------------
            ...
            */
        {
            ...
            // Create a new demand 
            demand = make_cwee_shared<Sdemand>();
            ...
            demand->Pat = dpat;
            demand->Name = nullptr;
            if (dname && strlen(dname) > 0) demand->Name = dname;
            demand->next = nullptr;

            // If node has no demands make this its first demand category
            if (node->D == nullptr) node->D = demand;
            ...
            */
        {
            ...
            {
                curve->X.AssureSize(size);
                curve->Y.AssureSize(size);
                curve->Capacity = size;
            }
            return 0;
        };
        static int		addlinkvertex(Slink* link, SCALER x, SCALER y)
            /*----------------------------------------------------------------
            ...
            */
        {
            ...
            if (!link->Vertices)
            {
                link->Vertices = make_cwee_shared<Svertices>();
                link->Vertices->Npts = 0;
                link->Vertices->Capacity = CHUNKSIZE;
                link->Vertices->X.ClearedResize(link->Vertices->Capacity);
                link->Vertices->Y.ClearedResize(link->Vertices->Capacity);
            }
            auto vertices = link->Vertices;
            ...
            //-----------------------------------------------------------
        {
            ...
            net->Rule[i].Premises = nullptr;
            net->Rule[i].ThenActions = nullptr;
            net->Rule[i].ElseActions = nullptr;
        };
            ...
            */
        {
            ...
            else if (pr->report.RptFile != nullptr && pr->report.Messageflag && errcode > 100)
            {
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                {
                    count++;
                    sprintf(pr->Msg, "Error 234: %s %s", geterrmsg(234, errmsg), net->Node[i].ID.c_str());
                    writeline(pr, pr->Msg);
                }
                if (count >= 10) break;
            }
            ...
            */
        {
            ...
            {
                node = &net->Node[i];
                for (demand = node->D; demand != nullptr; demand = demand->next)
                {
            ...
            */
        {
            ...
            // Convert nodal elevations & initial WQ (WQ source units are converted in QUALITY.C
            ...
            for (j = 1; j <= net->Ntanks; j++)
            {
                ...
                if (i > 0)
                {
                    ...
                    else
                    {
                        ...
                        tank->Vmin = interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), tank->Hmin);
                        tank->Vmax = interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), tank->Hmax);
                        tank->V0 = interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), tank->H0);

                        // Find a "nominal" diameter for tank
                        a = (curve->Y[n] - curve->Y[0]) / (curve->X[n] - curve->X[0]);
                        tank->A = ::epanet::sqrt((SCALER)(4.0 * a / PI));
                    }
                }
        ...
        static int      checktime(EN_Project pr, cweeSharedPtr<Spremise> p)
            //------------------------------------------------------------
        ...
        static int      checkstatus(EN_Project pr, cweeSharedPtr<Spremise> p)
            //------------------------------------------------------------
        ...
        static int      checkvalue(EN_Project pr, cweeSharedPtr<Spremise> p)
            //----------------------------------------------------------
        ...
        static int      onactionlist(EN_Project pr, int i, cweeSharedPtr<Saction> a)
            //-----------------------------------------------------------------------------
            ...
            //-----------------------------------------------------------------------------
        {
            ...
            link = a->link;
            auto actionItem = pr->rules.ActionList;
            while (actionItem != nullptr)
            {
                auto a1 = actionItem->action;
        ...
        static int      checkpremise(EN_Project pr, cweeSharedPtr<Spremise> p)
            //----------------------------------------------------------
        ...
        static void     updateactionlist(EN_Project pr, int i, cweeSharedPtr<Saction> actions)
            //---------------------------------------------------
            ...
            //--------------------------------------------------
        {
            ...
            while (a != nullptr)
            {
                ...
                {
                    AUTO actionItem = make_cwee_shared<SactionList>();
        ...
        static void     writeruleaction(EN_Project pr, int k, const char* ruleID)
            /*
            **--------------------------------------------------------------
            **   Input:   k  = link index
            **            *ruleID  = rule ID
            ...
            */
        {
            ...
            sprintf(pr->Msg, FMT63, clocktime(rpt->Atime, time->Htime),
                LinkTxt[Link[k].Type], Link[k].ID.c_str(), ruleID);
            writeline(pr, pr->Msg);
        };
            ...
            //-----------------------------------------------------------
        {
            ...
            n = 0;
            auto actionItem = rules->ActionList;
            while (actionItem != nullptr)
            {
                flag = FALSE;
                auto a = actionItem->action;
                k = a->link;
                s = hyd->LinkStatus[k];
                v = hyd->LinkSetting[k];
                x = a->setting;
            ...
            //----------------------------------------------------------
        {
            ...
            while (actionItem != nullptr)
            {
                auto nextItem = actionItem->next;
                actionItem = nullptr;
                actionItem = nextItem;
            }
            rules->ActionList = nullptr;
        };
        static int      checkrules(EN_Project pr, long dt)
            //-----------------------------------------------------
            ...
            //-----------------------------------------------------
        {
            ...
            for (i = 1; i <= net->Nrules; i++)
            {
                ...
                {
                    if (net->Rule[i].ElseActions != nullptr)
                    {
            ...
            if (rules->ActionList != nullptr) actionCount = takeactions(pr);
            clearactionlist(rules);
            return actionCount;
        };
        static void     writepremise(cweeSharedPtr<Spremise> p, FILE* f, EN_Network net)
            //-----------------------------------------------------------------------------
        ...
        static void     writeaction(cweeSharedPtr < Saction> a, FILE* f, EN_Network net)
            //-----------------------------------------------------------------------------
            ...
            //-----------------------------------------------------------------------------
        {
            ...
            getobjtxt(r_LINK, subtype, s_obj);
            strcpy(s_id, net->Link[a->link].ID.c_str());
            ...
            //-----------------------------------------------------------------------------
        {
            ...
            AUTO rule = &net->Rule[ruleIndex];

            // Write each premise clause to the file
            auto p = rule->Premises;
            fprintf(f, "\nIF   ");
            while (p != nullptr)
            {
            ...
            */
        {
            ...
            if (s1 == s2)
            {
                ...
                sprintf(pr->Msg, FMT56, LinkTxt[Link[k].Type], Link[k].ID.c_str(), setting);
                writeline(pr, pr->Msg);
                return;
            }
            ...
            if (j1 != j2)
            {
                sprintf(pr->Msg, FMT57, LinkTxt[Link[k].Type], Link[k].ID.c_str(), StatTxt[j1],
                    StatTxt[j2]);
                writeline(pr, pr->Msg);
            }
        };
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (newstat != hyd->OldStatus[net->Nlinks + i])
                {
                    if (Tank[i].A > 0.0)
                    {
                        snprintf(s1, MAXLINE, FMT50, atime, net->Node[n].ID.c_str(), StatTxt[newstat],
                            (hyd->NodeHead[n] - net->Node[n].El) * pr->Ucf[HEAD],
                            rpt->Field[HEAD].Units);
                    }
                    else
                    {
                        snprintf(s1, MAXLINE, FMT51, atime, net->Node[n].ID.c_str(), StatTxt[newstat]);
                    }
                    writeline(pr, s1);
                ...
                if (hyd->LinkStatus[i] != hyd->OldStatus[i])
                {
                    if (time->Htime == 0)
                    {
                        sprintf(s1, FMT52, atime, LinkTxt[(int)net->Link[i].Type],
                            net->Link[i].ID.c_str(), StatTxt[(int)hyd->LinkStatus[i]]);
                    }
                    else sprintf(s1, FMT53, atime, LinkTxt[Link[i].Type], net->Link[i].ID.c_str(),
            ...
            */
        {
            ...
            for (alink = net->Adjlist[i]; alink != nullptr; alink = alink->next)
            {
                ...
                if (marked[j] == 1)
                {
                    sprintf(pr->Msg, WARN03c, net->Link[k].ID.c_str());
                    writeline(pr, pr->Msg);
                    return;
                }
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                if (!marked[i] && hyd->NodeDemand[i] != 0.0)
                {
                    ...
                    {
                        sprintf(pr->Msg, WARN03a, node->ID.c_str(),
            ...
            */
        {
            ...
            for (i = 1; i <= net->Nvalves; i++)
            {
                ...
                if (hyd->LinkStatus[j] >= XFCV)
                {
                    if (rpt->Messageflag)
                    {
                        sprintf(pr->Msg, WARN05, LinkTxt[link->Type], link->ID.c_str(),
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                if (s == XHEAD || s == XFLOW)
                {
                    if (rpt->Messageflag)
                    {
                        sprintf(pr->Msg, WARN04, net->Link[j].ID.c_str(), StatTxt[s],
                            clocktime(rpt->Atime, time->Htime));
                        writeline(pr, pr->Msg);
                    }
                    flag = 4;
            ...
            */
        {
            ...
            case HILEVEL:
                ...
                sprintf(pr->Msg, FMT54, clocktime(rpt->Atime, time->Htime),
                    LinkTxt[Link[k].Type], Link[k].ID.c_str(),
                    NodeTxt[getnodetype(net, n)], Node[n].ID.c_str());
                break;
            ...
            */
        {
            ...
            q *= pr->Ucf[FLOW];
            AUTO curve = &pr->network->Curve[i];
            auto& x = curve->X;                      // x = flow
            auto& y = curve->Y;                      // y = head
            npts = curve->Npts;
            ...
            */
        {
            ...
            for (i = 1; i <= net->Nvalves; i++)
            {
                ...
                case PRV:
                    ...
                    hyd->LinkStatus[k] = next_prv_status(pr, k, status, hset,
                        hyd->NodeHead[n1], hyd->NodeHead[n2]);
                    break;
                case PSV:
                    hset = net->Node[n1].El + hyd->LinkSetting[k];
                    hyd->LinkStatus[k] = next_psv_status(pr, k, status, hset,
                        hyd->NodeHead[n1], hyd->NodeHead[n2]);
                    break;
                default:
                    continue;
                        ...
                        if (hyd->LinkStatus[k] == ACTIVE)
                        {
                            if (rpt->Statflag == FULL)
                            {
                                sprintf(pr->Msg, FMT61,
                                    clocktime(rpt->Atime, time->Htime), link->ID.c_str());
                                writeline(pr, pr->Msg);
                            }
            ...
            */
        {
            ...
            if (qlink >= 1)
            {
                sprintf(pr->Msg, FMT66, qchange, pr->network->Link[qlink].ID.c_str());
                writeline(pr, pr->Msg);
            }
            else if (qnode >= 1)
            {
                sprintf(pr->Msg, FMT67, qchange, pr->network->Node[qnode].ID.c_str());
                writeline(pr, pr->Msg);
            }
            if (hlink >= 1)
            {
                sprintf(pr->Msg, FMT68, herror, pr->network->Link[hlink].ID.c_str());
                writeline(pr, pr->Msg);
            }
        };
        ...
        static int   runhyd(EN_Project pr, long* t)
            /*
            **--------------------------------------------------------------
            **  Input:   none
        ...
        static int  nexthyd(EN_Project pr, long* tstep)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            ...
            */
        {
            ...
            {
                sum = 0.0;
                for (demand = net->Node[i].D; demand != nullptr; demand = demand->next)
                {
        ...
        static long  timestep(EN_Project pr)
            /*
            **----------------------------------------------------------------
            **  Input:   none
        ...
        static int  tanktimestep(EN_Project pr, long* tstep)
            /*
            **-----------------------------------------------------------------
        ...
        static void  controltimestep(EN_Project pr, long* tstep)
            /*
            **------------------------------------------------------------------
            **  Input:   *tstep = current time step
            **  Output:  *tstep = modified current time step
        ...
        static void  ruletimestep(EN_Project pr, long* tstep)
            /*
            **--------------------------------------------------------------
        ...
        static void  addenergy(EN_Project pr, long hstep)
            /*
            **-------------------------------------------------------------
            ...
            */
        {
            ...
            if (link->Type == PUMP)
            {
                ...
                if ((i = net->Pump[j].Ecurve) > 0)
                {
                    ...
                    AUTO curve = &net->Curve[i];
                    e = interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), q4eff);
        ...
        static void  tanklevels(EN_Project pr, long tstep)
            /*
            **----------------------------------------------------------------
            **  Input:   tstep = current time step
            **  Output:  none
            ...
            */
        {
            ...
            {
                AUTO curve = &net->Curve[j];
                y = (h - net->Node[tank->Node].El) * pr->Ucf[HEAD];
                v = interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), y) / pr->Ucf[VOLUME];
                return v;
            }
        };
            ...
            */
        {
            ...
            {
                AUTO curve = &net->Curve[j];
                y = interp(curve->Npts, curve->Y.Ptr(), curve->X.Ptr(), v * pr->Ucf[VOLUME]);
                h = net->Node[tank->Node].El + y / pr->Ucf[HEAD];
                return h;
            }
        };
        static SCALER findsourcequal(EN_Project pr, int n, SCALER volout, long tstep)
            /*
            **---------------------------------------------------------------------
            **   Input:   n = node index
            ...
            */
        {
            ...
            {
                seg = (Sseg*)Mempool_t::mempool_alloc(qual->SegPool, sizeof(Sseg));
                if (seg == nullptr)
                {
            ...
            seg->prev = nullptr;
            if (qual->FirstSeg[k] == nullptr) qual->FirstSeg[k] = seg;
            if (qual->LastSeg[k] != nullptr)  qual->LastSeg[k]->prev = seg;
            qual->LastSeg[k] = seg;
        };
        ...
        static void reactpipes(EN_Project pr, long dt)
            /*
            **--------------------------------------------------------------
            **   Input:   dt = time step
            **   Output:  none
        ...
        static void  reacttanks(EN_Project pr, long dt)
            /*
            **--------------------------------------------------------------
            **   Input:   dt = time step
            **   Output:  none
        ...
        static SCALER pipereact(EN_Project pr, int k, SCALER c, SCALER v, long dt)
            /*
            **------------------------------------------------------------
            **   Input:   k = link index
            ...
            */
        {
            ...
            dcbulk = rbulk * (SCALER)dt;
            dcwall = rwall * (SCALER)dt;
        ...
        static SCALER tankreact(EN_Project pr, SCALER c, SCALER v, SCALER kb, long dt)
            /*
            **-------------------------------------------------------
            ...
            */
        {
            ...
            if (mixzone == nullptr || stagzone == nullptr) return;
            ...
            */
        {
            ...
            k = net->Nlinks + i;
            if (qual->LastSeg[k] == nullptr || qual->FirstSeg[k] == nullptr) return;
            ...
            else if (qual->FirstSeg[k] == nullptr) tank->C = 0.0;
        ...
        static void transport(EN_Project pr, long tstep)
            /*
            **--------------------------------------------------------------
        ...
        static void  evalnodeinflow(EN_Project pr, int k, long tstep, SCALER* volin, SCALER* massin)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            ...
            while (v > 0.0)
            {
                ...
                *massin += vseg * seg->c;
                ...
                if (v >= 0.0 && vseg >= seg->v)
                {
                    ...
                    qual->FirstSeg[k] = seg->prev;
                    if (qual->FirstSeg[k] == nullptr) qual->LastSeg[k] = nullptr;
        ...
        static SCALER  findnodequal(EN_Project pr, int n, SCALER volin, SCALER massin, SCALER volout, long tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   n = node index
            ...
            */
        {
            ...
            if (net->Node[n].Type == JUNCTION)
            {
                ...
                volin -= fMIN(0.0, hyd->NodeDemand[n]) * tstep;
            ...
            */
        {
            ...
            for (alink = net->Adjlist[n]; alink != nullptr; alink = alink->next)
            {
                ...
                if (inflow == TRUE && qual->FirstSeg[k] != nullptr)
                {
                    c += qual->FirstSeg[k]->c;
                    kount++;
                }
                ...
                else if (inflow == FALSE && qual->LastSeg[k] != nullptr)
                {
                    c += qual->LastSeg[k]->c;
                    kount++;
                }
        ...
        static void evalnodeoutflow(EN_Project pr, int k, SCALER c, long tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            ...
            */
        {
            ...
            v = ABS(((hyd->LinkStatus[k] <= CLOSED) ? (SCALER)0.0 : hyd->LinkFlow[k])) * tstep;
            if (v == 0.0) return;
        ...
        static void updatemassbalance(EN_Project pr, int n, SCALER massin, SCALER volout, long tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   n = node index
            ...
            */
        {
            ...
            case JUNCTION:
                masslost = fMAX(0.0, hyd->NodeDemand[n]) * tstep * qual->NodeQual[n];
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                qual->FirstSeg[k] = nullptr;
                qual->LastSeg[k] = nullptr;
            ...
            for (j = 1; j <= net->Ntanks; j++)
            {
                ...
                {
                    // ... mixing zone segment
                    v1 = fMAX(0, v - net->Tank[j].V1frac * net->Tank[j].Vmax);
            ...
            */
        {
            ...
            if (qual->SegPool == nullptr) errcode = 101;
            ...
            qual->FirstSeg.ClearedResize(n);
            qual->LastSeg.ClearedResize(n);
            ...
            */
        {
            ...
            for (i = 1; i <= net->Nnodes; i++)
            {
                ...
                if (net->Node[i].S != nullptr) net->Node[i].S->Smass = 0.0;
            }
            if (qual->Qualflag == NONE) return errcode;
            ...
            if (qual->Qualflag == TRACE_QUAL) qual->NodeQual[qual->TraceNode] = 100.0;
        ...
        static int runqual(EN_Project pr, long* t)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int nextqual(EN_Project pr, long* tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int stepqual(EN_Project pr, long* tleft)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int savehyd(EN_Project pr, long* htime)
            /*
            **--------------------------------------------------------------
        ...
        static int savehydstep(EN_Project pr, long* hydstep)
            /*
            **--------------------------------------------------------------
        ...
        static int readhyd(EN_Project pr, long* hydtime)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int readhydstep(EN_Project pr, long* hydstep)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            ...
            */
        {
            ...
            if (rpt->Tstatflag != SERIES && out->TmpOutFile != nullptr)
            {
                x = (REAL4*)calloc(fMAX(net->Nnodes, net->Nlinks) + 1, sizeof(REAL4));
                if (x == nullptr) return 101;
            ...
            */
        {
            ...
            if (time->Dur > 0) t = (SCALER)time->Dur / 3600.;
            else t = 1.;
            w[0] = (REAL4)(wbulk / t);
            w[1] = (REAL4)(wwall / t);
            w[2] = (REAL4)(wtank / t);
            ...
            */
        {
            ...
            while (fgets(line, MAXLINE, InFile) != nullptr)
            {
                strcpy(s, line);
                tok = strtok(s, SEPSTR);
                ...
                else
                {
                    ...
                    case _TAGS:
                        if (*tok == ';' ||
                            (match("NODE", tok) && findnode(pr->network, strtok(nullptr, SEPSTR))) ||
                            (match("LINK", tok) && findlink(pr->network, strtok(nullptr, SEPSTR))))
                            write = TRUE;
                        break;
                    case _LABELS:
                    case _BACKDROP:
            ...
            */
        {
            ...
            if ((f = fopen(fname, "wt")) == nullptr) return 302;
            ...
            {
                node = &net->Node[i];
                fprintf(f, "\n %-31s %12.4f", node->ID.c_str(), (double)(node->El * pr->Ucf[ELEV]));
                if (node->Comment.Length() > 0) fprintf(f, "  ;%s", node->Comment.c_str());
            }
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (tank->A == 0.0)
                {
                    ...
                    sprintf(s, " %-31s %12.4f", node->ID.c_str(), (double)(node->El * pr->Ucf[ELEV]));
                    if ((j = tank->Pat) > 0) sprintf(s1, " %s", net->Pattern[j].ID.c_str());
                    else strcpy(s1, " ");
                    fprintf(f, "\n%s %-31s", s, s1);
                    if (node->Comment.Length() > 0) fprintf(f, " ;%s", node->Comment.c_str());
                }
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (tank->A > 0.0)
                {
                    ...
                    sprintf(s, " %-31s %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f",
                        node->ID.c_str(), 
                        (double)(node->El * pr->Ucf[ELEV]),
                        (double)((tank->H0 - node->El) * pr->Ucf[ELEV]),
                        (double)((tank->Hmin - node->El) * pr->Ucf[ELEV]),
                        (double)((tank->Hmax - node->El) * pr->Ucf[ELEV]),
                        (double)(::epanet::sqrt(4.0 * tank->A / PI) * pr->Ucf[ELEV]),
                        (double)(tank->Vmin * SQR(pr->Ucf[ELEV]) * pr->Ucf[ELEV]));
                    if ((j = tank->Vcurve) > 0) sprintf(s1, "%s", net->Curve[j].ID.c_str());
            ...
            for (i = 1; i <= net->Nlinks; i++)
            {
                ...
                if (link->Type <= PIPE)
                {
                    ...
                    sprintf(s, " %-31s %-31s %-31s %12.4f %12.4f %12.4f %12.4f",
                        link->ID.c_str(), 
                        net->Node[link->N1].ID.c_str(),
                        net->Node[link->N2].ID.c_str(),
                    ...
                    fprintf(f, "\n%s %-6s", s, s2);
                    if (link->Comment.Length() > 0) fprintf(f, " ;%s", link->Comment.c_str());
                }
            }
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                link = &net->Link[n];
                pump = &net->Pump[i];
                sprintf(s, " %-31s %-31s %-31s", link->ID.c_str(), net->Node[link->N1].ID.c_str(),
                    net->Node[link->N2].ID.c_str());
                ...
                {
                    sprintf(s1, "  HEAD %s", net->Curve[j].ID.c_str());
                }
                ...
                if ((j = pump->Upat) > 0)
                {
                    sprintf(s1, "  PATTERN  %s", net->Pattern[j].ID.c_str());
                    strcat(s, s1);
                }
            ...
            for (i = 1; i <= net->Nvalves; i++)
            {
                ...
                {
                    sprintf(s1, "%-31s %12.4f", net->Curve[j].ID.c_str(), (double)km);
                }
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                for (demand = node->D; demand != nullptr; demand = demand->next)
                {
                    sprintf(s, " %-31s %14.6f", node->ID.c_str(), (double)(ucf * demand->Base));
                    if ((j = demand->Pat) > 0) sprintf(s1, " %-31s", net->Pattern[j].ID.c_str());
                    else strcpy(s1, " ");
                    fprintf(f, "\n%s %-31s", s, s1);
                    if (demand->Name) fprintf(f, " ;%s", demand->Name.c_str());
                }
            }
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                fprintf(f, "\n %-31s %14.6f", node->ID.c_str(), (double)ke);
            }
            ...
            for (i = 1; i <= net->Nlinks; i++)
            {
                ...
                if (link->Type <= PUMP)
                {
                    ...
                    {
                        fprintf(f, "\n %-31s %s", link->ID.c_str(), StatTxt[CLOSED]);
                    }
                    ...
                    else if (link->Type == PUMP)
                    {
                        ...
                        if (pump->Hcurve == 0 && pump->Ptype != CONST_HP &&
                            link->Kc != 1.0)
                        {
                            fprintf(f, "\n %-31s %-.4f", link->ID.c_str(), (double)link->Kc);
                        }
                    }
                }
                ...
                else if (link->Kc == MISSING)
                {
                    if (link->Status == OPEN)
                    {
                        fprintf(f, "\n %-31s %s", link->ID.c_str(), StatTxt[OPEN]);
                    }
                    if (link->Status == CLOSED)
                    {
                        fprintf(f, "\n%-31s %s", link->ID.c_str(), StatTxt[CLOSED]);
            ...
            for (i = 1; i <= net->Npats; i++)
            {
                ...
                {
                    if (j % 6 == 0) fprintf(f, "\n %-31s", net->Pattern[i].ID.c_str());
                    fprintf(f, " %12.4f", (double)net->Pattern[i].F[j]);
                }
            }
            ...
            for (i = 1; i <= net->Ncurves; i++)
            {
                ...
                {
                    curve = &net->Curve[i];
                    fprintf(f, "\n %-31s %12.4f %12.4f", curve->ID.c_str(), (double)curve->X[j], (double)curve->Y[j]);
                }
            }
            ...
            for (i = 1; i <= net->Ncontrols; i++)
            {
                ...
                if (control->Setting == MISSING)
                {
                    sprintf(s, " LINK %s %s ", link->ID.c_str(), StatTxt[control->Status]);
                }
                else
                {
                    kc = control->Setting;
                    ...
                    sprintf(s, " LINK %s %.4f", link->ID.c_str(), (double)kc);
                }
                ...
                case HILEVEL:
                    ...
                    fprintf(f, "\n%s IF NODE %s %s %.4f", s, node->ID.c_str(),
            ...
            for (i = 1; i <= net->Nnodes; i++)
            {
                ...
                fprintf(f, "\n %-31s %14.6f", node->ID.c_str(), (double)(node->C0 * pr->Ucf[QUALITY]));
            }
            ...
            for (i = 1; i <= net->Nnodes; i++)
            {
                ...
                sprintf(s, " %-31s %-8s %14.6f", node->ID.c_str(), SourceTxt[source->Type],
                    (double)source->C0);
                if ((j = source->Pat) > 0)
                {
                    sprintf(s1, "%s", net->Pattern[j].ID.c_str());
                }
                else strcpy(s1, "");
                fprintf(f, "\n%s %s", s, s1);
            }
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                tank = &net->Tank[i];
                if (tank->A == 0.0) continue;
                fprintf(f, "\n %-31s %-8s %12.4f", net->Node[tank->Node].ID.c_str(),
            ...
            for (i = 1; i <= net->Nlinks; i++)
            {
                ...
                {
                    fprintf(f, "\n BULK   %-31s %-.6f", link->ID.c_str(), (double)(link->Kb * SECperDAY));
                }
                if (link->Kw != qual->Kwall)
                {
                    fprintf(f, "\n WALL   %-31s %-.6f", link->ID.c_str(), (double)(link->Kw * SECperDAY));
                }
            }
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                {
                    fprintf(f, "\n TANK   %-31s %-.6f", net->Node[tank->Node].ID.c_str(),
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                if (pump->Ecost > 0.0)
                {
                    fprintf(f, "\n PUMP %-31s PRICE   %-.4f", net->Link[pump->Link].ID.c_str(),
                        (double)pump->Ecost);
                }
                if (pump->Epat > 0.0)
                {
                    fprintf(f, "\n PUMP %-31s PATTERN %s", net->Link[pump->Link].ID.c_str(),
                        net->Pattern[pump->Epat].ID.c_str());
                }
                if (pump->Ecurve > 0.0)
                {
                    fprintf(f, "\n PUMP %-31s EFFIC   %s", net->Link[pump->Link].ID.c_str(),
                        net->Curve[pump->Ecurve].ID.c_str());
                }
            }
            ...
            case QualType::TRACE_QUAL:
                fprintf(f, "\n QUALITY             TRACE %-31s",
                    net->Node[qual->TraceNode].ID.c_str());
                break;
            case QualType::AGE:
                fprintf(f, "\n QUALITY             AGE");
                break;
            ...
            for (i = 0; i < FRICTION; i++)
            {
                ...
                {
                    fprintf(f, "\n %-20sPRECISION %d", field->Name.c_str(), field->Precision);
                    if (field->RptLim[LOW] < BIG)
                    {
                        fprintf(f, "\n %-20sBELOW %.6f", field->Name.c_str(), (double)field->RptLim[LOW]);
                    }
                    if (field->RptLim[HI] > -BIG)
                    {
                        fprintf(f, "\n %-20sABOVE %.6f", field->Name.c_str(), (double)field->RptLim[HI]);
                    }
                }
                else fprintf(f, "\n %-20sNO", field->Name.c_str());
            }
            ...
            for (i = 1; i <= net->Nnodes; i++)
            {
                ...
                fprintf(f, "\n %-31s %14.6f %14.6f", node->ID.c_str(), (double)node->X, (double)node->Y);
            }
            ...
            for (i = 1; i <= net->Nlinks; i++)
            {
                ...
                if (link->Vertices != nullptr)
                {
                    for (j = 0; j < link->Vertices->Npts; j++)
                        fprintf(f, "\n %-31s %14.6f %14.6f",
                            link->ID.c_str(), (double)link->Vertices->X[j], (double)link->Vertices->Y[j]);
                }
            }
