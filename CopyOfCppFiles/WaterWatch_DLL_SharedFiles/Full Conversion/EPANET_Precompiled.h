...
#pragma region Constexpr
    static constexpr SCALER QZERO = 1.e-6;  // Equivalent to zero flow in cfs
    // Constants used for computing Darcy-Weisbach friction factor
    static constexpr SCALER A1 = 3.14159265358979323850e+03;   // 1000*PI
    static constexpr SCALER A2 = 1.57079632679489661930e+03;   // 500*PI
    static constexpr SCALER A3 = 5.02654824574366918160e+01;   // 16*PI
    static constexpr SCALER A4 = 6.28318530717958647700e+00;   // 2*PI
    ...
    static constexpr SCALER AC = -5.14214965799093883760e-03;  // AA*AB
...
#pragma pop_macro("FALSE")   
    static constexpr auto   FULL = 2;
    static constexpr auto   BIG = 1.E10;
    static constexpr auto   TINY = 1.E-6;
    static constexpr auto   MISSING = -1.E-7;     // Missing value indicator // was -1.E-10
    static constexpr auto   DIFFUS = 1.3E-8;     // Diffusivity of chlorine
    static constexpr auto   VISCOS = 1.1E-5;     // Kinematic viscosity of water @ 20 deg C (sq ft/sec)
    static constexpr auto   MINPDIFF = 0.1;        // PDA min. pressure difference (psi or m)
    static constexpr auto   SEPSTR = " \t\n\r";  // Token separator characters
    static constexpr SCALER PI = 3.141592654;
    static constexpr auto   GPMperCFS = 448.831;
    static constexpr auto   AFDperCFS = 1.9837;
    static constexpr auto   MGDperCFS = 0.64632;
    static constexpr auto   IMGDperCFS = 0.5382;
    static constexpr auto   LPSperCFS = 28.317;
    static constexpr auto   LPMperCFS = 1699.0;
    static constexpr auto   CMHperCFS = 101.94;
    static constexpr auto   CMDperCFS = 2446.6;
    static constexpr auto   MLDperCFS = 2.4466;
    static constexpr auto   M3perFT3 = 0.028317;
    static constexpr auto   LperFT3 = 28.317;
    static constexpr auto   MperFT = 0.3048;
    static constexpr auto   PSIperFT = 0.4333;
    static constexpr auto   KPAperPSI = 6.895;
    static constexpr auto   KWperHP = 0.7457;
    static constexpr auto   SECperDAY = 86400;
    ...
    static constexpr SCALER HTOL = 0.0005;   // Default hydraulic head tolerance (ft)
    static constexpr SCALER QTOL = 0.0001;   // Default flow rate tolerance (cfs)
    ...
    static constexpr SCALER Q_STAGNANT = 0.005 / GPMperCFS;     // 0.005 gpm = 1.114e-5 cfs
#pragma endregion
#pragma region Enums
    /// Size Limts
     /**
    ...
    */
    ...
    public:
        ...
        cweeList<SCALER> F;               // pattern factors
    };
    ...
    public:
        ...
        cweeList<SCALER> X;            // x-values
        cweeList<SCALER> Y;            // y-values
    };

    class Sdemand             // Demand List Item
    {
    public:
        SCALER Base;             // baseline demand
        int    Pat;              // pattern index
        cweeStr Name;            // demand category name
    ...
    {
    public:
        SCALER TimeOnLine;       // hours pump is online
        SCALER Efficiency;       // total time wtd. efficiency
        SCALER KwHrsPerFlow;     // total kw-hrs per unit of flow
        SCALER KwHrs;            // total kw-hrs consumed
        SCALER MaxKwatts;        // max. kw consumed
        SCALER TotalCost;        // total pumping cost
        SCALER CurrentPower;     // current pump power (kw)
    ...
    public:
        ...
        cweeStr Comment;       // node comment
    };

    class Slink             // Link Object
    {
    public:
        cweeStr  ID;             // link ID
        int      N1;             // start node index
        int      N2;             // end node index
        SCALER   Diam;           // diameter
        SCALER   Len;            // length
        SCALER   Kc;             // roughness
        ...
        cweeSharedPtr<Svertices>  Vertices;     // internal vertex coordinates
        int      Rpt;            // reporting flag
        int      ResultIndex;    // saved result index
        cweeStr Comment;       // link comment
    };

    class Stank             // Tank Object
    {
    public:
        int     Node;            // node index of tank
        SCALER  A;               // tank area
        SCALER  Hmin;            // minimum water elev
        SCALER  Hmax;            // maximum water elev
        SCALER  H0;              // initial water elev
        SCALER  Vmin;            // minimum volume
        SCALER  Vmax;            // maximum volume
        SCALER  V0;              // initial volume
        SCALER  Kb;              // bulk reaction coeff.
        SCALER  V;               // tank volume
    ...
    class Spump             // Pump Object
    {
    public:
        int     Link;            // link index of pump
        int     Ptype;           // pump curve type
        SCALER  Q0;              // initial flow
        SCALER  Qmax;            // maximum flow
        SCALER  Hmax;            // maximum head
        ...
        SCALER  Ecost;           // unit energy cost
        Senergy Energy;          // energy usage statistics
    };

    class Svalve             // Valve Object
    {
    public:
    ...
    class SField             // Field Object of Report Table
    {
    public:
        cweeStr  Name;  // name of reported variable
    ...
    class Sseg               // Pipe Segment List Item
    {
    public:
        SCALER  v;             // segment volume
        SCALER  c;             // segment water quality
        cweeSharedPtr<Sseg> prev;    // previous segment in list
    };
    using Pseg = cweeSharedPtr<Sseg>; // Pointer to pipe segment list

    class Spremise       // Rule Premise Clause
    {
    public:
        ...
        cweeSharedPtr< Spremise> next;  // next premise clause
        
    };

    class Saction        // Rule Action Clause
    {
    public:
        int     link;              // link index
        int     status;            // link's status
        SCALER  setting;           // link's setting
        cweeSharedPtr< Saction> next;
    } ;
    ...
    public:
        ...
        cweeSharedPtr< Spremise> Premises;        // list of premises
        cweeSharedPtr< Saction> ThenActions;     // list of THEN actions
        cweeSharedPtr< Saction> ElseActions;     // list of ELSE actions
    } ;

    class SactionList    // Action List Item
    {
    public:
        int     ruleIndex;           // index of rule action belongs to
        cweeSharedPtr< Saction> action;             // an action clause
        //cweeSharedPtr< SactionList> next;  // next action on the list
        cweeSharedPtr< SactionList> next;  // next action on the list
    } ;
    ...
    */
    ...
    public:
        ...
        cweeList<SCALER> X;               // Temporary array for curve data
    } ;
    ...
    class Rules {
    public:
        cweeSharedPtr<SactionList> ActionList;     // Linked list of action items
        ...
        cweeSharedPtr<Spremise> LastPremise;    // Previous premise clause
        cweeSharedPtr<Saction> LastThenAction; // Previous THEN action
        cweeSharedPtr<Saction> LastElseAction; // Previous ELSE action
    ...
    class Smatrix {
    public:
        cweeList<SCALER> // cfs_p_ft_t
            Aii;        // Diagonal matrix coeffs.
        cweeList<SCALER> // cfs_p_ft_t
            Aij;        // Non-zero, off-diagonal matrix coeffs.
        cweeList<SCALER> // units::flowrate::cubic_foot_per_second_t
            F;          // Right hand side vector
        cweeList<SCALER> // units::length::foot_t
            B_ft;       // Right hand side vector result
        cweeList<SCALER> // squared_cfs_p_ft_t
    ...
    public:
        ...
        Mempool * SegPool;   // Memory pool for water quality segments
    ...
    public:
        char* key;
        int    data;
        cweeSharedPtr<DataEntry> next;
    };

    using HashTable = cweeSharedPtr<DataEntry>;
    ...
    public:
        ...
        cweeList < Snode> Node;          // Node array
        cweeList < Slink> Link;          // Link array
        cweeList < Stank> Tank;          // Tank array
        cweeList < Spump> Pump;          // Pump array
        cweeList < Svalve> Valve;         // Valve array
        cweeList<Spattern> Pattern;       // Time pattern array
        cweeList < Scurve> Curve;         // Data curve array
    ...
    using EN_Network = Network*; //  cweeSharedPtr<Network>;

    // Overall Project Wrapper
    class Project {
    public:
        Project() : network(new Network()) {};
        ~Project() { if (network) delete network; };

        EN_Network network; //  make_cwee_shared<Network>();                 // Pipe network wrapper
    ...
    using EN_Project = Project*; //  cweeSharedPtr<Project>;
...
#pragma region Enum Text Collections
    ...
    static const char* Fldname[] = { t_ELEV.c_str(),       t_DEMAND.c_str(),     t_HEAD.c_str(),
                               ...
                               (const char*)"",  (const char*)"",  (const char*)"",  (const char*)"",  (const char*)"",  (const char*)"", nullptr };
    static const char* Ruleword[] = { w_RULE.c_str(), w_IF.c_str(), w_AND.c_str(), w_OR.c_str(), w_THEN.c_str(), w_ELSE.c_str(), w_PRIORITY.c_str(), nullptr };
    static const char* Varword[] = { w_DEMAND.c_str(), w_HEAD.c_str(), w_GRADE.c_str(), w_LEVEL.c_str(), w_PRESSURE.c_str(),
        w_FLOW.c_str(), w_STATUS.c_str(),   w_SETTING.c_str(),   w_POWER.c_str(), w_TIME.c_str(), w_CLOCKTIME.c_str(), w_FILLTIME.c_str(), w_DRAINTIME.c_str(), nullptr };
    static const char* Object[] = { w_JUNC.c_str(),  w_RESERV.c_str(), w_TANK.c_str(), w_PIPE.c_str(), w_PUMP.c_str(), w_VALVE.c_str(), w_NODE.c_str(), w_LINK.c_str(), w_SYSTEM.c_str(), nullptr };
    static const char* Operator[] = { (const char*)"=",  (const char*)"<>",  (const char*)"<=",    (const char*)">=",    (const char*)"<", (const char*)">", w_IS.c_str(), w_NOT.c_str(), w_BELOW.c_str(), w_ABOVE.c_str(), nullptr };
    static const char* Value[] = { (const char*)"XXXX", w_OPEN.c_str(), w_CLOSED.c_str(), w_ACTIVE.c_str(), nullptr };
#pragma endregion
#pragma region DEFINES
#define ERRCODE(x) (errcode = ((errcode>100) ? (errcode) : (x)))
#pragma endregion
#pragma region inline templated functions
    template<typename T> INLINE AUTO MEMCHECK(T x) { return x == nullptr ? 101 : 0; }; // #define  MEMCHECK(x)  (((x) == nullptr) ? 101 : 0 )
    template<typename T> INLINE void FREE(T x) { ::free(x); x = nullptr; }; //  #define  FREE(x) do { free(x); (x) = nullptr; } while(0)
...
#pragma region MemPool
    ...
    public:
        ...
        static Mempool* mempool_create() {
            ...
            if (mempool->first == nullptr) return nullptr;
            return mempool;
        };
        static void mempool_delete(Mempool* mempool) {
        ...
        static void mempool_reset(Mempool* mempool) {
            mempool->current = mempool->first;
            mempool->current->free = mempool->current->block;
        };
        static char* mempool_alloc(Mempool* mempool, size_t size) {
            char* ptr;
            ...
            */
            ...
            if (!mempool->current) return nullptr;
            ...
            if (mempool->current->free >= mempool->current->end)
            {
                ...
                else
                {
                    ...
                    if (!mempool->current->next) return nullptr;
                    mempool->current = mempool->current->next;
                }
...
#pragma region Hash
    ...
    public:
        ...
        // Create a hash table
        static cweeList<HashTable> hashtable_create()
        {
            int i;
            cweeList<HashTable> ht; 
            ht.AssureSize(HASHTABLEMAXSIZE);
            for (auto& x : ht) x = nullptr;          
            return ht;
        };

        // Insert an entry into the hash table
        static int hashtable_insert(cweeList<HashTable>& ht, const char* key, int data)
        {
            unsigned int i = gethash(key);
            if (i >= HASHTABLEMAXSIZE) return 0;
            auto entry = make_cwee_shared< DataEntry>();
        ...
        static int hashtable_update(cweeList<HashTable>& ht, const char* key, int new_data)
        {
            unsigned int i = gethash(key);
        ...
        static int hashtable_delete(cweeList<HashTable>& ht, const char* key)
        {
            unsigned int i = gethash(key);
            HashTable entry, preventry;
            ...
            while (entry != nullptr)
            {
                if (strcmp(entry->key, key) == 0)
                {
                    if (preventry == nullptr) ht[i] = entry->next;
        ...
        static int hashtable_find(cweeList<HashTable>& ht, const char* key)
        {
            unsigned int i = gethash(key);
            HashTable entry;
        ...
        static char* hashtable_findkey(cweeList<HashTable>& ht, const char* key)
        {
            unsigned int i = gethash(key);
            HashTable entry;
            if (i >= HASHTABLEMAXSIZE) return nullptr;
            entry = ht[i];
            while (entry != nullptr)
            {
        ...
        static void hashtable_free(cweeList<HashTable>& ht)
        {
            HashTable entry, nextentry;
            int i;
...
#pragma region sparse matrix
    ...
    public:
            ...
            */
        {
            ...
            sm->Order.Clear(); sm->Order.AssureSize(Nnodes + 1);
            sm->Row.Clear(); sm->Row.AssureSize(Nnodes + 1);
            sm->Ndx.Clear(); sm->Ndx.AssureSize(Nlinks + 1);
            ...
            */
        {
            ...
            sm->Aij.Clear(); sm->Aij.AssureSize(sm->Ncoeffs + 1);
            sm->Aii.Clear(); sm->Aii.AssureSize(n);
            sm->F.Clear(); sm->F.AssureSize(n);
            sm->temp.Clear(); sm->temp.AssureSize(n);
            sm->link.Clear(); sm->link.AssureSize(n);
            sm->first.Clear(); sm->first.AssureSize(n);
    ...
    // Scan adjacency list of each node
            ...
            */
        {
            ...
            sm->XLNZ.Clear(); sm->XLNZ.AssureSize(n + 2);
            sm->NZSUB.Clear(); sm->NZSUB.AssureSize(sm->Ncoeffs + 2);
            sm->LNZ.Clear(); sm->LNZ.AssureSize(sm->Ncoeffs + 2);
...
#undef DAT
    ...
    //    //           s1 points to a valid memory location or is nullptr. E.g.,
    ...
    */
    ...
    static int      findmatch(const char* line, const char* keyword[])
        ...
        **           *keyword[] = list of nullptr terminated keywords
         ...
         */
    {
        ...
        for (n = 0; n < maxToks; n++) Tok[n] = nullptr;
        n = 0;
    ...
    static char*    clocktime(char* atime, long seconds)
        /*
        **--------------------------------------------------------------
    ...
    static void		adjustpattern(int* pat, int index)
        /*----------------------------------------------------------------
