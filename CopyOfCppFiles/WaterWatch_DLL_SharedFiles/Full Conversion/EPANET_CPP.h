...
namespace epanet {
    class epanet {public:
        ...
        static  int EN_runproject(EN_Project p, const char* inpFile, const char* rptFile, const char* outFile, void (*pviewprog)(char*))
            ...
            **  needed then the argument should be nullptr.
            **-------------------------------------------------------------------------
            */
        {
            int errcode = 0;
                ...
                */
        {
            ...
            // Initialize data arrays to nullptr
            epanet_postcompiled::initpointers(p);
            ...
            // Close input file
            if (p->parser.InFile != nullptr)
            {
                fclose(p->parser.InFile);
                p->parser.InFile = nullptr;
            }
                ...
                */
        {
            ...
            // Close report file
            if (p->report.RptFile != nullptr && p->report.RptFile != stdout)
            {
                fclose(p->report.RptFile);
                p->report.RptFile = nullptr;
            }

            // Close hydraulics file
            if (p->outfile.HydFile != nullptr)
            {
                fclose(p->outfile.HydFile);
                p->outfile.HydFile = nullptr;
            }
        ...
        static  int EN_runH(EN_Project p, long* currentTime)
            /*----------------------------------------------------------------
            **  Input:   none
        ...
        static  int EN_nextH(EN_Project p, long* tStep)
            /*----------------------------------------------------------------
            **  Input:   none
            ...
            */
        {
            ...
            if (p->outfile.HydFile == nullptr || !p->outfile.SaveHflag) return 104;

            // Open the permanent hydraulics file
            if ((f = fopen(filename, "w+b")) == nullptr) return 305;
        ...
        static  int EN_runQ(EN_Project p, long* currentTime)
            /*----------------------------------------------------------------
            **  Input:   none
        ...
        static  int EN_nextQ(EN_Project p, long* tStep)
            /*----------------------------------------------------------------
            **  Input:   none
        ...
        static  int EN_stepQ(EN_Project p, long* timeLeft)
            /*----------------------------------------------------------------
            **  Input:   none
        ...
        static  int EN_gettimeparam(EN_Project p, int param, long* value)
            /*----------------------------------------------------------------
        ...
        static  int EN_settimeparam(EN_Project p, int param, long value)
            /*----------------------------------------------------------------
            **  Input:   param = time parameter code (see EN_TimeParameter)
            ...
            */
        {
            ...
            *qualType = p->quality.Qualflag;
            if (p->quality.Qualflag == TRACE_QUAL) *traceNode = p->quality.TraceNode;
            return 0;
        };
            ...
            */
        {
            ...
            {
                qual->TraceNode = epanet_shared::findnode(net, traceNode);
            ...
            */
        {
            ...
            *index = 0;
            ...
            net->Node.AssureSize(net->Nnodes + 2);
            ...
            if (nodeType == EN_JUNCTION)
            {
                ...
                node = &net->Node[nIdx];
                node->D = nullptr;
                epanet_shared::adddemand(node, 0.0, 0, nullptr);
            ...
            else
            {
                ...
                // resize tanks array
                net->Tank.AssureSize(net->Ntanks + 1);
            ...
            */
        {
            ...
            strcpy(id, p->network->Node[index].ID.c_str());
            return 0;
        };
            ...
            */
        {
            ...
            *value = 0.0;
            ...
            case EN_TANKDIAM:
                ...
                if (index > nJuncs)
                {
                    v = ::epanet::sqrt(4.0 / cweeMath::PI * Tank[index - nJuncs].A) * Ucf[ELEV];
                }
                break;
            ...
            */
        {
            ...
            case EN_TANKLEVEL:
                ...
                else
                {
                    ...
                    Tank[j].H0 = value;
                    Tank[j].V0 = epanet_shared::tankvolume(p, j, Tank[j].H0);
            ...
            case EN_TANKDIAM:
                ...
                Tank[j].V0 = epanet_shared::tankvolume(p, j, Tank[j].H0);     // new init. volume
                Tank[j].Vmax = epanet_shared::tankvolume(p, j, Tank[j].Hmax); // new max. volume
                break;
            ...
            case EN_VOLCURVE:
                ...
                Tank[j].Vmin = epanet_shared::tankvolume(p, j, Tank[j].Hmin); // new min. volume
                Tank[j].V0 = epanet_shared::tankvolume(p, j, Tank[j].H0);     // new init. volume
                Tank[j].Vmax = epanet_shared::tankvolume(p, j, Tank[j].Hmax); // new max. volume
            ...
            case EN_MINLEVEL:
                ...
                if (i > 0)                                 // tank has a volume curve
                {
                    ...
                    Tank[j].Vmin = epanet_shared::tankvolume(p, j, hTmp); // new min. volume
                }
                Tank[j].Hmin = hTmp;
            ...
            case EN_MAXLEVEL:
                ...
                Tank[j].Vmax = epanet_shared::tankvolume(p, j, hTmp);     // new max. volume
                break;
            ...
            */
        {
            ...
            else if (!epanet_shared::adddemand(node, dmnd, patIndex, nullptr)) return 101;
            ...
            */
        {
            ...
            if (Tank[j].A == (decltype(::epanet::Stank::A))0) return 0;  // Tank is a Reservoir
            ...
            if (strlen(volcurve) > 0)
            {
                ...
                AUTO curve = &net->Curve[curveIndex];
                n = curve->Npts - 1;
            ...
            else Tank[j].Vmin = epanet_shared::tankvolume(p, j, Tank[j].Hmin);
            Tank[j].V0 = epanet_shared::tankvolume(p, j, Tank[j].H0);
            Tank[j].Vmax = epanet_shared::tankvolume(p, j, Tank[j].Hmax);
            return 0;
        };
        ...
        static  int EN_adddemand(EN_Project p, int nodeIndex, SCALER baseDemand, char* demandPattern, char* demandName)
            ...
            **           demandPattern = name of demand's time pattern (can be nullptr or empty)
            **           demandName = name of demand's category (can be nullptr or empty)
            **  Returns: error code
            ...
            */
        {
            ...
            *demandIndex = 0;
            if (!p->Openflag) return 102;
            if (nodeIndex <= 0 || nodeIndex > p->network->Nnodes) return 203;
            if (demandName == nullptr) return 253;
            ...
            */
        {
            ...
            if (d->Name) strcpy(demandName, d->Name.c_str());
            return 0;
        };
            ...
            */
        {
            ...
            *index = 0;
            ...
            else if (linkType == PUMP)
            {
                ...
                net->Npumps++;
                net->Pump.AssureSize(net->Npumps + 1);
            ...
            else
            {
                ...
                net->Valve.AssureSize(net->Nvalves + 1);
            ...
            */
        {
            ...
            strcpy(id, p->network->Link[index].ID.c_str());
            return 0;
        };
            ...
            */
        {
            ...
            *count = 0;
            ...
            auto& vertices = Link[index].Vertices;
            ...
            */
        {
            ...
            *value = (SCALER)p->network->Pattern[index].F[period - 1];
            return 0;
        };
            ...
            */
        {
            ...
            *value = 0.0;
            ...
            {
                *value += (SCALER)Pattern[index].F[i];
            }
            *value /= (SCALER)Pattern[index].Length;
            return 0;
        };
            ...
            */
        {
            ...
            strcpy(id, p->network->Curve[index].ID.c_str());
            return 0;
        };
            ...
            */
        {
            ...
            if (xValues == nullptr || yValues == nullptr) return 206;
            ...
            */
        {
            ...
            *priority = (SCALER)p->network->Rule[index].priority;
            ...
            {
                count = 0;
                auto premise = net->Rule[index].Premises;
                while (premise != nullptr)
                {
                    count++;
                ...
                *nPremises = count;
                ...
                count = 0;
                auto action = net->Rule[index].ThenActions;
                ...
                *nThenActions = count;
                ...
                count = 0;
                auto action = net->Rule[index].ElseActions;
                while (action != nullptr)
                {
                    count++;
                    action = action->next;
            ...
            */
        {
            ...
            auto premises = p->network->Rule[ruleIndex].Premises;
            auto premise = epanet_postcompiled::getpremise(premises, premiseIndex);
            if (premise == nullptr)  return 258;
            ...
            */
        {
            ...
            auto actions = p->network->Rule[ruleIndex].ThenActions;
            AUTO action = epanet_postcompiled::getaction(actions, actionIndex);
            if (action == nullptr) return 258;
            ...
            */
        {
            ...
            auto actions = p->network->Rule[ruleIndex].ElseActions;
            AUTO action = epanet_postcompiled::getaction(actions, actionIndex);
            if (action == nullptr) return 258;
