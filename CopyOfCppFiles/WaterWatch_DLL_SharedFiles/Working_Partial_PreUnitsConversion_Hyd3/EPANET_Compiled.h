...
#pragma region Shared Functions
            ...
            */
        {
            ...
            if (pump->Ptype == CONST_HP)  // Constant Hp pump
            {
                ...
                pump->Q0 = 1.0_cfs;   // Init. flow = 1 cfs
                return errcode;
            }
            ...
            else if (pump->Ptype == NOCURVE) // Pump curve specified
            {
                ...
                else
                {
                    ...
                    pump->Qmax = pr->convertToUnit< cubic_foot_per_second_t>(curve->X[npts - 1]);
                    pump->Q0 = pr->convertToUnit< cubic_foot_per_second_t>((curve->X[0] + curve->X[npts - 1]) / 2.0);
                    pump->Hmax = pr->convertToUnit < foot_t>(curve->Y[0]);
                }
                ...
                if (pump->Ptype == POWER_FUNC)
                {
                    ...
                    else
                    {
                        ...
                        pump->R = -b;
                        pump->N = c;
                        pump->Q0 = pr->convertToUnit< cubic_foot_per_second_t>(q1);
        ...
        static int		adddemand(Pnode node, cubic_foot_per_second_t dbase, int dpat, char* dname, Ppattern TimePat)
            /*----------------------------------------------------------------
        ...
        static int		resizecurve(Pcurve curve, int size)
            /*----------------------------------------------------------------
            ...
            for (j = 1; j <= net->Ntanks; j++)
            {
                ...
                if (i > 0)
                {
                    ...
                    else
                    {
                        ...
                        tank->Vmin = pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), (double)(tank->Hmin * pr->Ucf[ELEV])));
                        tank->Vmax = pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), (double)(tank->Hmax * pr->Ucf[ELEV])));
                        tank->V0 = pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), (double)(tank->H0 * pr->Ucf[ELEV])));
            ...
            if (hyd->LinkStatus[k] <= CLOSED || setting == 0.0)
            {
                ...
                if (pump->Ptype == NOCURVE)
                {
                    ...
                    if (pump->Ptype == CUSTOM)
                    {
                        ...
                        hloss = (SCALER)(double)pump->H0 * SQR(setting) + hgrad * (double)hyd->LinkFlow[k];
                    }
                    else
                    {
            ...
            */
        {
            ...
            *r = (curve->Y[k2] - curve->Y[k1]) / (curve->X[k2] - curve->X[k1]);
            *h0 = curve->Y[k1] - (*r) * curve->X[k1];
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                *dqsum += ABS(dq);
                ...
                if (ABS(dq) > (SCALER)(double)hbal->maxflowchange)
                {
                    hbal->maxflowchange = (double)ABS(dq);
                    hbal->maxflowlink = k;
                    hbal->maxflownode = -1;
                }
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                if (headerror > (double)hbal->maxheaderror)
                {
                    hbal->maxheaderror = (double)headerror;
                    hbal->maxheadlink = k;
                }
            }
        };
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                for (demand = net->Node[i]->D; demand != nullptr; demand = demand->next)
                {
                    ...
                    if (demand->TimePat) {
                        djunc = demand->Base * demand->TimePat->Pat((u64)time->GetCurrentRealHtime()) * hyd->Dmult;
        ...
        static void  getenergy(EN_Project pr, int k, kilowatt_t* kw, SCALER* eff)
            /*
            **----------------------------------------------------------------
            **  Input:   k    = link index
            ...
            */
        {
            ...
            else
            {
                ...
                v = (double)pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X.Ptr(), curve->Y.Ptr(), y));
                return v;
            }
        };
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                else
                {
                    fprintf(f, "\n%s %12.4f %12.4f %12.4f          0.0 %12.4f", s,
                        ...
                        (double)(((SCALER)(double)(-pump->H0) - pump->R * ::epanet::pow((SCALER)(double)pump->Q0, pump->N)) * pr->Ucf[HEAD]),
