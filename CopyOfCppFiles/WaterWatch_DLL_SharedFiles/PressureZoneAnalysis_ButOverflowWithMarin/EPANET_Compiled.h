...
#pragma region Shared Functions
        ...
        static void     setlinksetting(EN_Project pr, int index, SCALER value, SCALER* s, SCALER* k)
            /*----------------------------------------------------------------
            **  Input:   index  = link index
        ...
        static void     setlinkstatus(EN_Project pr, int index, char value, SCALER* s, SCALER* k)
            /*----------------------------------------------------------------
            **  Input:   index  = link index
        ...
        static void     pipecoeff(EN_Project pr, int k, cweeList<Plink> const& link)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            **   Output:  none
            ...
            for (auto i = 1; i <= numN; i++) {
                ...
                if (obj) {
                    ...
                    if (A_t == asset_t::VALVE) {
                        {
                            if (AUTO valve = net->Valve[::epanet::epanet_shared::findvalve(net, i)]) {
                                AUTO pat = obj->GetValue<_ENERGY_>();
                ...
                for (auto& obj : net->Zone) {
                    if (obj) {
                        ...
                        for (auto& link : obj->Boundary_Link) {
                            switch (link.get<1>()) {
                            case Szone::direction_t::FLOW_WITHIN_DMA: break;
                            case Szone::direction_t::FLOW_IN_DMA:
                                demand += link.get<0>()->GetCurrentValue<_FLOW_>(currentTime);
                                break;
                            case Szone::direction_t::FLOW_OUT_DMA:
                                demand -= link.get<0>()->GetCurrentValue<_FLOW_>(currentTime);
                                break;
                            }
                        }
        ...
        static void     SaveResultsForQualityStep(EN_Project pr) {
            EN_Network net = pr->network;
            Hydraul* hyd = &pr->hydraul;
            Smatrix* sm = &hyd->smatrix;
            Report* rpt = &pr->report;
