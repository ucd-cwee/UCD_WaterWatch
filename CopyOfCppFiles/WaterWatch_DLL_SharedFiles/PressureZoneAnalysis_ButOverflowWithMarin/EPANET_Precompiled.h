...
#undef ReduceMemoryFor
    ...
    public:
        ...
        enum class illDefined_t {
            Okay, 
            TankWithoutInflow, // Open pressure zone has no available water source -> OpenZone && IllDefined
            IsolatedJunction, // No pump or valve points to the node, and all the connected pipes (if any) are closed. -> Represented by (junction OR pipe) && IllDefined
            InactivePump, // Pump points towards the nodes but is never ON -> Represented by Pump && IllDefined
            InactiveValve // Valve points towards the nodes but is never ON -> Represented by Valve && IllDefined
        };
        enum class zoneType_t {
            Open, // connected directly to one or more storage tanks
            Draw, // connected directly to one or more infinite reservoirs
            Closed, // served by a head-increasing pump
            Reduced // served by a head-reducing valve or pipe            
        };
        ...
        cweeThreadedList<cweeUnion<Plink, direction_t>> Boundary_Link; // note that this only inludes the 'critical' links that determine inflow or outflow or can be controlled. 
        cweeThreadedList < Plink > Within_Link; // includes only the completely contained links

        foot_t AverageElevation() {
            foot_t El = 0; int n = 0;
            for (auto& node : Node) {
                if (node) {
