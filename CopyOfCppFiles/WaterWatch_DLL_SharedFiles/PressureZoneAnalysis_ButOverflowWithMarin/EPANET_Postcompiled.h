...
namespace epanet {
    class epanet_postcompiled { public:
            ...
            */
        {
            ...
            pr->hydraul.TankVolume.Clear();
            pr->quality.TankConcentration.Clear();
            ...
            //--------------------------------------------------------------
        {
            ...
            net->Rule.ClearedResize(n); for (auto& x : net->Rule) if (!x) x = make_cwee_shared<Srule>(); net->Rule[0] = nullptr;
            ...
            */
        {
            ...
            if (!errcode)
            {
                ...
                pr->network->Node.ClearedResize(n); for (auto& x : pr->network->Node) { if (!x) { x = make_cwee_shared<Snode>(); } } net->Node[0] = nullptr;
                ...
                pr->quality.TankConcentration.ClearedResize(pr->parser.MaxTanks + 1);
                pr->hydraul.TankVolume.ClearedResize(pr->parser.MaxTanks + 1);
            }
            ...
            if (!errcode)
            {
                ...
                pr->network->Link.ClearedResize(n); for (auto& x : pr->network->Link) { if (!x) x = make_cwee_shared<Slink>(); } net->Link[0] = nullptr;
            ...
            if (!errcode)
            {
                pr->network->Tank.ClearedResize(pr->parser.MaxTanks + 1); for (auto& x : pr->network->Tank) if (!x) x = make_cwee_shared<Stank>(); net->Tank[0] = nullptr;
                pr->network->Pump.ClearedResize(pr->parser.MaxPumps + 1); for (auto& x : pr->network->Pump) if (!x) x = make_cwee_shared<Spump>(); net->Pump[0] = nullptr;
                pr->network->Valve.ClearedResize(pr->parser.MaxValves + 1); for (auto& x : pr->network->Valve) if (!x) x = make_cwee_shared<Svalve>(); net->Valve[0] = nullptr;
                pr->network->Control.ClearedResize(pr->parser.MaxControls + 1); for (auto& x : pr->network->Control) if (!x) x = make_cwee_shared<Scontrol>(); net->Control[0] = nullptr;
            }

            // Initialize pointers used in nodes and links
            if (!errcode)
            {
                for (n = 1; n <= pr->parser.MaxNodes; n++)
                {
                    pr->network->Node[n]->D.Clear();    // node demand
                    pr->network->Node[n]->S = nullptr;    // node source
                    pr->network->Node[n]->Comment = NULL;
                }
                for (n = 1; n <= pr->parser.MaxLinks; n++)
                {
            ...
            */
        {
            ...
            pr->network->Pattern.Clear(); // patterns will self-clean if their counters go to 0.

            // Free memory for curves
            pr->network->Curve.Clear(); // Curves will self-clean if the counters go to 0.
            ...
            */
        {
            ...
            if (!errcode)
            {
                ...
                for (i = 1; i <= net->Ntanks; i++) x[i] = (REAL4)net->Tank[i]->Area();
            ...
            */
        {
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                else
                {
                    ...
                    t = (double)pump->Energy.TimeOnLine;  //currently holds total hrs. online
                    pump->Energy.TimeOnLine = (units::time::hour_t)(double)(t / hdur);
        ...
        static Sdemand*	finddemand(Pnode n, int index)
            /*----------------------------------------------------------------
            ...
            */
        {
            if ((index - 1) >= n->D.Num()) return nullptr;
            return &n->D[index - 1];
        };
            ...
            */
        {
            ...
            for (j = 1; j <= network->Nnodes; j++)
            {
                ...
                for (auto& demand : network->Node[j]->D) {
                    adjustpattern(&demand.Pat, index, &demand.TimePat);
                }
                // Adjust WQ source patterns
                source = network->Node[j]->S;
                if (source) adjustpattern(&source->Pat, index, &source->TimePat);
            }

            // Adjust patterns used by reservoir tanks
            for (j = 1; j <= network->Ntanks; j++) {
                adjustpattern(&network->Tank[j]->Pat, index, &network->Tank[j]->TimePat);
            }

            // Adjust patterns used by pumps
            for (j = 1; j <= network->Npumps; j++) {
                adjustpattern(&network->Pump[j]->Upat, index, &network->Pump[j]->TimeUpat);
                adjustpattern(&network->Pump[j]->Epat, index, &network->Pump[j]->TimeEpat);
            }
        };
            ...
            */
        {
            ...
            for (j = 1; j <= network->Ntanks; j++) {
                adjustcurve(&network->Tank[j]->Vcurve, index, &network->Tank[j]->Vcurve_Actual);
            }
            ...
            //----------------------------------------------------------------
        {
            ...
            case TIMEPAT:
                ...
                currentcomment = (char*)network->Pattern[index]->Comment.c_str();
                break;
            case CURVE:
                if (index < 1 || index > network->Ncurves) return 251;
                currentcomment = (char*)network->Curve[index]->Comment.c_str();
                break;
            default:
                strcpy(comment, "");
                return 251;
            ...
            //----------------------------------------------------------------
        {
            ...
            case TIMEPAT:
                if (index < 1 || index > network->Npats) return 251;
                network->Pattern[index]->Comment = newcomment;
                return 0;

            case CURVE:
                if (index < 1 || index > network->Ncurves) return 251;
                network->Curve[index]->Comment = newcomment;
                return 0;
            ...
            */
        {
            ...
            if (n > 0)
            {
                if (strcmp(id, network->Pattern[n]->ID.c_str()) == 0) return 0;
            ...
            n = n + 2;
            network->Pattern.AssureSize(n); for (auto& x : network->Pattern) if (!x) x = make_cwee_shared<Spattern>(); // network->Pattern[0] = nullptr;
            (network->Npats)++;

            // Initialize the pattern
            pattern = network->Pattern[network->Npats];
            pattern->ID = id;
            pattern->Comment.Clear();
            pattern->Pat.ClearData();
            return 0;
        };
            ...
            */
        {
            ...
            n = n + 2;
            network->Curve.AssureSize(n); for (auto& x : network->Curve) if (!x) x = make_cwee_shared<Scurve>(); // network->Curve[0] = nullptr;
            (network->Ncurves)++;
            ...
            */
        {
            ...
            pattern->Pat.AddUniqueValue((u64)pr->times.GetPatternStartTime(), 1.0); 
        ...
        static bool   TryFindValidDiameter(EN_Project pr, Plink link, int nodeIndex, foot_t& diameter) {
            EN_Network net = pr->network;
        ...
        static bool     IsLinkControlled(EN_Project const& pr, int LinkN) {
            bool controlled = false;
            {
                ...
                if (!controlled) {
                    ...
                    for (int ruleIndex = 1; ruleIndex < pr->network->Nrules; ruleIndex++) {
                        auto& rule = rules[ruleIndex];
                        if (rule) {
                            auto* actions = rule->ThenActions;
                            while (actions) {
        ...
        static bool     IsLinkControlled(EN_Project const& pr, Plink Link) {
            int LinkN = 0;
            LinkN = ::epanet::epanet_shared::findlink(pr->network, Link->ID);
            if (LinkN > 0)
                return IsLinkControlled(pr, LinkN);
            else 
                return false;
        };
        ...
        static void     parseZone(EN_Project const& pr, Pzone const& zone, cweeList<Pnode>& nodes, int nodeIndex, u64 const& startTime) {
            Padjlist adj; bool controlled;
            if (nodes[nodeIndex]) {
                ...
                while (adj) {
                    if (pr->network->Link[adj->link]) {
                        controlled = false;
                        if (pr->network->Link[adj->link]->Type == LinkType::PIPE) {
                            if ((StatusType)(double)pr->network->Link[adj->link]->Status(startTime) == StatusType::OPEN) {
                                // Open Pipe... is it controlled?   
                                controlled = IsLinkControlled(pr, adj->link);
                                if (!controlled) {
                                    // Open Pipe With No Controls Referencing It! Perfect candidate to merge with.                                     
                                    zone->Within_Link.Append(pr->network->Link[adj->link]);

                                    parseZone(pr, zone, nodes, adj->node, startTime);
                        ...
                        // this is either a closed pipe, check-valve, valve, pump, or a controlled open pipe.                                                          
                        if (pr->network->Link[adj->link]->N1 == nodeIndex) {
                            zone->Boundary_Link.Append(cweeUnion<Plink, Szone::direction_t>(pr->network->Link[adj->link], Szone::direction_t::FLOW_OUT_DMA));
                        }
                        else if (pr->network->Link[adj->link]->N2 == nodeIndex) {
                            zone->Boundary_Link.Append(cweeUnion<Plink, Szone::direction_t>(pr->network->Link[adj->link], Szone::direction_t::FLOW_IN_DMA));
                        }
                        else {
                            // Something went wrong with the Sparse Matrix.
                            zone->Within_Link.Append(pr->network->Link[adj->link]);
                            parseZone(pr, zone, nodes, adj->node, startTime);
                        }
                    }
        ...
        static void     combineZonesActual(cweeList<Pzone>& zones, Pzone first, Pzone second) {
            // we are being requested to merge these two zones together. 
            cweeStr name;
            ...
            Pzone newZone = make_cwee_shared<Szone>(name);
            if (static_cast<int>(second->Type) > static_cast<int>(first->Type)) {
                newZone->Type = first->Type;
            }
            else {
                newZone->Type = second->Type;
            }

            // add all of the 'within' nodes together
            for (auto& N : first->Node) {
                newZone->Node.Append(N);
                N->Zone = newZone;
            }
            for (auto& N : second->Node) {
                newZone->Node.Append(N);
                N->Zone = newZone;
            }

            // add all of the 'within' links together
            for (auto& L : first->Within_Link) newZone->Within_Link.Append(L);
            for (auto& L : second->Within_Link) newZone->Within_Link.Append(L);

            // add all of the 'boundary' links
            for (auto& L1 : first->Boundary_Link) {
                bool isUnique = true;
                for (auto& L2 : second->Boundary_Link) {
                ...
                if (isUnique) {
                    newZone->Boundary_Link.Append(L1);
                }
                else {
                    newZone->Within_Link.Append(L1.get<0>());
                }
            }
            for (auto& L1 : second->Boundary_Link) {
                bool isUnique = true;
                for (auto& L2 : first->Boundary_Link) {
                    if (L2.get<0>() == L1.get<0>()) {
                        isUnique = false;
                        break;
                    }
                ...
                if (isUnique) {
                    ...
                    // newZone->Within_Link.Append(L1.get<0>()); // Will have already been added by the previous zone! 
                }
            }
            
            // Remove the old zones from the list and add the new zone instead
            zones.RemoveFast(first);
            zones.RemoveFast(second);
            zones.Append(newZone);
        };
        static cweeList<Plink> getAllSharedLinks(Pzone const& zone_1, Pzone const& zone_2) {
            cweeList<Plink> out;
            for (auto& boundary_link1 : zone_1->Boundary_Link) {                
                for (auto& boundary_link2 : zone_2->Boundary_Link) {
                    if (boundary_link2.get<0>() == boundary_link1.get<0>()) {
                        // SHARED!
                        out.Append(boundary_link2.get<0>());
                        break;
                    }
                }
            }
        ...
        static bool     combineZones(EN_Project const& pr, cweeList<Pzone>& zones) { 
            // outter loop
            ...
            for (auto& zone_1 : zones) {
                ...
                for (auto& boundary_link : zone_1->Boundary_Link) {
                    ...
                    if (link) {
                        ...
                        AUTO sharedLinks = getAllSharedLinks(zone_1, zone_2);

                        // are any of these shared links controlled? 
                        bool anyControlled = false;
                        for (auto& shared_link : sharedLinks) {
                            anyControlled = anyControlled || IsLinkControlled(pr, shared_link);
                            if (anyControlled) break;
                        }
                        if (!anyControlled) {
                            // remove pipes / valves / pumps that are closed. (Since not controlled, will always be closed)
                            for (int i = sharedLinks.Num() - 1; i >= 0; i--) {
                                if ((StatusType)(double)sharedLinks[i]->Status(startTime) == StatusType::CLOSED) {
                                    sharedLinks.RemoveIndexFast(i);
                                }
                            }

                            // do any connections remain? (OPEN or ACTIVE)
                            if (sharedLinks.Num() > 0) {
                                // do any of these links not qualify?
                                bool allQualify = true;
                                for (auto& shared_link : sharedLinks) {
                                    bool qualifies = false;
                                    qualifies = qualifies || shared_link->Type_p == asset_t::PIPE; // qualifies if it is a pipe or check valve
                                    if (shared_link->Type == LinkType::TCV) {
                                        qualifies = qualifies || (shared_link->Kc == 0.0 || shared_link->Kc == MISSING); // qualifies if it is a TCV with a setting of 0 (basically a check valve)
                                    }

                                    //if ((!Zone1_HasWaterDemand) && /*(sharedLinks.Num() <= 1) && (zone_1->Boundary_Link.Num() <= 2) &&*/ (zone_1->Type != Szone::zoneType_t::Open)) { // one shared link (i.e. PRV) and the zone has only two boundary links (i.e. a FCV and the PRV in question) and there is nothing of import in between (i.e. simple reduced)
                                    //    // in this case, we can potentially merge a valve or pump or check valve. 
                                    //    qualifies = qualifies || true;
                                    //}                                    

                                    allQualify = allQualify && qualifies; 
                                    if (!allQualify) break;
                                }
                                if (allQualify) {
        ...
        static bool     isZoneWaterFlowPossible(EN_Project const& pr, Pzone& zone) {
            AUTO startTime = (u64)pr->times.GetSimStartTime();
            ...
            for (auto& boundary_link : zone->Boundary_Link) {
                ...
                if (link) {
                    if (link->Type == LinkType::PIPE) { 
                        if (IsLinkControlled(pr, link)) { return true; }
                        else {
                            // is it closed? 
                            if ((StatusType)(double)link->Status(startTime) == StatusType::CLOSED) {
                                continue;
                            }
                            else {
                                return true;
                        ...
                        if (boundary_link.get<1>() == Szone::direction_t::FLOW_IN_DMA) {
                            if (IsLinkControlled(pr, link)) { return true; }
                            else {
                                // is it closed? 
                                if ((StatusType)(double)link->Status(startTime) == StatusType::CLOSED) {
        ...
        static int      parseNetwork(EN_Project pr) {
            EN_Network net = pr->network;
            auto* time = &pr->times;
            ...
            // Determine approximate pump diameters (downstream > upstream > assume 1_ft)
            for (auto& pump : net->Pump) {
                if (pump) {
                    foot_t diam = 0;
                    if (TryFindValidDiameter(pr, pump.CastReference<Slink>(), pump->N2, diam)) {}
                    else if (TryFindValidDiameter(pr, pump.CastReference<Slink>(), pump->N1, diam)) {}
                    else { diam = 1_ft; }
                    pump->Diam = diam;
                }
            }
            ...
            {
                // Every node becomes its own private zone.
                AUTO startTime = (u64)pr->times.GetSimStartTime();
                cweeList<Pzone> zones;
                cweeList<Pnode> nodes = net->Node;
                auto& links = net->Link;
                auto& controls = net->Control;
                auto& rules = net->Rule;
                Padjlist adj;

                // loop through the reservoirs first and discover draw pressure zones.
                for (int i = 0; i < nodes.Num(); i++) {
                    Pnode node = nodes[i];
                    if (node && node->Type_p == asset_t::RESERVOIR) {
                        cweeStr name = cweeStr::printf("Open DMA (Reservoir %s)", node->Name_p.c_str());
                        Szone::illDefined_t illDefin = Szone::illDefined_t::Okay;
                        Szone::zoneType_t zT = Szone::zoneType_t::Open;
                        if (AUTO resP = node.CastReference<Stank>()) {
                            if (resP->Diameter == 0_ft) {
                                // infinite reservoir
                                name = cweeStr::printf("Draw Zone (Reservoir %s)", node->Name_p.c_str());
                                illDefin = Szone::illDefined_t::Okay;
                                zT = Szone::zoneType_t::Draw;

                                Pzone zone = make_cwee_shared<Szone>(name);
                                parseZone(pr, zone, nodes, i, startTime);
                ...
                // loop through the tanks and discover open pressure zones.
                for (int i = 0; i < nodes.Num(); i++) {
                    Pnode node = nodes[i];
                    if (node && node->Type_p == asset_t::RESERVOIR) {
                        cweeStr name = cweeStr::printf("Open DMA (Reservoir %s)", node->Name_p.c_str());
                        ...
                        if (AUTO resP = node.CastReference<Stank>()) {
                            if (resP->Diameter == 0_ft) {
                                ...
                                illDefin = Szone::illDefined_t::TankWithoutInflow;
                                zT = Szone::zoneType_t::Open;
                            }
                        }
                        Pzone zone = make_cwee_shared<Szone>(name);
                        parseZone(pr, zone, nodes, i, startTime);
                ...
                // loop through the remaining junctions and discover the closed or reduced pressure zones.
                for (int i = 0; i < nodes.Num(); i++) { 
                    Pnode node = nodes[i];
                    if (node) {
                        Pzone zone = make_cwee_shared<Szone>(cweeStr::printf("Reduced Zone (Junction %s)", node->Name_p.c_str()));
                        zone->IllDefined = Szone::illDefined_t::IsolatedJunction;
                        zone->Type = Szone::zoneType_t::Reduced;

                        parseZone(pr, zone, nodes, i, startTime);

                        // craft a better name for the zone based on the boundary links (if any)
                        {
                            bool done = false;
                            for (auto& L : zone->Boundary_Link) {
                                if (!done && L.get<1>() == Szone::direction_t::FLOW_IN_DMA) {
                                    if (L.get<0>()->Type_p == asset_t::PUMP) {
                                        // Closed Zone
                                        zone->Name_p = cweeStr::printf("Closed Zone (Pump %s)", L.get<0>()->Name_p.c_str());
                                        zone->IllDefined = Szone::illDefined_t::InactivePump;
                                        zone->Type = Szone::zoneType_t::Closed;
                                        done = true;
                                        break;
                                    }
                                }
                            ...
                            for (auto& L : zone->Boundary_Link) {
                                if (!done && L.get<1>() == Szone::direction_t::FLOW_IN_DMA) {
                                    if (L.get<0>()->Type_p == asset_t::VALVE) {
                                        // Reduced Zone
                                        zone->Name_p = cweeStr::printf("Reduced Zone (Valve %s)", L.get<0>()->Name_p.c_str());
                                        zone->IllDefined = Szone::illDefined_t::InactiveValve;
                                        zone->Type = Szone::zoneType_t::Reduced;
                                        done = true;
                                        break;
                                    }
                            ...
                            for (auto& L : zone->Boundary_Link) {
                                if (!done && L.get<1>() == Szone::direction_t::FLOW_IN_DMA) {
                                    if (L.get<0>()->Type_p == asset_t::PIPE) {
                                        // Reduced Zone
                                        zone->Name_p = cweeStr::printf("Reduced Zone (Pipe %s)", L.get<0>()->Name_p.c_str());
                                        zone->IllDefined = Szone::illDefined_t::IsolatedJunction;
                                        zone->Type = Szone::zoneType_t::Reduced;
                                        done = true;
                                        break;
                            ...
                            // Reduced Zone
                            if (!done) {
                                zone->Name_p = cweeStr::printf("Reduced Zone (Junction %s)", node->Name_p.c_str());
                                zone->IllDefined = Szone::illDefined_t::IsolatedJunction;
                                zone->Type = Szone::zoneType_t::Reduced;
                            }
                        }
                ...
                while (combineZones(pr, zones)) {}

                // Evaluate and (for the last time) rename the zone based on the observed characteristics
                for (auto& zone : zones) {
                    if (isZoneWaterFlowPossible(pr, zone)) {
                        zone->IllDefined = Szone::illDefined_t::Okay;
                    }
                    else {
                        // zone->Name_p = "Ill-Defined " + zone->Name_p;
                        switch (zone->IllDefined) {
                        case Szone::illDefined_t::Okay: zone->IllDefined = Szone::illDefined_t::IsolatedJunction; break;
                        case Szone::illDefined_t::IsolatedJunction:  break; // pump/valve faces wrong way or pipes are all closed
                        case Szone::illDefined_t::InactivePump:  break; // pump faces right way but is off
                        case Szone::illDefined_t::InactiveValve:  break; // valve faces right way but is off
                        case Szone::illDefined_t::TankWithoutInflow:  break; // tank has no possible fill
                        }
                    }
                }
            ...
            */
        {
            ...
            parser->PrevPat = nullptr;
            parser->PrevCurve = nullptr;
            ...
            */
        {
            ...
            ERRCODE(epanet_postcompiled::parseNetwork(pr));
            return errcode;
        };
            ...
            */
        {
            ...
            node->C0(pr->times.GetSimStartTime()) = 0.0;
            node->S = nullptr;
            node->Ke = 0.0;
            node->Rpt = 0;
            node->ResultIndex = 0;
            ...
            if (!epanet_shared::adddemand(node, pr->convertToUnit<cubic_foot_per_second_t>(y), p, NULL, p >= 0 ? (Ppattern)net->Pattern[p] : Ppattern())) return 101;
            ...
            */
        {
            ...
            node->Comment = xstrcpy(&node->Comment, parser->Comment, MAXMSG);
            tank->Node = i;
            tank->InitialHead(pr->times.GetSimStartTime()) = pr->convertToUnit<foot_t>((double)initlevel); // fixed during units conversion
            ...
            //*******************************************************************
            tank->Vcurve = curve;
            if (tank->Vcurve > 0) tank->Vcurve_Actual = net->Curve[curve];
            else tank->Vcurve_Actual = nullptr;
            ...
            */
        {
            ...
            link->Kw = MISSING;
            link->Type = type;
            link->Status(pr->times.GetSimStartTime()) = (double)status;
            link->Rpt = 0;
            link->ResultIndex = 0;
            ...
            */
        {
            ...
            else
            {
                ...
                pump->R = -b;
                pump->N = c;
                pump->InitialFlow(pr->times.GetSimStartTime()) = pr->convertToUnit< cubic_foot_per_second_t>(q1);
            ...
            */
        {
            ...
            link->Kw = 0.0;
            link->Type = PUMP;
            link->Status(pr->times.GetSimStartTime()) = (double)OPEN;
            link->Rpt = 0;
            link->ResultIndex = 0;
            link->Comment = xstrcpy(&link->Comment, parser->Comment, MAXMSG);
            ...
            while (m < n)
            {
                ...
                else if (match(parser->Tok[m - 1], w_PATTERN))  // Speed/status pattern
                {
                    ...
                    pump->Upat = p;
                    pump->TimeUpat = net->Pattern[pump->Upat];
                }
            ...
            */
        {
            ...
            if (match(parser->Tok[4], w_PRV))
                ...
                type = GPV;
            else if (match(parser->Tok[4], w_PAT)) {
                type = PRV;
            ...
            link->Status(pr->times.GetSimStartTime()) = (double)(StatusType)status;
            link->Rpt = 0;
            link->ResultIndex = 0;
            link->Comment = xstrcpy(&link->Comment, parser->Comment, MAXMSG);
            ...
            */
        {
            ...
            if (parser->PrevPat && strcmp(parser->Tok[0], parser->PrevPat->ID.c_str()) == 0)
            {
            ...
            else
            {
                ...
                if (parser->Comment[0])
                {
                    pattern->Comment = parser->Comment;
                }
            }

            // Expand the pattern's factors array
            j = 1;
            auto pst = pr->times.GetPatternStartTime();
            int N_pat = pattern->Pat.GetNumValues();
            for (auto t0 = pst + (u64)((N_pat) * pr->times.Pstep); j <= n; t0 += (u64)pr->times.Pstep) {
                u64 t = (u64)t0;
                if (!getfloat(parser->Tok[j], &x)) return setError(parser, j, 202);
                pattern->Pat.AddUniqueValue(t, x);
                j++;
            }
            ...
            */
        {
            ...
            if (net->Node[j]->D.Num() > 0 && hyd->NodeDemand[j] != (cubic_foot_per_second_t)(double)MISSING)
            {
                auto& demand = net->Node[j]->D[0];
                // First category encountered will overwrite demand category created when junction was read from [JUNCTIONS] section
                demand.Base = pr->convertToUnit<cubic_foot_per_second_t>(y);
                demand.Pat = p;
                if (demand.Pat >= 0) demand.TimePat = net->Pattern[demand.Pat];
                if (parser->Comment[0])
                {
            ...
            else if (!epanet_shared::adddemand(net->Node[j], pr->convertToUnit<cubic_foot_per_second_t>(y), p, parser->Comment, p >= 0 ? net->Pattern[p] : Ppattern())) return 101;
            return 0;
        };
            ...
            */
        {
            ...
            if (ctltype == TIMEOFDAY) {
                control->Time = (double)units::math::fmod((SCALER)(double)control->Time, SECperDAY);
            }
            control->Grade = level;
            return 0;
        };
            ...
            */
        {
            ...
            if (n == 2)
            {
                ...
                Node[j]->C0(pr->times.GetSimStartTime()) = c0;
            }
            ...
            else
            {
                    ...
                    (i2 = atol(parser->Tok[1])) > 0)
                {
                    ...
                    {
                        i = atol(Node[j]->ID);
                        if (i >= i1 && i <= i2) Node[j]->C0(pr->times.GetSimStartTime()) = c0;
                    }
                }
            ...
            */
        {
            ...
            i = j - net->Njuncs;
            if (net->Tank[i]->Diameter == 0.0_ft) return 0;
            net->Tank[i]->MixModel = (MixType)m;
            net->Tank[i]->V1frac = v;
            return 0;
        };
        static void changestatus(EN_Project pr, int j, StatusType status, SCALER y)
            /*
            **--------------------------------------------------------------
            **  Input:   j      = link index
            ...
            */
        {
            ...
            {
                if (status != ACTIVE) link->Status(pr->times.GetSimStartTime()) = (double)status;
            }
            ...
            */
        {
            ...
            if (n == 1)
            {
                ...
                changestatus(pr, j, (StatusType)status, y);
            }
                ...
                (i2 = atol(parser->Tok[1])) > 0)
            {
                ...
                {
                    i = atol(net->Link[j]->ID);
                    if (i >= i1 && i <= i2) changestatus(pr, j, (StatusType)status, y);
                }
            }
            ...
            */
        {
            ...
            if (match(parser->Tok[n - 2], w_PRICE))
            {
                ...
                if (j == 0) hyd->Ecost = (double)y;
                else        Pump[j]->Ecost = (double)y;
                return 0;
            }
            ...
            //----------------------------------------------------------
        {
            ...
            Prule rule = net->Rule[net->Nrules];
            ...
            */
        {
            ...
            epanet_shared::writeline(pr, s);
            for (i = 1; i <= net->Ntanks; i++) if (net->Tank[i]->Diameter == 0.0_ft) nres++;
