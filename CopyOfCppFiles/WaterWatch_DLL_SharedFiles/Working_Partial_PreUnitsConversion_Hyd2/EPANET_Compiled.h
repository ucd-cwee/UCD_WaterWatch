...
#pragma region Shared Functions
            ...
            */
        {
            ...
            if (hyd->HeadErrorLimit > 0.0_ft &&
                hbal->maxheaderror > (double)hyd->HeadErrorLimit) return 0;
            if (hyd->FlowChangeLimit > 0.0_cfs &&
                hbal->maxflowchange > (double)hyd->FlowChangeLimit) return 0;
                        ...
                        }
                        {
                            ...
                            if (pat) {
                                if (hyd->LinkStatus[i] <= CLOSED) {
                                    ...
                                    pat->AddUniqueValue(currentTime, hyd->LinkFlow[i] / cweeEng::SurfaceAreaCircle_ft2(obj->Diam));
                                }
                                pat->CompressLastValueAdded();
                            }
                        }
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                for (demand = net->Node[i]->D; demand != NULL; demand = demand->next)
                {
                    ...
                    if (djunc > 0.0) hyd->Dsystem += (cubic_foot_per_second_t)(double)djunc;
                    sum += djunc;
                }
            ...
            */
        {
            ...
            for (j = 1; j <= net->Npumps; j++)
            {
                ...
                k = pump->Link;
                if (hyd->LinkStatus[k] <= CLOSED) continue;
                q = (double)fMAX(QZERO, ABS(hyd->LinkFlow[k]));
            ...
            hyd->Emax = fMAX(hyd->Emax, (kilowatt_t)(double)psum);
        };
