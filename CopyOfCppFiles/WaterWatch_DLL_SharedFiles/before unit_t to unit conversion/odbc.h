...
#pragma region NANODBC DEFINITIONS AND DECLARATIONS
...
#pragma region NANODBC DECLARATIONS
namespace nanodbc
{
#ifdef DOXYGEN
...
#define NANODBC_DEPRECATED
    ...
    private:
        cweeSharedPtr<transaction_impl> impl_;
    };
    ...
    private:
        cweeSharedPtr<statement_impl> impl_;
    };
    ...
    private:
        cweeSharedPtr<connection_impl> impl_;
    };
    ...
    private:
        cweeSharedPtr<result_impl> impl_;
    };
...
#pragma endregion
#pragma region NANODBC DEFINITIONS
...
namespace nanodbc
{
    INLINE type_incompatible_error::type_incompatible_error()
    ...
    INLINE null_access_error::null_access_error()
    ...
    INLINE index_range_error::index_range_error()
    ...
    INLINE programming_error::programming_error(const std::string& info)
    ...
    INLINE database_error::database_error(SQLHANDLE handle, short handle_type, const std::string& info)
    ...
    INLINE const long database_error::native() const noexcept
    {
        return native_error;
    }

    INLINE const std::string database_error::state() const noexcept
    {
        return sql_state;
    }
...
namespace nanodbc
{
    ...
    template <class T>
    INLINE T connection::connection_impl::get_info_impl(short info_type) const
    {
        T value;
        RETCODE rc;
    ...
    template <>
    INLINE string connection::connection_impl::get_info_impl<string>(short info_type) const
    {
    ...
    INLINE string connection::connection_impl::dbms_name() const
    {
        return get_info<string>(SQL_DBMS_NAME);
    }

    INLINE string connection::connection_impl::dbms_version() const
    {
        return get_info<string>(SQL_DBMS_VER);
    }

    INLINE string connection::connection_impl::driver_name() const
    {
        return get_info<string>(SQL_DRIVER_NAME);
    }

    INLINE string connection::connection_impl::database_name() const
    {
...
namespace nanodbc
{
    ...
    template <class T>
    INLINE void statement::statement_impl::bind(
        param_direction direction,
        short param_index,
        T const* values,
        std::size_t batch_size,
    ...
    INLINE void statement::statement_impl::bind_strings(
        param_direction direction,
        short param_index,
        std::vector<T> const& values,
        bool const* nulls /*= nullptr*/,
    ...
    template <>
    INLINE bool statement::statement_impl::equals(const std::string& lhs, const std::string& rhs)
    {
        return std::strncmp(lhs.c_str(), rhs.c_str(), lhs.size()) == 0;
    }

    template <>
    INLINE bool statement::statement_impl::equals(const wide_string& lhs, const wide_string& rhs)
    {
    ...
    template <>
    INLINE bool statement::statement_impl::equals(const date& lhs, const date& rhs)
    {
        return lhs.year == rhs.year && lhs.month == rhs.month && lhs.day == rhs.day;
    }

    template <>
    INLINE bool statement::statement_impl::equals(const time& lhs, const time& rhs)
    {
        return lhs.hour == rhs.hour && lhs.min == rhs.min && lhs.sec == rhs.sec;
    }

    template <>
    INLINE bool statement::statement_impl::equals(const timestamp& lhs, const timestamp& rhs)
    {
        return lhs.year == rhs.year && lhs.month == rhs.month && lhs.day == rhs.day &&
    ...
    template <>
    INLINE std::vector<wide_string::value_type>&
    ...
    template <>
    INLINE std::vector<std::string::value_type>&
        statement::statement_impl::get_bound_string_data(short param_index)
    {
...
namespace nanodbc
{
    ...
    namespace detail
    {
        INLINE auto from_string(std::string const& s, float)
        {
            return std::stof(s);
        }

        INLINE auto from_string(std::string const& s, double)
        {
            return std::stod(s);
        }

        INLINE auto from_string(std::string const& s, long long)
        {
            return std::stoll(s);
        }

        INLINE auto from_string(std::string const& s, unsigned long long)
        {
            return std::stoull(s);
        }

        template <typename R, typename std::enable_if<std::is_integral<R>::value, int>::type = 0>
        INLINE auto from_string(std::string const& s, R)
        {
    ...
    INLINE void result::result_impl::get_ref_from_string_column(short column, T& result) const
    {
        bound_column& col = bound_columns_[column];
        const char* s = col.pdata_ + rowset_position_ * col.clen_;
        switch (col.ctype_)
...
namespace nanodbc
{
    ...
    INLINE result execute(connection& conn, const string& query, long batch_operations, long timeout)
    {
        class statement statement;
        return statement.execute_direct(conn, query, batch_operations, timeout);
    }

    INLINE void just_execute(connection& conn, const string& query, long batch_operations, long timeout)
    {
        class statement statement;
        statement.just_execute_direct(conn, query, batch_operations, timeout);
    }

    INLINE result execute(statement& stmt, long batch_operations)
    {
        return stmt.execute(batch_operations);
    }

    INLINE void just_execute(statement& stmt, long batch_operations)
    {
        return stmt.just_execute(batch_operations);
    }

    INLINE result transact(statement& stmt, long batch_operations)
    {
    ...
    INLINE void just_transact(statement& stmt, long batch_operations)
    {
        class transaction transaction(stmt.connection());
    ...
    INLINE void prepare(statement& stmt, const string& query, long timeout)
    {
...
namespace nanodbc
{
    ...
    INLINE connection::connection(const connection& rhs)
        : impl_(rhs.impl_)
    {
    }

    INLINE connection::connection(connection&& rhs) noexcept
    ...
    INLINE void connection::swap(connection& rhs) noexcept
    {
        using std::swap;
        swap(impl_, rhs.impl_);
    }

    INLINE connection::connection(const string& dsn, const string& user, const string& pass, long timeout)
        : impl_(new connection_impl(dsn, user, pass, timeout))
    {
    }

    INLINE connection::connection(const string& connection_string, long timeout)
    ...
    INLINE void connection::allocate()
    {
        impl_->allocate();
    }

    INLINE void connection::deallocate()
    {
        impl_->deallocate();
    }

    INLINE void connection::connect(const string& dsn, const string& user, const string& pass, long timeout)
    {
        impl_->connect(dsn, user, pass, timeout);
    }

    INLINE void connection::connect(const string& connection_string, long timeout)
    {
        impl_->connect(connection_string, timeout);
    }

#if !defined(NANODBC_DISABLE_ASYNC) && defined(SQL_ATTR_ASYNC_DBC_EVENT)
    INLINE bool connection::async_connect(
        const string& dsn,
        const string& user,
        const string& pass,
        void* event_handle,
    ...
    INLINE bool connection::async_connect(const string& connection_string, void* event_handle, long timeout)
    {
        return impl_->connect(connection_string, timeout, event_handle) == SQL_STILL_EXECUTING;
    }

    INLINE void connection::async_complete()
    {
    ...
    INLINE std::size_t connection::transactions() const
    {
        return impl_->transactions();
    }

    template <class T>
    INLINE T connection::get_info(short info_type) const
    {
        return impl_->get_info<T>(info_type);
    }

    INLINE void* connection::native_dbc_handle() const
    {
        return impl_->native_dbc_handle();
    }

    INLINE void* connection::native_env_handle() const
    {
        return impl_->native_env_handle();
    }

    INLINE string connection::dbms_name() const
    {
        return impl_->dbms_name();
    }

    INLINE string connection::dbms_version() const
    {
        return impl_->dbms_version();
    }

    INLINE string connection::driver_name() const
    {
        return impl_->driver_name();
    }

    INLINE string connection::database_name() const
    {
        return impl_->database_name();
    }

    INLINE string connection::catalog_name() const
    {
        return impl_->catalog_name();
    }

    INLINE std::string connection::DriverName() const
    {
        return impl_->DriverName();
    }
    INLINE void connection::DriverName(std::string name)
    {
        impl_->DriverName(name);
    }


    INLINE std::size_t connection::ref_transaction()
    {
        return impl_->ref_transaction();
    }

    INLINE std::size_t connection::unref_transaction()
    {
        return impl_->unref_transaction();
    }

    INLINE bool connection::rollback() const
    {
        return impl_->rollback();
    }

    INLINE void connection::rollback(bool onoff)
    {
...
namespace nanodbc
{
    ...
    INLINE transaction::transaction(const class connection& conn)
        : impl_(new transaction_impl(conn))
    {
    }

    INLINE transaction::transaction(const transaction& rhs)
        : impl_(rhs.impl_)
    {
    }

    INLINE transaction::transaction(transaction&& rhs) noexcept
        : impl_(std::move(rhs.impl_))
    {
    }

    INLINE transaction& transaction::operator=(transaction rhs)
    {
        swap(rhs);
        return *this;
    }

    INLINE void transaction::swap(transaction& rhs) noexcept
    {
        using std::swap;
        swap(impl_, rhs.impl_);
    }
    ...
    INLINE void transaction::rollback() noexcept
    {
        impl_->rollback();
    }

    INLINE class connection& transaction::connection()
    {
        return impl_->connection();
    }

    INLINE const class connection& transaction::connection() const
    {
        return impl_->connection();
    }

    INLINE transaction::operator class connection& ()
    {
        return impl_->connection();
    }

    INLINE transaction::operator const class connection& () const
    {
        return impl_->connection();
    }
...
namespace nanodbc
{
    ...
    INLINE statement::statement(class connection& conn)
        : impl_(new statement_impl(conn))
    {
    }

    INLINE statement::statement(statement&& rhs) noexcept
        : impl_(std::move(rhs.impl_))
    {
    }

    INLINE statement::statement(class connection& conn, const string& query, long timeout)
        : impl_(new statement_impl(conn, query, timeout))
    {
    }

    INLINE statement::statement(const statement& rhs)
        : impl_(rhs.impl_)
    {
    }

    INLINE statement& statement::operator=(statement rhs)
    {
        swap(rhs);
        return *this;
    }

    INLINE void statement::swap(statement& rhs) noexcept
    {
        using std::swap;
        swap(impl_, rhs.impl_);
    }

    INLINE statement::~statement() noexcept {}

    INLINE void statement::open(class connection& conn)
    {
        impl_->open(conn);
    }

    INLINE bool statement::open() const
    {
        return impl_->open();
    }

    INLINE bool statement::connected() const
    {
        return impl_->connected();
    }

    INLINE const class connection& statement::connection() const
    {
        return impl_->connection();
    }

    INLINE class connection& statement::connection()
    {
        return impl_->connection();
    }

    INLINE void* statement::native_statement_handle() const
    {
        return impl_->native_statement_handle();
    }

    INLINE void statement::close()
    {
        impl_->close();
    }

    INLINE void statement::cancel()
    {
        impl_->cancel();
    }

    INLINE void statement::prepare(class connection& conn, const string& query, long timeout)
    {
        impl_->prepare(conn, query, timeout);
    }

    INLINE void statement::prepare(const string& query, long timeout)
    {
        impl_->prepare(query, timeout);
    }

    INLINE void statement::timeout(long timeout)
    {
        impl_->timeout(timeout);
    }

    INLINE result statement::execute_direct(
        class connection& conn,
        const string& query,
        long batch_operations,
        long timeout)
...
#if defined(NANODBC_DO_ASYNC_IMPL)
    INLINE bool statement::async_prepare(const string& query, void* event_handle, long timeout)
    {
        return impl_->async_prepare(query, event_handle, timeout);
    }

    INLINE bool statement::async_execute_direct(
        class connection& conn,
        void* event_handle,
        const string& query,
        long batch_operations,
    ...
    INLINE bool statement::async_execute(void* event_handle, long batch_operations, long timeout)
    {
        return impl_->async_execute(event_handle, batch_operations, timeout, *this);
    }

    INLINE void statement::complete_prepare()
    {
        return impl_->complete_prepare();
    }

    INLINE result statement::complete_execute(long batch_operations)
    {
        return impl_->complete_execute(batch_operations, *this);
    }

    INLINE result statement::async_complete(long batch_operations)
    {
        return impl_->complete_execute(batch_operations, *this);
    }

    INLINE void statement::enable_async(void* event_handle)
    {
        impl_->enable_async(event_handle);
    }

    INLINE void statement::disable_async() const
    {
        impl_->disable_async();
    }
#endif

    INLINE void statement::just_execute_direct(
        class connection& conn,
        const string& query,
        long batch_operations,
        long timeout)
    ...
    INLINE  result statement::execute(long batch_operations, long timeout)
    {
        return impl_->execute(batch_operations, timeout, *this);
    }

    INLINE void statement::just_execute(long batch_operations, long timeout)
    {
        impl_->just_execute(batch_operations, timeout, *this);
    }

    INLINE result statement::procedure_columns(
        const string& catalog,
        const string& schema,
        const string& procedure,
        const string& column)
    ...
    INLINE long statement::affected_rows() const
    {
        return impl_->affected_rows();
    }

    INLINE short statement::columns() const
    {
        return impl_->columns();
    }

    INLINE short statement::parameters() const
    {
        return impl_->parameters();
    }

    INLINE void statement::reset_parameters() noexcept
    {
        impl_->reset_parameters();
    }

    INLINE unsigned long statement::parameter_size(short param_index) const
    {
...
#define NANODBC_INSTANTIATE_BINDS(type)                                                            \
    template  INLINE void statement::bind(short, const type*, param_direction);              /* 1-ary */   \
    template  INLINE void statement::bind(short, const type*, std::size_t, param_direction); /* n-ary */   \
    template  INLINE void statement::bind(                                                                 \
...
#define NANODBC_INSTANTIATE_BIND_STRINGS(type)                                                     \
    template  INLINE void statement::bind_strings(short, std::vector<type> const&, param_direction);       \
    template  INLINE void statement::bind_strings(                                                         \
...
#undef NANODBC_INSTANTIATE_BINDS
    ...
    template <class T>
    INLINE void statement::bind(short param_index, const T* value, param_direction direction)
    {
    ...
    template <class T, typename>
    INLINE void statement::bind_strings(
        short param_index,
        std::vector<T> const& values,
        param_direction direction)
    {
    ...
    INLINE void statement::bind_null(short param_index, std::size_t batch_size)
    {
        impl_->bind_null(param_index, batch_size);
    }

    INLINE void statement::describe_parameters(
        const std::vector<short>& idx,
        const std::vector<short>& type,
        const std::vector<unsigned long>& size,
        const std::vector<short>& scale)
...
namespace nanodbc
{
    ...
    INLINE catalog::tables::tables(result& find_result)
        : result_(find_result)
    {
    }

    INLINE bool catalog::tables::next()
    {
        return result_.next();
    }

    INLINE  string catalog::tables::table_catalog() const
    {
        // TABLE_CAT might be NULL
        return result_.get<string>(0, string());
    }

    INLINE    string catalog::tables::table_schema() const
    {
        // TABLE_SCHEM might be NULL
        return result_.get<string>(1, string());
    }

    INLINE     string catalog::tables::table_name() const
    {
        // TABLE_NAME column is never NULL
        return result_.get<string>(2);
    }

    INLINE     string catalog::tables::table_type() const
    {
        // TABLE_TYPE column is never NULL
        return result_.get<string>(3);
    }

    INLINE     string catalog::tables::table_remarks() const
    {
        // REMARKS might be NULL
        return result_.get<string>(4, string());
    }

    INLINE     catalog::table_privileges::table_privileges(result& find_result)
        : result_(find_result)
    {
    }

    INLINE    bool catalog::table_privileges::next()
    {
        return result_.next();
    }

    INLINE     string catalog::table_privileges::table_catalog() const
    {
        // TABLE_CAT might be NULL
        return result_.get<string>(0, string());
    }

    INLINE     string catalog::table_privileges::table_schema() const
    {
        // TABLE_SCHEM might be NULL
        return result_.get<string>(1, string());
    }

    INLINE     string catalog::table_privileges::table_name() const
    {
        // TABLE_NAME column is never NULL
        return result_.get<string>(2);
    }

    INLINE     string catalog::table_privileges::grantor() const
    {
        // GRANTOR might be NULL
        return result_.get<string>(3, string());
    }

    INLINE     string catalog::table_privileges::grantee() const
    {
        // GRANTEE column is never NULL
        return result_.get<string>(4);
    }

    INLINE     string catalog::table_privileges::privilege() const
    {
        // PRIVILEGE column is never NULL
        return result_.get<string>(5);
    }

    INLINE     string catalog::table_privileges::is_grantable() const
    {
        // IS_GRANTABLE might be NULL
        return result_.get<string>(6, string());
    }

    INLINE     catalog::primary_keys::primary_keys(result& find_result)
        : result_(find_result)
    {
    }

    INLINE     bool catalog::primary_keys::next()
    {
        return result_.next();
    }

    INLINE     string catalog::primary_keys::table_catalog() const
    {
        // TABLE_CAT might be NULL
        return result_.get<string>(0, string());
    }

    INLINE     string catalog::primary_keys::table_schema() const
    {
        // TABLE_SCHEM might be NULL
        return result_.get<string>(1, string());
    }

    INLINE     string catalog::primary_keys::table_name() const
    {
        // TABLE_NAME is never NULL
        return result_.get<string>(2);
    }

    INLINE     string catalog::primary_keys::column_name() const
    {
        // COLUMN_NAME is never NULL
        return result_.get<string>(3);
    }

    INLINE     short catalog::primary_keys::column_number() const
    {
        // KEY_SEQ is never NULL
        return result_.get<short>(4);
    }

    INLINE     string catalog::primary_keys::primary_key_name() const
    {
        // PK_NAME might be NULL
        return result_.get<string>(5);
    }

    INLINE     catalog::columns::columns(result& find_result)
        : result_(find_result)
    {
    }

    INLINE     bool catalog::columns::next()
    {
        return result_.next();
    }

    INLINE     string catalog::columns::table_catalog() const
    {
        // TABLE_CAT might be NULL
        return result_.get<string>(0, string());
    }

    INLINE     string catalog::columns::table_schema() const
    {
        // TABLE_SCHEM might be NULL
        return result_.get<string>(1, string());
    }

    INLINE     string catalog::columns::table_name() const
    {
        // TABLE_NAME is never NULL
        return result_.get<string>(2);
    }

    INLINE     string catalog::columns::column_name() const
    {
        // COLUMN_NAME is never NULL
        return result_.get<string>(3);
    }

    INLINE     short catalog::columns::data_type() const
    {
        // DATA_TYPE is never NULL
        return result_.get<short>(4);
    }

    INLINE     string catalog::columns::type_name() const
    {
        // TYPE_NAME is never NULL
        return result_.get<string>(5);
    }

    INLINE     long catalog::columns::column_size() const
    {
        // COLUMN_SIZE
        return result_.get<long>(6);
    }

    INLINE     long catalog::columns::buffer_length() const
    {
        // BUFFER_LENGTH
        return result_.get<long>(7);
    }

    INLINE     short catalog::columns::decimal_digits() const
    {
        // DECIMAL_DIGITS might be NULL
        return result_.get<short>(8, 0);
    }

    INLINE     short catalog::columns::numeric_precision_radix() const
    {
        // NUM_PREC_RADIX might be NULL
        return result_.get<short>(9, 0);
    }

    INLINE     short catalog::columns::nullable() const
    {
        // NULLABLE is never NULL
        return result_.get<short>(10);
    }

    INLINE     string catalog::columns::remarks() const
    {
        // REMARKS might be NULL
        return result_.get<string>(11, string());
    }

    INLINE     string catalog::columns::column_default() const
    {
        // COLUMN_DEF might be NULL, if no default value is specified
        return result_.get<string>(12, string());
    }

    INLINE     short catalog::columns::sql_data_type() const
    {
        // SQL_DATA_TYPE is never NULL
        return result_.get<short>(13);
    }

    INLINE     short catalog::columns::sql_datetime_subtype() const
    {
        // SQL_DATETIME_SUB might be NULL
        return result_.get<short>(14, 0);
    }

    INLINE     long catalog::columns::char_octet_length() const
    {
        // CHAR_OCTET_LENGTH might be NULL
        return result_.get<long>(15, 0);
    }

    INLINE     long catalog::columns::ordinal_position() const
    {
        // ORDINAL_POSITION is never NULL
        return result_.get<long>(16);
    }

    INLINE     string catalog::columns::is_nullable() const
    {
        // IS_NULLABLE might be NULL.
        return result_.get<string>(17, string());
    }

    INLINE     catalog::catalog(connection& conn)
        : conn_(conn)
    {
    }

    INLINE     catalog::tables catalog::find_tables(
        const string& table,
        const string& type,
        const string& schema,
        const string& catalog)
    ...
    INLINE     catalog::table_privileges
    ...
    INLINE     catalog::columns catalog::find_columns(
        const string& column,
        const string& table,
        const string& schema,
        const string& catalog)
    ...
    INLINE     std::list<string> catalog::list_catalogs()
    {
    ...
    INLINE     std::list<string> catalog::list_schemas()
    {
...
namespace nanodbc
{
    ...
    INLINE    result::result(statement stmt, long rowset_size)
        : impl_(new result_impl(stmt, rowset_size))
    {
    }

    INLINE     result::result(result&& rhs) noexcept
        : impl_(std::move(rhs.impl_))
    {
    }

    INLINE     result::result(const result& rhs)
        : impl_(rhs.impl_)
    {
    }
    ...
    INLINE     void result::swap(result& rhs) noexcept
    {
        using std::swap;
        swap(impl_, rhs.impl_);
    }

    INLINE     void* result::native_statement_handle() const
    {
        return impl_->native_statement_handle();
    }

    INLINE     long result::rowset_size() const noexcept
    {
        return impl_->rowset_size();
    }

    INLINE     long result::affected_rows() const
    {
        return impl_->affected_rows();
    }

    INLINE     bool result::has_affected_rows() const
    {
        return impl_->has_affected_rows();
    }

    INLINE     long result::rows() const noexcept
    {
        return impl_->rows();
    }

    INLINE     short result::columns() const
    {
        return impl_->columns();
    }

    INLINE     bool result::first()
    {
        return impl_->first();
    }

    INLINE     bool result::last()
    {
        return impl_->last();
    }
...
#if defined(NANODBC_DO_ASYNC_IMPL)
    INLINE bool result::async_next(void* event_handle)
    {
        return impl_->async_next(event_handle);
    }

    INLINE     bool result::complete_next()
    {
        return impl_->complete_next();
    }
#endif

    INLINE     bool result::prior()
    {
        return impl_->prior();
    }

    INLINE     bool result::move(long row)
    {
        return impl_->move(row);
    }

    INLINE     bool result::skip(long rows)
    {
        return impl_->skip(rows);
    }

    INLINE unsigned long result::position() const
    {
        return impl_->position();
    }

    INLINE bool result::at_end() const noexcept
    {
        return impl_->at_end();
    }

    INLINE bool result::is_null(short column) const
    {
        return impl_->is_null(column);
    }

    INLINE bool result::is_null(const string& column_name) const
    {
        return impl_->is_null(column_name);
    }

    INLINE short result::column(const string& column_name) const
    {
        return impl_->column(column_name);
    }

    INLINE string result::column_name(short column) const
    {
        return impl_->column_name(column);
    }

    INLINE long result::column_size(short column) const
    {
        return impl_->column_size(column);
    }

    INLINE long result::column_size(const string& column_name) const
    {
        return impl_->column_size(column_name);
    }

    INLINE int result::column_decimal_digits(short column) const
    {
        return impl_->column_decimal_digits(column);
    }

    INLINE int result::column_decimal_digits(const string& column_name) const
    {
        return impl_->column_decimal_digits(column_name);
    }

    INLINE int result::column_datatype(short column) const
    {
        return impl_->column_datatype(column);
    }

    INLINE int result::column_datatype(const string& column_name) const
    {
        return impl_->column_datatype(column_name);
    }

    INLINE string result::column_datatype_name(short column) const
    {
        return impl_->column_datatype_name(column);
    }

    INLINE string result::column_datatype_name(const string& column_name) const
    {
        return impl_->column_datatype_name(column_name);
    }

    INLINE int result::column_c_datatype(short column) const
    {
        return impl_->column_c_datatype(column);
    }

    INLINE int result::column_c_datatype(const string& column_name) const
    {
        return impl_->column_c_datatype(column_name);
    }

    INLINE  bool result::next_result()
    {
        return impl_->next_result();
    }

    template <class T>
    INLINE void result::get_ref(short column, T& result) const
    {
        return impl_->get_ref<T>(column, result);
    }

    template <class T>
    INLINE void result::get_ref(short column, const T& fallback, T& result) const
    {
        return impl_->get_ref<T>(column, fallback, result);
    }

    template <class T>
    INLINE void result::get_ref(const string& column_name, T& result) const
    {
        return impl_->get_ref<T>(column_name, result);
    }

    template <class T>
    INLINE void result::get_ref(const string& column_name, const T& fallback, T& result) const
    {
        return impl_->get_ref<T>(column_name, fallback, result);
    }

    template <class T>
    INLINE T result::get(short column) const
    {
        return impl_->get<T>(column);
    }

    template <class T>
    INLINE T result::get(short column, const T& fallback) const
    {
        return impl_->get<T>(column, fallback);
    }

    template <class T>
    INLINE T result::get(const string& column_name) const
    {
        return impl_->get<T>(column_name);
    }

    template <class T>
    INLINE T result::get(const string& column_name, const T& fallback) const
    {
...
public:
    /*! Connect to a remote or locally hosted SQL server (requires username and password) or local SQLite database (with full filepath in server slot). */
     nanodbc::connection CreateConnection(cweeStr Server, cweeStr UserID, cweeStr Password, cweeStr Driver) {
        ...
        cweeStr ext; Server.ExtractFileExtension(ext);
            ...
            if (Driver.IsEmpty()) {
                for (nanodbc::driver& driver : nanodbc::list_drivers()) {
                    ...
                    connection.connect(connectionString, 1);

                    if (connection.connected() == true && GetTableNames(connection).Num() > 0) {
                        connection.DriverName(driver.name.c_str());
                        return connection;
                    }
                }
            }
                ...
                if (connection.connected() == true && GetTableNames(connection).Num() > 0) {
                    connection.DriverName(Driver.c_str());
                    return connection;
                }
                else {
     ...
     bool IsConnected(nanodbc::connection con) {
        return con.connected();
    };

     void EndConnection(nanodbc::connection con) {
        if (IsConnected(con)) con.disconnect();
    };

     nanodbc::result Query(nanodbc::connection con, const cweeStr& query, int batchSize) {
     ...
     cweeThreadedList<cweeStr> GetNextRow(nanodbc::result& results) {
        cweeThreadedList<cweeStr> out;
        GetNextRow(results, out);
        return out;
    };

     bool GetNextRow(nanodbc::result& results, cweeThreadedList<cweeStr>& out) {
            // if (results.at_end()) return false;
            ...
            if (out.Num() == columns) {
                if (results.next()) {
                    ...
                    {
                        out[col] = results.get<nanodbc::string>(col, nullV.c_str()).c_str();
                ...
                if (results.next()) {
                    ...
                    for (int col = 0; col < columns; ++col)
                    {
                        out.Append(results.get<nanodbc::string>(col, nullV.c_str()).c_str());
                    }
     ...
     cweeThreadedList < cweeThreadedList<cweeStr> > GetResults(nanodbc::result& results) {
        cweeThreadedList < cweeThreadedList<cweeStr> > out(results.rows() + 100);
     ...
     cweeThreadedList < cweeThreadedList<cweeStr> > GetResults(nanodbc::connection con, const cweeStr& query, int batchSize) {
        nanodbc::result results = this->Query(con, query, batchSize);
        return this->GetResults(results);
    };

     void GetResults(nanodbc::connection con, const cweeStr& query, cweeThreadedList < cweeThreadedList<cweeStr> >& out, int batchSize) {
        nanodbc::result results = this->Query(con, query, batchSize);

            bool passed = results.next();
            int numRows = results.rows();
            int numCols = results.columns();
            cweeStr nullV;
            ...
            if (out.Num() >= numRows && out.Num() != 0) {
                ...
                while (passed && numCols > 0) {
                    ...
                    if (out[row].Num() == numCols) {
                        ...
                        {
                            t = results.get<nanodbc::string>(col, nullV.c_str()).c_str();
                            t.ReduceSpaces();
                            out[row][col] = t;
                        }
                    }
     ...
     cweeThreadedList<cweeStr> GetDatabaseNames(nanodbc::connection con) {
        cweeThreadedList<cweeStr> out;
            // could be MSSQL or MYSQL 
            if (cweeStr(con.DriverName().c_str()).Find("MYSQL", false) < 0 && cweeStr(con.DriverName().c_str()).Find("myodbc", false) < 0) {
                // likely not a MySQL driver (i.e. MSSQL)
                auto queryResult = this->GetResults(con, "SELECT name FROM sys.databases;", 100);
                ...
                auto queryResult = this->GetResults(con, "SHOW SCHEMAS;", 100);
                for (auto& row : queryResult) {
                    for (auto& col : row) {
                        if (!col.IsEmpty())
                            out.AddUnique(col);
     ...
     cweeThreadedList<cweeStr> GetTableNames(nanodbc::connection con, const cweeStr& databaseName = "") {
            cweeThreadedList<cweeStr> out;
            ...
            if (cweeStr(con.DriverName().c_str()).Find("MYSQL", false) < 0 && cweeStr(con.DriverName().c_str()).Find("myodbc", false) < 0) {
                ...
                if (databaseName.IsEmpty()) {
                    ...
                    if (listDB.Num() > 0) {
                        for (auto& x : listDB) {
                            for (auto& y : this->GetResults(con, cweeStr::printf("SELECT TABLE_NAME FROM %s.information_schema.tables;", x.c_str()), 1)) {
                    ...
                    queryResult = this->GetResults(con, cweeStr::printf("SELECT TABLE_NAME FROM %s.information_schema.tables;", databaseName.c_str()), 1);

                if (queryResult.Num() <= 0) {
                    queryResult = this->GetResults(con, "SELECT TABLE_NAME FROM information_schema.tables;", 1);
                }
                ...
                if (databaseName.IsEmpty()) {
                    ...
                    if (listDB.Num() > 0) {
                        for (auto& x : listDB) {
                            for (auto& y : this->GetResults(con, cweeStr::printf("SELECT table_name FROM information_schema.tables WHERE table_schema = '%s';", x.c_str()), 1)) {
                                queryResult.Append(y);
                            }
                        }
                    }
                    ...
                    queryResult = this->GetResults(con, cweeStr::printf("SELECT table_name FROM information_schema.tables WHERE table_schema = '%s';", databaseName.c_str()), 1);
     ...
     cweeThreadedList<cweeStr> GetColumnNames(nanodbc::connection con, cweeStr tableName, const cweeStr& databaseName = "") {
            cweeThreadedList<cweeStr> out;
            cweeStr query;
            ...
            if (databaseName.IsEmpty()) {
                ...
                auto queryResult = this->GetResults(con, query.c_str(), 100);
                for (auto& row : queryResult) {
                    for (auto& col : row) {
                        out.Append(col);
                    }
                ...
                if (out.Num() <= 0) {
                    query = cweeStr::printf("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '%s';", tableName.c_str());
                    queryResult = this->GetResults(con, query.c_str(), 100);
                    for (auto& row : queryResult) {
                        for (auto& col : row) {
                            out.Append(col);
                        }
     ...
     cweeThreadedList<cweeStr> FirstColumnOnly(const cweeThreadedList < cweeThreadedList<cweeStr> >& results) {
     ...
     cweeStr SafeString(const cweeStr& in, nanodbc::connection* optionalConnectionForTableSchemaCheck = nullptr) {
        /* use case examples:
     ...
     cweeStr PartialTableNameToFullTableName(nanodbc::connection con, const cweeStr& in) {
        /* use case examples:
            EBMUD.SRCE\srce-rtygood.PERSONS     -->     EBMUD.[SRCE\srce-rtygood].PERSONS
            EBMUD.ebmud.ami_usage               -->     EBMUD.ebmud.ami_usage
     ...
     cweeThreadedList < cweeStr > GetTableSchema(nanodbc::connection con, const cweeStr& tableName, const cweeStr& databaseName = "") {
            cweeThreadedList < cweeStr > out;
            cweeThreadedList < cweeThreadedList<cweeStr> > queryResult;

            if (databaseName.IsEmpty())
                queryResult = this->GetResults(con, cweeStr::printf("SELECT TABLE_SCHEMA FROM information_schema.tables WHERE TABLE_NAME = '%s';", tableName.c_str()), 100);
            else
                queryResult = this->GetResults(con, cweeStr::printf("SELECT TABLE_SCHEMA FROM %s.information_schema.tables WHERE TABLE_NAME = '%s';", databaseName.c_str(), tableName.c_str()), 100);
     ...
     bool CreateTable(nanodbc::connection con, cweeStr tableName, const cweeThreadedList<cweeStr>& columnNames, cweeStr databaseName = "") {
     ...
     cweeStr GetDatabaseName(nanodbc::connection con, cweeStr tableName) {
            for (auto& x : GetDatabaseNames(con)) {
                for (auto& y : this->GetResults(con, cweeStr::printf("SELECT TABLE_NAME FROM %s.information_schema.tables;", x.c_str()), 100)) {
                    if (y == tableName) {
                        return x;
                    }
                }
     ...
     bool TableExists(nanodbc::connection con, cweeStr tableName) {
        for (auto& x : this->GetTableNames(con)) {
            if (x == tableName) return true;
        }
        return false;
    };

     void InsertRow(nanodbc::connection con, cweeStr tableFullPath, const cweeThreadedList<cweeStr>& values) {
        cweeStr query;
            // could be MSSQL or MYSQL
            if (cweeStr(con.DriverName().c_str()).Find("MYSQL", false) < 0 && cweeStr(con.DriverName().c_str()).Find("myodbc", false) < 0) {
                // likely not a MySQL driver (i.e. MSSQL)
...
static cweeSharedPtr< cweeOdbcLocal > odbc = make_cwee_shared<cweeOdbcLocal>();