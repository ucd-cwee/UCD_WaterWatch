...
public:
	...
	public:
		...
		void* source() const noexcept final { return static_cast<void*>(&d); };
...
#else
		/*
		Original... works, but breaks when casting skips a generation. (I.e. this breaks:)

		class A{};
		class B:public A{ int B_Obj; };
		class C:public B{ int C_Obj; };

		cweeSharedPtr<A> a = nullptr;
		cweeSharedPtr<B> b = nullptr;
		cweeSharedPtr<C> c = C();

		b = c.CastReference<B>(); // works perfectly -- no object slice!
		a = c.CastReference<A>(); // works perfectly -- no object slice!
		a = b.CastReference<A>(); // Break! Will not crash, but will object-slice! Only carries the characteristics seen in B, none unique to C. (i.e. C_Obj is lost forever).
		*/
		std::function<newP(void*)> _getter = [](void* p) {
