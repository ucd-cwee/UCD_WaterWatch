		...
		if (!IsFinished.load()) {
			...
			if (GetResult().IsTypeOf<cweeJob>()) {
				...
				if (iterationNumber < 20) {
					...
					Result.Clear();
					cweeToasts::submitToast("C++ cweeJob Looping", FunctionName());
...
public:
	...
	public:
		...
		public:
			...
			public:
				...
				bool IsRunning() { return (m_Waiting.GetValue() != 1); };
	...
	public:
		...
		bool Wait_Once() {
			...
			if (1) {
				bool busy = false;
				if (cweeJobThreadsData::cweeCpuThread::cweeCpuThreadData::TryExtractNextJob(m_Jobs, out)) { out.Invoke(); WakeUpAll(); busy = true; }
				for (auto& thread : m_Threads) {
					if (thread.IsRunning()) {
						busy = true;
						break;
...
static cweeSharedPtr<cweeJobThreads> cweeSharedJobThreads = make_cwee_shared<cweeJobThreads>(new cweeJobThreads());
#pragma endregion
