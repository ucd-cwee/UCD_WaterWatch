...
long long DataManager::getCount(int element, int* count, Network* nw)
{
    int err = 0;
    *count = 0;
    switch (element)
...
int DataManager::getNodeIndex(const char* name, int* index, Network* nw)
{
...
int DataManager::getNodeId(int index, char* id, Network* nw)
{
    ...
    std::strcpy(id, nw->node(index)->name.c_str());
    return 0;
}
...
int DataManager::getLinkIndex(const char* name, int* index, Network* nw)
{
...
int DataManager::getLinkId(int index, char* id, Network* nw)
{
    ...
    std::strcpy(id, nw->link(index)->name.c_str());
    return 0;
}
