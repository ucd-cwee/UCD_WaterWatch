        ...
        int   NumberOfBoundaryLinksBetweenZones_Impl(cweeSharedPtr<Szone> const& zone) {
            ...
            for (auto& BL : Boundary_Link) {
                ...
                case direction_t::FLOW_IN_DMA:
                    reply = BL.get<0>()->StartingNode->Zone->NumberOfBoundaryLinksBetweenZones_Impl<distance + 1>(zone);
                    break;
                case direction_t::FLOW_OUT_DMA:
                    reply = BL.get<0>()->EndingNode->Zone->NumberOfBoundaryLinksBetweenZones_Impl<distance + 1>(zone);
                    break;
