...
#pragma region Shared Functions
        static int      updatepumpparams(EN_Project pr, int pumpindex)
            /*
            **-------------------------------------------------------------
        ...
        static int		findpattern(EN_Network network, char* id)
            /*----------------------------------------------------------------
        ...
        static void     writeline(EN_Project pr, char* s)
            /*
            **--------------------------------------------------------------
            **   Input:   *s = text string
            **   Output:  none
        ...
        static int		findtank(EN_Network network, int index)
            /*----------------------------------------------------------------
        ...
        static int		findpump(EN_Network network, int index)
            /*----------------------------------------------------------------
            **  Input:   index = link ID
            **  Output:  none
        ...
        static int		findvalve(EN_Network network, int index)
            /*----------------------------------------------------------------
            **  Input:   index = link ID
            **  Output:  none
        ...
        static int		findnode(EN_Network network, char* id)
            /*----------------------------------------------------------------
            **  Input:   id = node ID
            **  Output:  none
        ...
        static int		findlink(EN_Network network, char* id)
            /*----------------------------------------------------------------
            **  Input:   id = link ID
            **  Output:  none
        ...
        static int		findcurve(EN_Network network, char* id)
            /*----------------------------------------------------------------
        ...
        static int		adddemand(Pnode node, SCALER dbase, int dpat, char* dname)
            /*----------------------------------------------------------------
        ...
        static int		valvecheck(EN_Project pr, int index, int type, int j1, int j2)
            /*
            **--------------------------------------------------------------
        ...
        static int		resizecurve(Scurve* curve, int size)
            /*----------------------------------------------------------------
            ...
            */
        {
            ...
            {
                x = (SCALER*)realloc(curve->X, size * sizeof(SCALER));
                if (x == NULL) return 101;
                y = (SCALER*)realloc(curve->Y, size * sizeof(SCALER));
                if (y == NULL)
                {
                    free(x);
                    return 101;
        ...
        static int		addlinkvertex(Plink link, SCALER x, SCALER y)
            /*----------------------------------------------------------------
            ...
            */
        {
            ...
            if (link->Vertices == NULL)
            {
                ...
                if (vertices == nullptr) return 101;
                vertices->Npts = 0;
                vertices->Capacity = CHUNKSIZE;
                vertices->X.ClearedResize(vertices->Capacity);
                vertices->Y.ClearedResize(vertices->Capacity);
            ...
            {
                vertices->Capacity += CHUNKSIZE;
                vertices->X.AssureSize(vertices->Capacity);
                vertices->Y.AssureSize(vertices->Capacity);
            }
            n = vertices->Npts;
            vertices->X[n] = x;
            vertices->Y[n] = y;
        ...
        static void		clearrule(EN_Project pr, int i)
            //-----------------------------------------------------------
        ...
        static void		errmsg(EN_Project pr, int errcode)
            /*----------------------------------------------------------------
        ...
        static void     setdefaults(EN_Project pr)
            /*
            **----------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static void     initreport(Report* rpt)
            /*
            **----------------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static void     adjustdata(EN_Project pr)
            /*
            **----------------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static void     initunits(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static void     convertunits(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  none
            ...
            */
        {
            ...
            for (j = 1; j <= net->Ntanks; j++)
            {
                ...
                i = tank->Node;
                node = net->Node[i];
                tank->H0 = node->El + tank->H0;
                tank->Hmin = node->El + tank->Hmin;
                tank->Hmax = node->El + tank->Hmax;
                tank->A = PI * SQR(tank->Diameter) / 4.0;                
                tank->Kb /= SECperDAY;
                tank->V = tank->V0;
                tank->C = node->C0;
            }
        ...
        static int      inittanks(EN_Project pr)
            /*
            **---------------------------------------------------------------
            **  Input:   none
            **  Output:  returns error code
            ...
            */
        {
            ...
            {
                tank = net->Tank[j];
                if (tank->A == 0.0_sq_ft) continue;  // Skip reservoirs

                // Check for valid lower/upper tank levels
                levelerr = 0;
                if (tank->H0 > tank->Hmax || tank->Hmin > tank->Hmax || tank->H0 < tank->Hmin) levelerr = 1;
                ...
                if (i > 0)
                {
                    curve = &net->Curve[i];
                    n = curve->Npts - 1;
                    if (tank->Hmin < pr->convertToUnit<foot_t>(curve->X[0]) || tank->Hmax > pr->convertToUnit<foot_t>(curve->X[n]))
                    {
                        levelerr = 1;
                    }
                    ...
                    else
                    {
                        ...
                        tank->Vmin = pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X, curve->Y, (double)(tank->Hmin * pr->Ucf[ELEV])));
                        tank->Vmax = pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X, curve->Y, (double)(tank->Hmax * pr->Ucf[ELEV])));
                        tank->V0 = pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X, curve->Y, (double)(tank->H0 * pr->Ucf[ELEV])));

                        // Find a "nominal" diameter for tank
                        tank->Diameter = pr->convertToUnit<foot_t>((curve->Y[n] - curve->Y[0]) / (curve->X[n] - curve->X[0]));
                        tank->A = PI * SQR(tank->Diameter) / 4.0;
                    }
                }
        ...
        static void     resetpumpflow(EN_Project pr, int i)
            /*
            **-------------------------------------------------------------------
            **  Input:   i = link index
            **  Output:  none
        ...
        static void     setlinksetting(EN_Project pr, int index, SCALER value, StatusType* s, SCALER* k)
            /*----------------------------------------------------------------
            **  Input:   index  = link index
        ...
        static int      checktime(EN_Project pr, Spremise* p)
            //------------------------------------------------------------
        ...
        static int      checkstatus(EN_Project pr, Spremise* p)
            //------------------------------------------------------------
        ...
        static int      checkvalue(EN_Project pr, Spremise* p)
            //----------------------------------------------------------
            ...
            //----------------------------------------------------------
        {
            ...
            case r_PRESSURE:
                x = (double)((foot_t)(double)hyd->NodeHead[i] - Node[i]->El) * Ucf[PRESSURE];
                break;

            case r_LEVEL:
                x = (double)((foot_t)(double)hyd->NodeHead[i] - Node[i]->El) * Ucf[HEAD];
                break;
            ...
            case r_FILLTIME:
                ...
                j = i - Njuncs;
                if (Tank[j]->A == 0.0_sq_ft) return 0;
                if (NodeDemand[i] <= TINY) return 0;
                x = (double)((Tank[j]->Vmax - Tank[j]->V) / (cubic_foot_per_second_t)(double)NodeDemand[i]);
                break;
            ...
            case r_DRAINTIME:
                ...
                if (Tank[j]->A == 0.0_sq_ft) return 0;
                if (NodeDemand[i] >= -TINY) return 0;
                x = (double)((Tank[j]->Vmin - Tank[j]->V) / (cubic_foot_per_second_t)(double)NodeDemand[i]);
                break;
        ...
        static int      onactionlist(EN_Project pr, int i, Saction* a)
            //-----------------------------------------------------------------------------
        ...
        static int      checkpremise(EN_Project pr, Spremise* p)
            //----------------------------------------------------------
        ...
        static void     updateactionlist(EN_Project pr, int i, Saction* actions)
            //---------------------------------------------------
        ...
        static int      evalpremises(EN_Project pr, int i)
            //----------------------------------------------------------
        ...
        static void     setlinkstatus(EN_Project pr, int index, char value, StatusType* s, SCALER* k)
            /*----------------------------------------------------------------
            **  Input:   index  = link index
        ...
        static void     writeruleaction(EN_Project pr, int k, char* ruleID)
            /*
            **--------------------------------------------------------------
            **   Input:   k  = link index
            **            *ruleID  = rule ID
        ...
        static void     clearactionlist(Rules* rules)
            //----------------------------------------------------------
        ...
        static int      checkrules(EN_Project pr, units::time::second_t dt)
            //-----------------------------------------------------
        ...
        static int      writerule(EN_Project pr, FILE* f, int ruleIndex)
            //-----------------------------------------------------------------------------
        ...
        static void     writestatchange(EN_Project pr, int k, char s1, char s2)
            /*
            **--------------------------------------------------------------
            **   Input:   k  = link index
            **            s1 = old link status
        ...
        static void     writerelerr(EN_Project pr, int iter, SCALER relerr)
            /*
            **-----------------------------------------------------------------
        ...
        static void     writehydstat(EN_Project pr, int iter, SCALER relerr)
            /*
            **--------------------------------------------------------------
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                {
                    if (Tank[i]->A > 0.0_sq_ft && ABS((foot_t)(double)hyd->NodeHead[n] - Tank[i]->Hmax) < 0.001_ft)
                        newstat = OVERFLOWING;
                ...
                {
                    if (Tank[i]->A > 0.0_sq_ft)
                    {
                        snprintf(s1, MAXLINE, FMT50, atime, net->Node[n]->ID, StatTxt[newstat],
                            ((foot_t)(double)hyd->NodeHead[n] - net->Node[n]->El) * pr->Ucf[HEAD],
        ...
        static void     marknodes(EN_Project pr, int m, int* nodelist, char* marked)
            /*
            **----------------------------------------------------------------
        ...
        static void     getclosedlink(EN_Project pr, int i, char* marked)
            /*
            **----------------------------------------------------------------
        ...
        static void     writehyderr(EN_Project pr, int errnode)
            /*
            **-----------------------------------------------------------
            **   Input:   none
            **   Output:  none
        ...
        static int      writehydwarn(EN_Project pr, int iter, SCALER relerr)
            /*
            **--------------------------------------------------------------
            **   Input:   iter = # iterations to find hydraulic solution
            ...
            */
        {
            ...
            if (hyd->DemandModel == DDA)
            {
                ...
                {
                    node = net->Node[i];
                    if ((foot_t)(double)hyd->NodeHead[i] < node->El && hyd->NodeDemand[i] > 0.0)
                        hyd->DeficientNodes++;
                }
        ...
        static int      getnodetype(EN_Network net, int i)
            /*
            **---------------------------------------------------------
            ...
            */
        {
            ...
            if (net->Tank[i - net->Njuncs]->A == 0.0_sq_ft) return 1;
            return 2;
        };
        static void     writecontrolaction(EN_Project pr, int k, int i)
            /*
            ----------------------------------------------------------------
            **   Input:   k  = link index
            **            i  = control index
        ...
        static void     writemassbalance(EN_Project pr)
            /*
            **-------------------------------------------------------------
            **   Input:   none
            **   Output:  none
            ...
            */
        {
            ...
            SCALER q = ABS(hyd->LinkFlow[k]);
            SCALER r = link->R;                         // Resistance coeff.
            SCALER ml = link->Km;                       // Minor loss coeff.
            SCALER e = link->Kc / link->Diam;           // Relative roughness
            SCALER s = hyd->Viscos * link->Diam;        // Viscosity / diameter
            SCALER hloss, hgrad, f, dfdq, r1;
        ...
        // For closed pipe use headloss formula: hloss = CBIG*q
            ...
            hgrad = hyd->Hexp * r * ::epanet::pow(q, hyd->Hexp - 1.0);
           
            // auto link = pr->network->Link[k];
        ...
        static void  curvecoeff(EN_Project pr, int i, SCALER q, SCALER* h0, SCALER* r)
            /*
            **-------------------------------------------------------------------
            **   Input:   i   = curve index
            **            q   = flow rate
        ...
        static void headlosscoeffs(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  none
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                sm->F[row] += (double)(((foot_t)(double)hloss + node->El) / hgrad);
            ...
            */
        {
            ...
            int i, row;
            SCALER  dp,         // pressure range over which demand can vary (ft)
        ...
        // Get demand function parameters
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                if (hgrad > 0.0)
                {
                    row = sm->Row[i];
                    sm->Aii[row] += 1.0 / hgrad;
                    sm->F[row] += (double)(((foot_t)(double)hloss + net->Node[i]->El + (foot_t)(double)hyd->Pmin) / hgrad);
                }
            }
        };
        ...
        static void  emitterheadloss(EN_Project pr, int i, SCALER* hloss, SCALER* hgrad)
            /*
            **-------------------------------------------------------------
            **   Input:   i = node index
            ...
            */
        {
            ...
            q = hyd->EmitterFlow[i];
            *hgrad = hyd->Qexp * ke * ::epanet::pow(ABS(q), hyd->Qexp - 1.0);
        ...
        static void  demandheadloss(EN_Project pr, int i, SCALER dp, SCALER n, SCALER* hloss, SCALER* hgrad)
            /*
            **--------------------------------------------------------------
            **   Input:   i  = junction index
            ...
            */
        {
            ...
            SCALER d = hyd->DemandFlow[i];
            SCALER dfull = hyd->NodeDemand[i];
            SCALER r = d / dfull;
        ...
        static void  matrixcoeffs(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static void  resistcoeff(EN_Project pr, int k)
            /*
            **--------------------------------------------------------------------
            **  Input:   k = link index
            **  Output:  none
            ...
            */
        {
            ...
            case PIPE:
                ...
                {
                case HW:
                    link->R = 4.727 * L / ::epanet::pow(e, hyd->Hexp) / ::epanet::pow(d, 4.871);
                    break;
                case DW:
        ...
        static StatusType   next_prv_status(EN_Project pr, int k, StatusType s, SCALER hset, SCALER h1, SCALER h2)
            /*
            **-----------------------------------------------------------
            **  Input:   k    = link index
            **           s    = current status
        ...
        static StatusType   next_psv_status(EN_Project pr, int k, StatusType s, SCALER hset, SCALER h1, SCALER h2)
            /*
            **-----------------------------------------------------------
            **  Input:   k    = link index
            **           s    = current status
        ...
        static int          calc_and_set_prv_and_psv_status(EN_Project pr)
            /*
            **-----------------------------------------------------------------
            **  Input:   none
            ...
            */
        {
            ...
            for (i = 1; i <= net->Nvalves; i++)
            {
                ...
                case PRV:
                    ...
                    hyd->LinkStatus[k] = next_prv_status(pr, k, status, hset, hyd->NodeHead[n1], hyd->NodeHead[n2]);
                    break;
                case PSV:
                    hset = (double)(net->Node[n1]->El + (foot_t)(double)hyd->LinkSetting[k]);
                    hyd->LinkStatus[k] = next_psv_status(pr, k, status, hset, hyd->NodeHead[n1], hyd->NodeHead[n2]);
                    break;
                default:
                    continue;
                }
        ...
        static StatusType  cvstatus(EN_Project pr, StatusType s, SCALER dh, SCALER q)
            /*
            **--------------------------------------------------
        ...
        static StatusType  pumpstatus(EN_Project pr, int k, SCALER dh)
            /*
            **--------------------------------------------------
            **  Input:   k  = link index
        ...
        static StatusType  next_fcv_status(EN_Project pr, int k, StatusType s, SCALER h1, SCALER h2)
            /*
            **-----------------------------------------------------------
            **  Input:   k    = link index
            **           s    = current status
            ...
            */
        {
            ...
            else if (s == XFCV && hyd->LinkFlow[k] >= hyd->LinkSetting[k]) // else if the flowis currently in a bad state, see if the flow has returned to the desired range, and set to ACTIVE to clamp the flow. 
            {
                status = ACTIVE;
            }
            return status;
        };
        static void  tankstatus(EN_Project pr, int k, int n1, int n2)
            /*
            **----------------------------------------------------------------
            **  Input:   k  = link index
            ...
            */
        {
            ...
            // Ignore reservoirs
            tank = net->Tank[i];
            if (tank->A == 0.0_sq_ft) return;

            // Find head difference across link
            h = hyd->NodeHead[n1] - hyd->NodeHead[n2];

            // If tank is full, then prevent flow into it
            if ((foot_t)(double)hyd->NodeHead[n1] >= (tank->Hmax - (foot_t)(double)hyd->Htol) && !tank->CanOverflow)
            {
            ...
            if ((foot_t)(double)hyd->NodeHead[n1] <= (tank->Hmin + (foot_t)(double)hyd->Htol))
            {
            ...
            */
        {
            ...
            int change = FALSE,             // Status change flag
                ...
                n1,                         // Start node index
                n2;                         // End node index
            SCALER dh;                      // Head difference across link
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                {
                    hyd->LinkStatus[k] = next_fcv_status(pr, k, status, hyd->NodeHead[n1], hyd->NodeHead[n2]);
                }
        ...
        static int  badvalve(EN_Project pr, int n)
            /*
            **-----------------------------------------------------------------
            **  Input:   n = node index
        ...
        static SCALER newflows(EN_Project pr, Hydbalance* hbal)
            /*
            **----------------------------------------------------------------
            ...
            */
        {
            ...
            SCALER  dqsum,                 // Network flow change
        ...
        static void  newlinkflows(EN_Project pr, Hydbalance* hbal, SCALER* qsum, SCALER* dqsum)
            /*
            **----------------------------------------------------------------
            **  Input:   hbal = ptr. to hydraulic balance information
        ...
        static void newemitterflows(EN_Project pr, Hydbalance* hbal, SCALER* qsum, SCALER* dqsum)
            /*
            **----------------------------------------------------------------
            **  Input:   hbal = ptr. to hydraulic balance information
            **           qsum = sum of current system flows
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                dh = (double)((foot_t)(double)hyd->NodeHead[i] - net->Node[i]->El);
                dq = (hloss - dh) / hgrad;
                dq *= hyd->RelaxFactor;
        ...
        static void newdemandflows(EN_Project pr, Hydbalance* hbal, SCALER* qsum, SCALER* dqsum)
            /*
            **----------------------------------------------------------------
            **  Input:   hbal = ptr. to hydraulic balance information
            **           qsum = sum of current system flows
            ...
            */
        {
            ...
            // Get demand function parameters
            if (hyd->DemandModel == DDA) return;
            dp = fMAX((hyd->Preq - hyd->Pmin), MINPDIFF);
            n = 1.0 / hyd->Pexp;
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                demandheadloss(pr, i, dp, n, &hloss, &hgrad);
                dh = (double)((foot_t)(double)hyd->NodeHead[i] - net->Node[i]->El - (foot_t)(double)hyd->Pmin);
                dq = (hloss - dh) / hgrad;
                dq *= hyd->RelaxFactor;
                hyd->DemandFlow[i] -= dq;
        ...
        static int pdaconverged(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            ...
            */
        {
            ...
            for (i = 1; i <= pr->network->Njuncs; i++)
            {
                ...
                if (hyd->DemandFlow[i] > TOL && (foot_t)(double)hyd->NodeHead[i] - pr->network->Node[i]->El - (foot_t)(double)hyd->Pmin < (foot_t)(double)(-TOL))
                    converged = 0;
        ...
        static void  reporthydbal(EN_Project pr, Hydbalance* hbal)
            ...
            */
        {
            SCALER qchange = hbal->maxflowchange * pr->Ucf[FLOW];
            SCALER herror = hbal->maxheaderror * pr->Ucf[HEAD];
        ...
        static int  hasconverged(EN_Project pr, SCALER* relerr, Hydbalance* hbal)
            /*
            **--------------------------------------------------------------
        ...
        static cweeSharedPtr<Sasset> FindOrMakeAssetPtr(EN_Project pr, cweeStr name) {
            AUTO ptr = pr->network->Asset.Find(Sasset::Hash(type, name));
            if (!ptr) { 
                cweeSharedPtr<Sasset> a = make_cwee_shared<Sasset>(type, cweeAssetValueCollection<type>().Values);
                a->Name_p = name;
        ...
        static void SaveResultsForTimeStep(EN_Project pr) {
            EN_Network net = pr->network;
            Hydraul* hyd = &pr->hydraul;
            Smatrix* sm = &hyd->smatrix;
            Report* rpt = &pr->report;
            ...
            for (auto i = 1; i <= net->Nnodes; i++){
                ...
                cweeSharedPtr<Sasset> ptr;
                switch (obj->Type) {
                case NodeType::JUNCTION: ptr = FindOrMakeAssetPtr<asset_t::JUNCTION>(pr, cweeStr(obj->ID));
                    break;
                case NodeType::RESERVOIR: ptr = FindOrMakeAssetPtr<asset_t::RESERVOIR>(pr, cweeStr(obj->ID));
                    break;
                case NodeType::TANK: ptr = FindOrMakeAssetPtr<asset_t::RESERVOIR>(pr, cweeStr(obj->ID));
                    break;
                }     
                if (ptr) {
                    // After an analysis, DemandFlow contains node's required demand while NodeDemand contains delivered demand + emitter flow
                    {
                        AUTO pat = ptr->GetValue<_HEAD_>();
                        if (pat) {
                            pat->AddUniqueValue(currentTime, (foot_t)(double)hyd->NodeHead[i]);
                            pat->CompressLastValueAdded();
                        }
                    }
                    {
                        AUTO pat = ptr->GetValue<_DEMAND_>();
                        if (pat) {
                            pat->AddUniqueValue(currentTime, (cubic_foot_per_second_t)(double)hyd->NodeDemand[i]);
                            pat->CompressLastValueAdded();
                        }
                    }
                }
            }
            for (auto i = 1; i <= net->Nlinks; i++) {
                auto obj = net->Link[i];
                // ensure the assets exist
                cweeSharedPtr<Sasset> ptr;
                switch (obj->Type) {
                case LinkType::CVPIPE: ptr = FindOrMakeAssetPtr<asset_t::PIPE>(pr, cweeStr(obj->ID));
                    break;        // pipe with check valve
                case LinkType::PIPE: ptr = FindOrMakeAssetPtr<asset_t::PIPE>(pr, cweeStr(obj->ID));
                    break;          // pipe
                case LinkType::PUMP: ptr = FindOrMakeAssetPtr<asset_t::PUMP>(pr, cweeStr(obj->ID));
                    break;          // pump
                case LinkType::PRV: ptr = FindOrMakeAssetPtr<asset_t::VALVE>(pr, cweeStr(obj->ID));
                    break;           // pressure reducing valve
                case LinkType::PSV: ptr = FindOrMakeAssetPtr<asset_t::VALVE>(pr, cweeStr(obj->ID));
                    break;           // pressure sustaining valve
                case LinkType::PBV: ptr = FindOrMakeAssetPtr<asset_t::VALVE>(pr, cweeStr(obj->ID));
                    break;           // pressure breaker valve
                case LinkType::FCV: ptr = FindOrMakeAssetPtr<asset_t::VALVE>(pr, cweeStr(obj->ID));
                    break;           // flow control valve
                case LinkType::TCV: ptr = FindOrMakeAssetPtr<asset_t::VALVE>(pr, cweeStr(obj->ID));
                    break;           // throttle control valve
                case LinkType::GPV: ptr = FindOrMakeAssetPtr<asset_t::VALVE>(pr, cweeStr(obj->ID));
                    break;           // general purpose valve
                }
                if (ptr) {
                    if (ptr->Type_p == asset_t::PIPE || ptr->Type_p == asset_t::PUMP || ptr->Type_p == asset_t::VALVE) {
                        {
                            AUTO pat = ptr->GetValue<_FLOW_>();
                            if (pat) {
                                pat->AddUniqueValue(currentTime, (cubic_foot_per_second_t)(double)hyd->LinkFlow[i]);
                                pat->CompressLastValueAdded();
                            }
                        }
                        {
                            AUTO pat = ptr->GetValue<_VELOCITY_>();
                            if (pat) {
                                if (hyd->LinkStatus[i] <= CLOSED) {
                                    pat->AddUniqueValue(currentTime, 0_fps);
                                }
                                else {
                                    pat->AddUniqueValue(currentTime, ((cubic_foot_per_second_t)(double)hyd->LinkFlow[i]) / (cweeEng::SurfaceAreaCircle_ft2((foot_t)(double)obj->Diam)));
                                }
                                pat->CompressLastValueAdded();
                            }
                        }
                        {
                            AUTO pat = ptr->GetValue<_HEADLOSS_>();
                            if (pat) {
                                auto h = hyd->NodeHead[obj->N1] - hyd->NodeHead[obj->N2];
                                if (obj->Type != PUMP) h = ABS(h);
                                pat->AddUniqueValue(currentTime, (foot_t)(double)h);
                                pat->CompressLastValueAdded();
                            }
                        }
                        {
                            AUTO pat = ptr->GetValue<_STATUS_>();
                            if (pat) {
                    ...
                    if (ptr->Type_p == asset_t::PUMP || ptr->Type_p == asset_t::VALVE) {
                        {
                            AUTO pat = ptr->GetValue<_SETTING_>();
                            if (pat) {
                                if (hyd->LinkSetting[i] == MISSING) pat->AddUniqueValue(currentTime, 0);
                                    ...
                                    default:
                                        pat->AddUniqueValue(currentTime, hyd->LinkSetting[i]);
                                        break;
                                    }
                                }
                                pat->CompressLastValueAdded();
                    ...
                    if (ptr->Type_p == asset_t::PUMP) {
                        {
                            AUTO pat = ptr->GetValue<_ENERGY_>();
                            if (pat) {                               
                                SCALER kw, eff;
                                epanet_shared::getenergy(pr, i, &kw, &eff);
                                pat->AddUniqueValue(currentTime, (kilowatt_t)(double)kw);
                                pat->CompressLastValueAdded();
                            }
                        }
                    }
        ...
        static int  hydsolve(EN_Project pr, int* iter, SCALER* relerr)
            /*
            **-------------------------------------------------------------------
            **  Input:   none
            ...
            */
        {
            ...
            SCALER fullDemand;            // Full demand for a node (cfs)
            ...
            *iter = 1;
            while (*iter <= maxtrials)
            {
                ...
                for (i = 1; i <= net->Njuncs; i++)
                {
                    hyd->NodeHead[i] = (SCALER)sm->B_ft[sm->Row[i]];   // Update heads
                }
        ...
        static int  openhyd(EN_Project pr)
            /*
             *--------------------------------------------------------------
             *  Input:   none
             *  Output:  returns error code
        ...
        static void inithyd(EN_Project pr, int initflag)
            /*
            **--------------------------------------------------------------
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                tank = net->Tank[i];
                tank->V = tank->V0;
                hyd->NodeHead[tank->Node] = (double)tank->H0;
        ...
        static int   runhyd(EN_Project pr, units::time::second_t* t)
            /*
            **--------------------------------------------------------------
            **  Input:   none
        ...
        static int  nexthyd(EN_Project pr, units::time::second_t* tstep)
            /*
            **--------------------------------------------------------------
            **  Input:   none
        ...
        static void  closehyd(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  returns error code
        ...
        static void  freematrix(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static int  allocmatrix(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  returns error code
        ...
        static void  initlinkflow(EN_Project pr, int i, char s, SCALER k)
            /*
            **--------------------------------------------------------------------
            **  Input:   i = link index
            **           s = link status
        ...
        static void  demands(EN_Project pr)
            /*
            **--------------------------------------------------------------------
            **  Input:   none
            **  Output:  none
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                for (demand = net->Node[i]->D; demand != NULL; demand = demand->next)
                {
                    ...
                    j = demand->Pat;
                    djunc = (demand->Base) * net->Pattern[j].Pat((u64)time->GetCurrentRealHtime()) * hyd->Dmult;
            ...
            for (n = 1; n <= net->Ntanks; n++)
            {
                ...
                if (tank->A == 0.0_sq_ft)
                {
                    ...
                    {
                        i = tank->Node;
                        hyd->NodeHead[i] = (double)(net->Node[i]->El * net->Pattern[j].Pat((u64)time->GetCurrentRealHtime()));
                    }
                }
            }
            ...
            for (n = 1; n <= net->Npumps; n++)
            {
                ...
                {
                    i = pump->Link;
                    setlinksetting(pr, i, net->Pattern[j].Pat((u64)time->GetCurrentRealHtime()), &hyd->LinkStatus[i], &hyd->LinkSetting[i]);
                }
            }
        };
        static int  controls(EN_Project pr)
            /*
            **---------------------------------------------------------------------
            **  Input:   none
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ncontrols; i++)
            {
                ...
                {                   
                    ...
                    // if ((long)(double)(time->Htime + time->Tstart) % (long)SECperDAY == (long)(double)control->Time) { reset = 1; }
                }
        ...
        static units::time::second_t  timestep(EN_Project pr)
            /*
            **----------------------------------------------------------------
            **  Input:   none
        ...
        static int  tanktimestep(EN_Project pr, units::time::second_t* tstep)
            /*
            **-----------------------------------------------------------------
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (q > 0.0 && (foot_t)(double)h < tank->Hmax) v = (double)(tank->Vmax - tank->V);
                else if (q < 0.0 && (foot_t)(double)h > tank->Hmin) v = (double)(tank->Vmin - tank->V);
                else continue;
        ...
        static void  controltimestep(EN_Project pr, units::time::second_t* tstep)
            /*
            **------------------------------------------------------------------
            **  Input:   *tstep = current time step
            **  Output:  *tstep = modified current time step
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ncontrols; i++)
            {
                ...
                if ((n = control->Node) > 0)
                {
                    ...
                    {
                        v = (double)((cubic_foot_t)(double)tankvolume(pr, j, control->Grade) - net->Tank[j]->V);
                        t = (double)ROUND(v / q);
                    }
                }
        ...
        static void  ruletimestep(EN_Project pr, units::time::second_t* tstep)
            /*
            **--------------------------------------------------------------
        ...
        static void  addenergy(EN_Project pr, units::time::second_t hstep)
            /*
            **-------------------------------------------------------------
            ...
            */
        {
            ...
            int    i, j, k;
            long   m, n;
            SCALER c0, c,            // Energy cost (cost/kwh)
            ...
            if (time->Dur == 0_s) dt = 1.0;
            else if (time->Htime < time->Dur)
            {
                dt = (SCALER)(double)hstep / 3600.0;
            }
            else dt = 0.0;
            if (dt == 0.0) return;
            ...
            for (j = 1; j <= net->Npumps; j++)
            {
                ...
                if ((i = pump->Epat) > 0)
                {
                    c *= net->Pattern[i].Pat((u64)time->GetCurrentRealHtime());
                }
                else c *= f0;
        ...
        static void  getenergy(EN_Project pr, int k, SCALER* kw, SCALER* eff)
            /*
            **----------------------------------------------------------------
            **  Input:   k    = link index
        ...
        static void  getallpumpsenergy(EN_Project pr)
            /*
            **-------------------------------------------------------------
            **  Input:   none
            **  Output:  none
        ...
        static void  tanklevels(EN_Project pr, units::time::second_t tstep)
            /*
            **----------------------------------------------------------------
            **  Input:   tstep = current time step
            **  Output:  none
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                n = tank->Node;
                AUTO dv = (cubic_foot_per_second_t)(double)hyd->NodeDemand[n] * tstep;
                tank->V += dv;

                // Check if tank full/empty within next second
                if (tank->V + ((cubic_foot_per_second_t)(double)hyd->NodeDemand[n]*1_s) >= tank->Vmax)
                {
                    tank->V = tank->Vmax;
                }
                else if (tank->V - ((cubic_foot_per_second_t)(double)hyd->NodeDemand[n] * 1_s) <= tank->Vmin)
                {
                    tank->V = tank->Vmin;
                }
                hyd->NodeHead[n] = tankgrade(pr, i, (double)tank->V);
            }
        };
        static SCALER  tankvolume(EN_Project pr, int i, SCALER h)
            /*
            **--------------------------------------------------------------------
            **  Input:   i = tank index
            ...
            */
        {
            ...
            j = tank->Vcurve;
            if (j == 0) return (double)(tank->Vmin + ((foot_t)(double)h - tank->Hmin) * tank->A);

            // If curve exists, interpolate on h to find volume v remembering that volume curve is in original units.
            else
            {
                curve = &net->Curve[j];
                y = (double)((foot_t)(double)h - net->Node[tank->Node]->El) * pr->Ucf[HEAD];
                v = (double)pr->convertToUnit<cubic_foot_t>(interp(curve->Npts, curve->X, curve->Y, y));
                return v;
            }
        };
        static SCALER  tankgrade(EN_Project pr, int i, SCALER v)
            /*
            **-------------------------------------------------------------------
            **  Input:   i = tank index
            **           v = volume in tank
            ...
            */
        {
            ...
            j = tank->Vcurve;
            if (j == 0) return (double)(tank->Hmin + ((cubic_foot_t)(double)v - tank->Vmin) / tank->A);
            ...
            else
            {
                ...
                h = (double)(net->Node[tank->Node]->El + pr->convertToUnit<foot_t>(y));
                return h;
            }
        };
        static SCALER findsourcequal(EN_Project pr, int n, SCALER volout, units::time::second_t tstep)
            /*
            **---------------------------------------------------------------------
            **   Input:   n = node index
            ...
            */
        {
            ...
            if (qual->Qualflag == CHEM) {
                ...
                if ((double)(volout / tstep) <= Q_STAGNANT) return 0.0;
                ...
                case CONCEN:
                    if (net->Node[n]->Type == JUNCTION)
                    {
                        ...
                        {
                            c = -c * hyd->NodeDemand[n] * (double)tstep / volout;
                        }
                        else c = 0.0;
                    }
                    break;
            ...
            if (qual->Qualflag == ENERGYINTENSITY) {
                ...
                if (source == NULL)    return 0.0;
                if (source->C0 == 0.0) return 0.0;
                if (volout / (double)tstep <= Q_STAGNANT) return 0.0;
        ...
        static void reversesegs(EN_Project pr, int k)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            **   Output:  none
        ...
        static void addseg(EN_Project pr, int k, SCALER v, SCALER c)
            /*
            **-------------------------------------------------------------
        ...
        static char setreactflag(EN_Project pr)
            /*
            **-----------------------------------------------------------
            **   Input:   none
        ...
        static void ratecoeffs(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  none
        ...
        static void reactpipes(EN_Project pr, units::time::second_t dt)
            /*
            **--------------------------------------------------------------
            **   Input:   dt = time step
            **   Output:  none
        ...
        static void  reacttanks(EN_Project pr, units::time::second_t dt)
            /*
            **--------------------------------------------------------------
            **   Input:   dt = time step
            **   Output:  none
        ...
        static SCALER piperate(EN_Project pr, int k)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            ...
            */
        {
            ...
            q = (hyd->LinkStatus[k] <= CLOSED) ? (SCALER)0.0 : hyd->LinkFlow[k];
        ...
        static SCALER pipereact(EN_Project pr, int k, SCALER c, SCALER v, units::time::second_t dt)
            /*
            **------------------------------------------------------------
            **   Input:   k = link index
        ...
        static SCALER tankreact(EN_Project pr, SCALER c, SCALER v, SCALER kb, units::time::second_t dt)
            /*
            **-------------------------------------------------------
        ...
        static SCALER bulkrate(EN_Project pr, SCALER c, SCALER kb, SCALER order)
            /*
            **-----------------------------------------------------------
        ...
        static SCALER wallrate(EN_Project pr, SCALER c, SCALER d, SCALER kw, SCALER kf)
            /*
            **------------------------------------------------------------
            **   Input:   c = current WQ concentration
            **            d = pipe diameter
        ...
        static SCALER mixtank(EN_Project pr, int n, SCALER volin, SCALER massin, SCALER volout)
            /*
            **------------------------------------------------------------
            **   Input:   n      = node index
        ...
        static void tankmix1(EN_Project pr, int i, SCALER vin, SCALER win, SCALER vnet)
            /*
            **---------------------------------------------
            **   Input:   i = tank index
            **            vin = inflow volume
        ...
        static void tankmix2(EN_Project pr, int i, SCALER vin, SCALER win, SCALER vnet)
            /*
            **------------------------------------------------
            **   Input:   i = tank index
            **            vin = inflow volume
            ...
            */
        {
            ...
            vmz = (double)(tank->V1frac * tank->Vmax);
        ...
        static void tankmix3(EN_Project pr, int i, SCALER vin, SCALER win, SCALER vnet)
            /*
            **----------------------------------------------------------
            **   Input:   i = tank index
            **            vin = inflow volume
        ...
        static void tankmix4(EN_Project pr, int i, SCALER vin, SCALER win, SCALER vnet)
            /*
            **----------------------------------------------------------
            **   Input:   i = tank index
            **            vin = inflow volume
        ...
        static void transport(EN_Project pr, units::time::second_t tstep)
            /*
            **--------------------------------------------------------------
            ...
            */
        {
            ...
            for (j = 1; j <= net->Nnodes; j++)
            {
                ...
                for (alink = net->Adjlist[n]; alink != NULL; alink = alink->next)
                {
                    ...
                    else volout += ABS(((hyd->LinkStatus[k] <= CLOSED) ? (SCALER)0.0 : hyd->LinkFlow[k]));
                }

                // ... if node is a junction, add on any external outflow (e.g., demands)
                if (net->Node[n]->Type == JUNCTION)
                {
                    volout += fMAX(0.0, hyd->NodeDemand[n]);
                }
        ...
        static void  evalnodeinflow(EN_Project pr, int k, units::time::second_t tstep, SCALER* volin, SCALER* massin)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
        ...
        static SCALER  findnodequal(EN_Project pr, int n, SCALER volin, SCALER massin, SCALER volout, units::time::second_t tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   n = node index
            ...
            */
        {
            ...
            if (net->Node[n]->Type == JUNCTION)
            {
                ...
                volin -= fMIN(0.0, hyd->NodeDemand[n]) * (double)tstep;
        ...
        static SCALER  noflowqual(EN_Project pr, int n)
            /*
            **--------------------------------------------------------------
            **   Input:   n = node index
        ...
        static void evalnodeoutflow(EN_Project pr, int k, SCALER c, units::time::second_t tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            ...
            */
        {
            ...
            v = ABS(((hyd->LinkStatus[k] <= CLOSED) ? (SCALER)0.0 : hyd->LinkFlow[k])) * (double)tstep;
            if (v == 0.0) return;
        ...
        static void updatemassbalance(EN_Project pr, int n, SCALER massin, SCALER volout, units::time::second_t tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   n = node index
            ...
            */
        {
            ...
            case JUNCTION:
                masslost = fMAX(0.0, hyd->NodeDemand[n]) * (double)tstep * qual->NodeQual[n];
        ...
        static int sortnodes(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  returns an error code
        ...
        static int selectnonstacknode(EN_Project pr, int numsorted, int* indegree)
            /*
            **--------------------------------------------------------------
        ...
        static void initsegs(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  none
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                {
                    v = (0.785398 * net->Link[(k)]->Len * SQR(net->Link[(k)]->Diam));
                    j = net->Link[k]->N2;
                    c = qual->NodeQual[j];
                    addseg(pr, k, v, c);
                }
            ...
            {
                // Skip reservoirs
                if (net->Tank[j]->A == 0.0_sq_ft) continue;

                // Establish initial tank quality & volume
                k = net->Tank[j]->Node;
                c = net->Node[k]->C0;
                v = (double)net->Tank[j]->V0;
                ...
                if (net->Tank[j]->MixModel == MIX2)
                {
                    ...
                    v1 = (double)fMAX(
                        0_cu_ft, 
                        ((cubic_foot_t)(double)v) - (net->Tank[j]->Vmax * net->Tank[j]->V1frac)
                    );
        ...
        static int openqual(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  returns error code
            ...
            */
        {
            ...
            qual->PipeRateCoeff = (SCALER*)calloc(n, sizeof(SCALER));
        ...
        static int initqual(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  none
        ...
        static int runqual(EN_Project pr, units::time::second_t* t)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int nextqual(EN_Project pr, units::time::second_t* tstep)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int stepqual(EN_Project pr, units::time::second_t* tleft)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int closequal(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  returns error code
        ...
        static SCALER avgqual(EN_Project pr, int k)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
        ...
        static SCALER sourcequal(EN_Project pr, Psource source)
            /*
            **--------------------------------------------------------------
            ...
            */
        {
            ...
            i = source->Pat;
            if (i == 0)  return c;
            return (c * net->Pattern[i].Pat((u64)time->GetCurrentRealQtime()));
        };
        ...
        static SCALER  findstoredmass(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            ...
            */
        {
            ...
            {
                // ... skip reservoirs
                if (net->Tank[i]->A == 0.0_sq_ft) continue;
        ...
        static int savehyd(EN_Project pr, units::time::second_t* htime)
            /*
            **--------------------------------------------------------------
        ...
        static int savehydstep(EN_Project pr, units::time::second_t* hydstep)
            /*
            **--------------------------------------------------------------
        ...
        static int readhyd(EN_Project pr, units::time::second_t* hydtime)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int readhydstep(EN_Project pr, units::time::second_t* hydstep)
            /*
            **--------------------------------------------------------------
            **   Input:   none
        ...
        static int saveoutput(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **   Input:   none
            **   Output:  returns error code
        ...
        static int nodeoutput(EN_Project pr, int j, REAL4* x, SCALER ucf)
            /*
            **--------------------------------------------------------------
            ...
            */
        {
            ...
            case PRESSURE:
                for (i = 1; i <= net->Nnodes; i++)
                {
                    x[i] = (REAL4)((double)((foot_t)(double)hyd->NodeHead[i] - net->Node[i]->El) * ucf);
                }
                break;
        ...
        static int linkoutput(EN_Project pr, int j, REAL4* x, SCALER ucf)
            /*
            **----------------------------------------------------------------
                ...
                for (i = 1; i <= net->Nlinks; i++)
                {
                    if (net->Link[i]->Type <= PIPE && ABS(hyd->LinkFlow[i]) > TINY)
                    {
                        h = ABS(hyd->NodeHead[net->Link[i]->N1] -
                            hyd->NodeHead[net->Link[i]->N2]);
                        f = 39.725 * h * ::epanet::pow(net->Link[i]->Diam, 5) /
        ...
        static int savefinaloutput(EN_Project pr)
            /*
            **--------------------------------------------------------------
            **  Input:   none
            **  Output:  returns error code
        ...
        static int savetimestat(EN_Project pr, REAL4* x, HdrType objtype)
            /*
            **--------------------------------------------------------------
        ...
        static int savenetreacts(EN_Project pr, SCALER wbulk, SCALER wwall, SCALER wtank, SCALER wsource)
            /*
            **-----------------------------------------------------
        ...
        static void saveauxdata(EN_Project pr, FILE* f)
            /*
            ------------------------------------------------------------
        ...
        static int saveinpfile(EN_Project pr, const char* fname)
            /*
            -------------------------------------------------
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (tank->A == 0.0_sq_ft)
                {
                    ...
                    sprintf(s, " %-31s %12.4f", node->ID, (double)(node->El));
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (tank->A > 0.0_sq_ft)
                {
                    ...
                    sprintf(s, " %-31s %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f",
                        node->ID, 
                        (double)(node->El),
                        (double)((tank->H0 - node->El)),
                        (double)((tank->Hmin - node->El)),
                        (double)((tank->Hmax - node->El)),
                        (double)(::epanet::sqrt(4.0 * tank->A / PI)),
                        (double)(tank->Vmin));
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                else
                {
                    fprintf(f, "\n%s %12.4f %12.4f %12.4f          0.0 %12.4f", s,
                        ...
                        (double)((-pump->H0 - pump->R * ::epanet::pow(pump->Q0, pump->N)) * pr->Ucf[HEAD]),
                        (double)(pump->Q0 * pr->Ucf[FLOW]),
            ...
            for (i = 1; i <= net->Npats; i++)
            {
                ...
                for (auto& knot : net->Pattern[i].Pat.GetTimeSeries((u64)pr->times.GetPatternStartTime(), (u64)(pr->times.GetPatternStartTime()+ (u64)pr->times.Dur), (u64)pr->times.Pstep)) {
                    if (j % 6 == 0) fprintf(f, "\n %-31s", net->Pattern[i].ID);
                    fprintf(f, " %12.4f", (double)knot.second);
                    j++;
                }
            }
            ...
            for (i = 1; i <= net->Ncurves; i++)
            {
                ...
                {
                    curve = &net->Curve[i];
                    fprintf(f, "\n %-31s %12.4f %12.4f", curve->ID, (double)curve->X[j], (double)curve->Y[j]);
                }
            }
            ...
            for (i = 1; i <= net->Nnodes; i++)
            {
                ...
                fprintf(f, "\n %-31s %14.6f", node->ID, (double)(node->C0 * pr->Ucf[QUALITY]));
            }
            ...
            case QualType::TRACE_QUAL:
                fprintf(f, "\n QUALITY             TRACE %-31s",                    
                    net->Node[epanet_shared::findnode(net, (char*)qual->TraceNode.c_str())]->ID);
                break;
            case QualType::AGE:
                fprintf(f, "\n QUALITY             AGE");
                break;
            ...
            for (i = 1; i <= net->Nlinks; i++)
            {
                ...
                if (link->Vertices != NULL)
                {
                    for (j = 0; j < link->Vertices->Npts; j++)
                        fprintf(f, "\n %-31s %14.6f %14.6f",
                            link->ID, (double)link->Vertices->X[j], (double)link->Vertices->Y[j]);
                }
            }
