...
namespace epanet {
    class epanet {public:
            ...
            */
        {
            ...
            *index = 0;
            ...
            if (nodeType == EN_JUNCTION)
            {
                ...
                net->Njuncs++;
                nIdx = net->Njuncs;
                net->Node[nIdx] = make_cwee_shared<Snode>(asset_t::JUNCTION, std::move(cweeAssetValueCollection<asset_t::JUNCTION>().Values), id);
                node = net->Node[nIdx];
                node->D = nullptr;
            ...
            */
        {
            ...
            case EN_MINVOLUME:
                ...
                if (i > 0)                                 // tank has a volume curve
                {
                    ...
                    Tank[j]->Vmin = p->convertToUnit<cubic_foot_t>(value);                  // new min. volume
                }
            ...
            case EN_VOLCURVE:
                ...
                Tank[j]->Vmax = epanet_shared::tankvolume(p, j, Tank[j]->Hmax); // new max. volume
                Tank[j]->A = p->convertToUnit<square_foot_t>((curve->Curve.GetCurrentValue(curve->Curve.GetMaxTime()) - curve->Curve.GetCurrentValue(curve->Curve.GetMinTime())) / (curve->Curve.GetMaxTime() - curve->Curve.GetMinTime()));
            ...
            case EN_MINLEVEL:
                ...
                if (i > 0)                                 // tank has a volume curve
                {
                    ...
                    Tank[j]->Vmin = epanet_shared::tankvolume(p, j, (foot_t)(double)hTmp); // new min. volume
                }
            ...
            case EN_MAXLEVEL:
                ...
                Tank[j]->Vmax = epanet_shared::tankvolume(p, j, (foot_t)(double)hTmp);     // new max. volume
                break;
            ...
            */
        {
            ...
            Tank[j]->Diameter = units::math::sqrt(Tank[j]->A * 4.0 / PI);
            ...
            */
        {
            ...
            *index = 0;
            ...
            if (linkType <= PIPE) {
                net->Npipes++;
                net->Link[n] = make_cwee_shared<Slink>(asset_t::PIPE, std::move(cweeAssetValueCollection<asset_t::PIPE>().Values), id);
            ...
            */
        {
            ...
            n = net->Ncontrols + 1;
            net->Control.AssureSize(n + 1); for (auto& x : net->Control) if (!x) x = make_cwee_shared<Scontrol>();
            ...
            */
        {
            ...
            // Resize rules array
            net->Rule.AssureSize(net->Nrules + 2); for (auto& x : net->Rule) if (!x) x = make_cwee_shared<Srule>();
            rules->Errcode = 0;
