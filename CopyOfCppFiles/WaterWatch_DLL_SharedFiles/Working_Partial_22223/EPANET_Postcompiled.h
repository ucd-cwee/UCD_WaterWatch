...
namespace epanet {
    class epanet_postcompiled { public:
            ...
            //--------------------------------------------------------------
        {
            ...
            net->Rule.ClearedResize(n); for (auto& x : net->Rule) if (!x) x = make_cwee_shared<Srule>();
            ...
            */
        {
            ...
            if (!errcode)
            {
                ...
                pr->network->Control.ClearedResize(pr->parser.MaxControls + 1); for (auto& x : pr->network->Control) if (!x) x = make_cwee_shared<Scontrol>();
            }
            ...
            */
        {
            ...
            n = n + 2;
            network->Pattern.AssureSize(n); for (auto& x : network->Pattern) if (!x) x = make_cwee_shared<Spattern>();
            (network->Npats)++;
            ...
            */
        {
            ...
            n = n + 2;
            network->Curve.AssureSize(n); for (auto& x : network->Curve) if (!x) x = make_cwee_shared<Scurve>();
            (network->Ncurves)++;
        ...
        static int      parseNetwork(EN_Project pr) {
            ...
            // Fix pattern looping by hand-copying the data backwards and forwards
            /*
            for (auto& pattern : net->Pattern) {
                AUTO minPatTime = pattern->Pat.GetMinTime();
                AUTO maxPatTime = pattern->Pat.GetMaxTime();
                if (maxPatTime > minPatTime) { // nothing can be done in terms of looping if there is only one value
                    // first, ensure we have the project duration covered. 
                    {
                        AUTO patStep = time->Pstep;
                        AUTO simStart = time->GetSimStartTime();
                        AUTO simEnd = ((u64)time->Dur) + simStart;

                        int N_max = (u64)((maxPatTime - minPatTime) / (u64)patStep);
                        AUTO t_0 = minPatTime;
                        for (AUTO t = cweeTime((u64)maxPatTime + (u64)patStep); t <= simEnd; t += (u64)patStep) {
                            AUTO v = pattern->Pat.GetCurrentValue(t_0);
                            pattern->Pat.AddUniqueValue((u64)t, v);

                            t_0 += (u64)patStep;
                            if (t_0 > maxPatTime) {
                                t_0 -= (maxPatTime - minPatTime);
                                t_0 -= (u64)patStep;
                            }
                        }
                    }
            ...
            */
        {
            ...
            // Save junction data
            net->Node[njuncs] = make_cwee_shared<Snode>(asset_t::JUNCTION, std::move(cweeAssetValueCollection<asset_t::JUNCTION>().Values), parser->Tok[0]);
            ...
            */
        {
            ...
            // Save pipe data
            net->Link[net->Nlinks] = make_cwee_shared<Slink>(asset_t::PIPE, std::move(cweeAssetValueCollection<asset_t::PIPE>().Values), parser->Tok[0]);
            ...
            */
        {
            ...
            else
            {
                ...
                pump->R = -b;
                pump->N = c;
                pump->InitialFlow() = pr->convertToUnit< cubic_foot_per_second_t>(q1);
