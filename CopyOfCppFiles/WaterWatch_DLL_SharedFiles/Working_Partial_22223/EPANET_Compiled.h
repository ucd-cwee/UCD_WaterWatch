...
#pragma region Shared Functions
            ...
            */
        {
            ...
            if (pump->Ptype == CONST_HP)  // Constant Hp pump
            {
                ...
                pump->InitialFlow() = 1.0_cfs;   // Init. flow = 1 cfs
                return errcode;
            }
            ...
            else if (pump->Ptype == NOCURVE) // Pump curve specified
            {
                ...
                else
                {
                    ...
                    pump->Qmax = pr->convertToUnit< cubic_foot_per_second_t>(curve->Curve.GetMaxTime());
                    pump->InitialFlow() = pr->convertToUnit< cubic_foot_per_second_t>((curve->Curve.GetMinTime() + curve->Curve.GetMaxTime()) / 2.0); // assumes the initial operating point is halfway between the min and max flows in the curve
                    pump->Hmax = pr->convertToUnit < foot_t>(curve->Curve.GetCurrentValue(curve->Curve.GetMinTime()));
                }
                ...
                if (pump->Ptype == POWER_FUNC)
                {
                    ...
                    else
                    {
                        ...
                        pump->R = -b;
                        pump->N = c;
                        pump->InitialFlow() = pr->convertToUnit< cubic_foot_per_second_t>(q1); // assumes the initial operating point is the middle point of the 3 provided points
            ...
            */
        {
            ...
            time->Rstep_JunctionsPipes = time->Rstep * 2.0;         // 1 hr reporting period
            ...
            */
        {
            ...
            time->Rstep_JunctionsPipes = time->Rstep * 2.0;
            ...
            for (j = 1; j <= net->Ntanks; j++)
            {
                ...
                if (i > 0)
                {
                    ...
                    else
                    {
                        ...
                        tank->Vmin = pr->convertToUnit<cubic_foot_t>(curve->Curve.GetCurrentValue((double)(tank->Hmin * pr->Ucf[ELEV])));
                        tank->Vmax = pr->convertToUnit<cubic_foot_t>(curve->Curve.GetCurrentValue((double)(tank->Hmax * pr->Ucf[ELEV])));

                        // Find a "nominal" diameter for tank
                        tank->A = pr->convertToUnit<square_foot_t>((curve->Curve.GetCurrentValue(curve->Curve.GetMaxTime()) - curve->Curve.GetCurrentValue(curve->Curve.GetMinTime())) / (curve->Curve.GetMaxTime() - curve->Curve.GetMinTime()));
                        tank->Diameter = units::math::sqrt(tank->A * 4.0 / PI);
                    }
                }
            ...
            */
        {
            ...
            if (pump->Ptype == CONST_HP)
                pr->hydraul.LinkFlow[i] = pump->InitialFlow();
        };
            ...
            //----------------------------------------------------------
        {
            ...
            case r_FILLTIME:
                ...
                x = (double)((Tank[j]->Vmax - hyd->TankVolume[j]) / NodeDemand[i]);
                break;
            ...
            case r_DRAINTIME:
                ...
                if (Tank[j]->A == 0.0_sq_ft) return 0;
                if (NodeDemand[i] >= (cubic_foot_per_second_t)(double)(-TINY)) return 0;
                x = (double)((Tank[j]->Vmin - hyd->TankVolume[j]) / NodeDemand[i]);
                break;
        ...
        static void  pipecoeff(EN_Project pr, int k, Plink& link)
            /*
            **--------------------------------------------------------------
            **   Input:   k = link index
            **   Output:  none
...

        {
            ...
            const auto& ml = link->Km;
            const auto& r = link->R_FlowResistance;

            // Friction head loss gradient
            hgrad = (double)(hyd->Hexp * r * ::epanet::pow(q, hyd->Hexp - 1.0));
        ...
        static void  emitterheadloss(EN_Project pr, int i, foot_t* hloss, ft_per_cfs_t* hgrad)
            /*
            **-------------------------------------------------------------
            **   Input:   i = node index
        ...
        static void  demandheadloss(EN_Project pr, int i, foot_t dp, SCALER n, foot_t* hloss, ft_per_cfs_t* hgrad)
            /*
            **--------------------------------------------------------------
            **   Input:   i  = junction index
        ...
        static void  newlinkflows(EN_Project pr, Hydbalance* hbal, cubic_foot_per_second_t* qsum, cubic_foot_per_second_t* dqsum)
            /*
            **----------------------------------------------------------------
            **  Input:   hbal = ptr. to hydraulic balance information
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                const auto& n1 = link->N1;
                const auto& n2 = link->N2;
        ...
        static void newemitterflows(EN_Project pr, Hydbalance* hbal, cubic_foot_per_second_t* qsum, cubic_foot_per_second_t* dqsum)
            /*
            **----------------------------------------------------------------
            **  Input:   hbal = ptr. to hydraulic balance information
            **           qsum = sum of current system flows
        ...
        static void newdemandflows(EN_Project pr, Hydbalance* hbal, cubic_foot_per_second_t* qsum, cubic_foot_per_second_t* dqsum)
            /*
            **----------------------------------------------------------------
            **  Input:   hbal = ptr. to hydraulic balance information
            **           qsum = sum of current system flows
        ...
        static cweeSharedPtr<Sasset> FindOrMakeAssetPtr(EN_Project pr, cweeStr name) {
            ...
            if (!ptr) { 
                cweeSharedPtr<Sasset> a = make_cwee_shared<Sasset>(type, std::move(cweeAssetValueCollection<type>().Values), name);
        ...
        static void SaveResultsForTimeStep(EN_Project pr) {
            ...
            AUTO t = units::math::fmod(time->Htime, time->Rstep_JunctionsPipes);
            ...
            for (auto i = 1; i <= net->Nlinks; i++) {
                auto& obj = net->Link[i];
                if (obj) {
                    if (obj->Type_p == asset_t::PIPE || obj->Type_p == asset_t::PUMP || obj->Type_p == asset_t::VALVE) {
                        if ((t == 0_s) || obj->Type_p != asset_t::PIPE) {
                    ...
                    if (obj->Type_p == asset_t::PUMP || obj->Type_p == asset_t::VALVE) {
                        {
            ...
            */
        {
            ...
            {
                hyd->LinkFlow[i] = k * n->Pump[findpump(n, i)]->InitialFlow();
            }
            else
            {
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (q > 0.0 && (foot_t)(double)h < tank->Hmax) v = (double)(tank->Vmax - hyd->TankVolume[i]);
                else if (q < 0.0 && (foot_t)(double)h > tank->Hmin) v = (double)(tank->Vmin - hyd->TankVolume[i]);
                else continue;
            ...
            */
        {
            ...
            hyd->Etotal = (kilowatt_t)(double)psum;
        };
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (hyd->TankVolume[i] + (hyd->NodeDemand[n] * 1_s) >= tank->Vmax)
                {
                    hyd->TankVolume[i] = tank->Vmax;
                }
                else if (hyd->TankVolume[i] - (hyd->NodeDemand[n] * 1_s) <= tank->Vmin)
                {
                    hyd->TankVolume[i] = tank->Vmin;
                }

                hyd->NodeHead[n] = tankgrade(pr, i, hyd->TankVolume[i]);
            }
        };
        static cubic_foot_t  tankvolume(EN_Project pr, int i, foot_t h)
            /*
            **--------------------------------------------------------------------
            **  Input:   i = tank index
        ...
        static foot_t  tankgrade(EN_Project pr, int i, cubic_foot_t v)
            /*
            **-------------------------------------------------------------------
            **  Input:   i = tank index
            **           v = volume in tank
            ...
            */
        {
            ...
            else
            {
                ...
                h = net->Node[tank->Node]->El + pr->convertToUnit<foot_t>(y);
                return h;
            }
        };
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (tank->A > 0.0_sq_ft)
                {
                    ...
                    sprintf(s, " %-31s %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f",
                        node->ID, 
                        (double)(node->El),
                        (double)((tank->InitialHead(pr->times.GetSimStartTime()) - node->El)),
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                else
                {
                    fprintf(f, "\n%s %12.4f %12.4f %12.4f          0.0 %12.4f", s,
                        ...
                        (double)(((SCALER)(double)(-pump->H0) - pump->R * ::epanet::pow((SCALER)(double)pump->InitialFlow(), pump->N)) * pr->Ucf[HEAD]),
                        (double)(pump->InitialFlow() * pr->Ucf[FLOW]),
