...
#define cweeAssetValueCollection_UseSet
#ifdef cweeAssetValueCollection_UseSet
#define AddValueType(v) { cweeBalancedPattern<typename DefaultUnits<v>::unit> pat; pat.SetInterpolationType(interpolation_t::IT_LEFT_CLAMP); pat.SetBoundaryType(boundary_t::BT_CLAMPED); Values.AddOrReplace(cweeUnion<int, cweeAny>(static_cast<int>(v), cweeAny(pat))); }
...
#define AddValueType(v) { cweeBalancedPattern<typename DefaultUnits<v>::unit> pat; pat.SetInterpolationType(interpolation_t::IT_LEFT_CLAMP); pat.SetBoundaryType(boundary_t::BT_CLAMPED); Values.Emplace(static_cast<int>(v), pat); }
    using ValuesContainerType = cweeThreadedMap<int, cweeAny>;
#endif
   
    template <asset_t type> class cweeAssetValueCollection {
    public:
        cweeAssetValueCollection() : 
            Values(
#ifdef cweeAssetValueCollection_UseSet
                 std::function<int(const cweeUnion<int, cweeAny>&)>([](const cweeUnion<int, cweeAny>& v)->int { return v.get<0>(); })
#endif
            )
        {
...
#undef AddValueType
    ...
    public: // construction or destruction
        Sasset() : 
            ...
            Name_p() 
        {};
        Sasset(asset_t type_m, ValuesContainerType&& values) : Type_p(type_m), Values_p(std::forward<ValuesContainerType>(values)), Name_p() {};
        Sasset(asset_t type_m, ValuesContainerType&& values, const cweeStr& name) : Type_p(type_m), Values_p(std::forward<ValuesContainerType>(values)), Name_p(name) {};
        virtual	~Sasset() {};
    ...
    public:
        ...
        template<value_t v> cweeSharedPtr<cweeBalancedPattern<typename DefaultUnits<v>::unit>>		GetValue() const {
            //cweeBalancedPattern<typename DefaultUnits<v>::unit>* out = nullptr;
            //AUTO iter = Values_p.find(static_cast<int>(v));
            //if (iter != Values_p.end() && iter.state.iter.second) {
            //    AUTO ptr = iter.state.iter.second.Get();
            //    if (ptr) {
            //        out = ptr->cast<cweeBalancedPattern<typename DefaultUnits<v>::unit>*>();
            //    }
            //}

            cweeSharedPtr<cweeBalancedPattern<typename DefaultUnits<v>::unit>> out = nullptr;
#ifdef cweeAssetValueCollection_UseSet
                AUTO anyP = Values_p.Find(static_cast<int>(v));
                if (anyP && anyP->get<1>() && anyP->get<1>().IsTypeOf<cweeBalancedPattern<typename DefaultUnits<v>::unit>>()) {
                    out = anyP->get<1>().cast<cweeSharedPtr<cweeBalancedPattern<typename DefaultUnits<v>::unit>>>();
                }
                return out;
#else
    ...
    private:
        ValuesContainerType	Values_p; // private access due to the type-erasure
    ...
    public:
        Snode() : Sasset() {};
        Snode(asset_t type_m, ValuesContainerType&& values) : Sasset(type_m, std::forward<ValuesContainerType>(values)) {};
        Snode(asset_t type_m, ValuesContainerType&& values, const cweeStr& name) : Sasset(type_m, std::forward<ValuesContainerType>(values), name) {};
        virtual ~Snode() {};
    private:
    ...
    public:
        ...
        SCALER&  C0() { return C0_internal; };             // initial quality
    ...
    public:
        Slink() : Sasset() {};
        Slink(asset_t type_m, ValuesContainerType && values) : Sasset(type_m, std::forward<ValuesContainerType>(values)) {};
        Slink(asset_t type_m, ValuesContainerType && values, const cweeStr& name) : Sasset(type_m, std::forward<ValuesContainerType>(values), name) {};
        virtual ~Slink() {};
    ...
    {
    public:
        Stank() : Snode(asset_t::RESERVOIR, std::move(cweeAssetValueCollection<asset_t::RESERVOIR>().Values)) {};
        Stank(const cweeStr& name) : Snode(asset_t::RESERVOIR, std::move(cweeAssetValueCollection<asset_t::RESERVOIR>().Values), name) {};
        virtual ~Stank() {};
    ...
    {
    public:
        Spump() : Slink(asset_t::PUMP, std::move(cweeAssetValueCollection<asset_t::PUMP>().Values)) {};
        Spump(const cweeStr& name) : Slink(asset_t::PUMP, std::move(cweeAssetValueCollection<asset_t::PUMP>().Values), name) {};
        virtual ~Spump() {};
    private:
    ...
    public:
        ...
        cubic_foot_per_second_t&
            InitialFlow() { return Q0_internal; };              // initial flow
        cubic_foot_per_second_t
                Qmax;            // maximum flow
    ...
    {
    public:
        Svalve() : Slink(asset_t::VALVE, std::move(cweeAssetValueCollection<asset_t::VALVE>().Values)) {};
        Svalve(const cweeStr& name) : Slink(asset_t::VALVE, std::move(cweeAssetValueCollection<asset_t::VALVE>().Values), name) {};
        virtual ~Svalve() {};
    ...
    */
    ...
    public:
        Network() :
            ...
            System(make_cwee_shared<Sasset>(asset_t::ANY, std::move(cweeAssetValueCollection<asset_t::ANY>().Values), "System"))
        {
    ...
    INLINE cubic_foot_t Stank::Volume(cweeSharedPtr<Project> pr, foot_t head) {
        cubic_foot_t out = A * (Hmin - El);
        foot_t lvl = units::math::fmax(head - Hmin, 0.0_ft);
        if (Vcurve_Actual) {
            out += pr->convertToUnit<cubic_foot_t>(Vcurve_Actual->Curve.GetCurrentValue((double)(lvl * pr->Ucf[ELEV])));
        }
        else {
            out += A * lvl;
        }
