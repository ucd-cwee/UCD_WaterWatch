...
template< class Y_Axis_Type = units::length::meter_t, class X_Axis_Type = u64 > class cweeBalancedPattern : public cweeBalancedPatternDetail::_cwee_balanced_pattern_t {
public:
	...
	cweeBalancedPattern(){ AUTO g = lock.Guard(); container.Reserve(granularity); };
	...
	cweeBalancedPattern(const cweeThreadedList<pairT>& data) {
		{
			AUTO g = lock.Guard();
			container.Reserve(granularity);
		}
	...
	cweeThreadedList<node_type*>						UnsafeGetKnotSeries(const X_Axis_Type& t0 = -std::numeric_limits < X_Axis_Type>::max(), const X_Axis_Type& t1 = std::numeric_limits < X_Axis_Type>::max()) const {
	 ...
	 X_Axis_Type										GetAvgTime() const {
		...
		for (auto& x : container) {
			if (x.object) {
				num++;
				out -= (out / (X_Axis_Type)num);
				out += (x.key / (X_Axis_Type)num);
			}
		}
