...
#pragma region Shared Functions
            ...
            */
        {
            ...
            if (pump->Ptype == CONST_HP)
                pr->hydraul.LinkFlow[i] = (cubic_foot_per_second_t)(double)pump->Q0;
        };
            ...
            //----------------------------------------------------------
        {
            ...
            case r_FILLTIME:
                ...
                x = (double)((Tank[j]->Vmax - Tank[j]->V) / NodeDemand[i]);
                break;
            ...
            case r_DRAINTIME:
                ...
                if (Tank[j]->A == 0.0_sq_ft) return 0;
                if (NodeDemand[i] >= (cubic_foot_per_second_t)(double)(-TINY)) return 0;
                x = (double)((Tank[j]->Vmin - Tank[j]->V) / NodeDemand[i]);
                break;
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                {
                    if (Tank[i]->A > 0.0_sq_ft && ABS(hyd->NodeHead[n] - Tank[i]->Hmax) < 0.001_ft)
                        newstat = OVERFLOWING;
            ...
            */
        {
            ...
            for (i = 1; i <= net->Npumps; i++)
            {
                ...
                {
                    if (hyd->LinkFlow[j] > (cubic_foot_per_second_t)(double)(hyd->LinkSetting[j] * pump->Qmax)) s = XFLOW;
            ...
            */
        {
            ...
            SCALER s = hyd->Viscos * link->Diam;        // Viscosity / diameter = ft2/s * ft = cfs
            ...
            {
                r = 16.0 * PI * s * r;
                hloss = (double)hyd->LinkFlow[k] * (r + ml * q);
                hgrad = r + 2.0 * ml * q;
            }
            ...
            else
            {
                ...
                r1 = f * r + ml;
                hloss = r1 * q * (double)hyd->LinkFlow[k];
            ...
            if (hyd->LinkStatus[k] <= CLOSED || setting == 0.0)
            {
                ...
                if (pump->Ptype == NOCURVE)
                {
                    ...
                    if (pump->Ptype == CUSTOM)
                    {
                        ...
                        hloss = pump->H0 * SQR(setting) + hgrad * (double)hyd->LinkFlow[k];
                    }
                    else
                    {
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                sm->Aii[row] += (cfs_p_ft_t)(double)(1.0 / hgrad);
                sm->F[row] += (cubic_foot_per_second_t)(double)(((foot_t)(double)hloss + node->El) / hgrad);
        ...
        // Get demand function parameters
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                if (hgrad > 0.0)
                {
                    row = sm->Row[i];
                    sm->Aii[row] += (cfs_p_ft_t)(double)(1.0 / hgrad);
                    sm->F[row] += (cubic_foot_per_second_t)(double)(((foot_t)(double)hloss + net->Node[i]->El + (foot_t)(double)hyd->Pmin) / hgrad);
                }
            }
        };
            ...
            */
        {
            ...
            SCALER d = (double)hyd->DemandFlow[i];
            SCALER dfull = (double)hyd->NodeDemand[i];
            SCALER r = d / dfull;
            ...
            */
        {
            ...
            if (dh > hmax + (SCALER)(double)hyd->Htol) return XHEAD;
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                n1 = link->N1;
                n2 = link->N2;
                dh = (double)(hyd->NodeHead[n1] - hyd->NodeHead[n2]);
                ...
                if (link->Type == CVPIPE)
                {
                    hyd->LinkStatus[k] = cvstatus(pr, hyd->LinkStatus[k], dh, (double)hyd->LinkFlow[k]);
                }
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                dh = (double)(hyd->NodeHead[n1] - hyd->NodeHead[n2]);
                dq = (double)(hyd->Y[k] - (hyd->P[k] * (foot_t)(double)dh));
                ...
                {
                    n = findpump(net, k);
                    if (net->Pump[n]->Ptype == CONST_HP && dq > (SCALER)(double)hyd->LinkFlow[k])
                    {
                        dq = (double)(hyd->LinkFlow[k] / 2.0);
                    }
                }

                // Update link flow and system flow summation
                hyd->LinkFlow[k] -= (cubic_foot_per_second_t)(double)dq;
                *qsum += (SCALER)(double)ABS(hyd->LinkFlow[k]);
                *dqsum += ABS(dq);
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                dh = (double)(hyd->NodeHead[i] - net->Node[i]->El);
                dq = (hloss - dh) / hgrad;
                dq *= hyd->RelaxFactor;
                hyd->EmitterFlow[i] -= (cubic_foot_per_second_t)(double)dq;

                // Update system flow summation
                *qsum += (double)ABS(hyd->EmitterFlow[i]);
                *dqsum += ABS(dq);
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                demandheadloss(pr, i, dp, n, &hloss, &hgrad);
                dh = (double)(hyd->NodeHead[i] - net->Node[i]->El - (foot_t)(double)hyd->Pmin);
                dq = (hloss - dh) / hgrad;
                dq *= hyd->RelaxFactor;
                hyd->DemandFlow[i] -= (cubic_foot_per_second_t)(double)dq;

                // Update system flow summation
                *qsum += (SCALER)(double)ABS(hyd->DemandFlow[i]);
                *dqsum += ABS(dq);
            ...
            */
        {
            ...
            for (k = 1; k <= net->Nlinks; k++)
            {
                ...
                n2 = link->N2;
                dh = (double)(hyd->NodeHead[n1] - hyd->NodeHead[n2]);
                headloss = (double)(hyd->Y[k] / hyd->P[k]);
                        ...
                        }
                        {
                            ...
                            if (pat) {
                                if (hyd->LinkStatus[i] <= CLOSED) {
                                    ...
                                    pat->AddUniqueValue(currentTime, hyd->LinkFlow[i] / (cweeEng::SurfaceAreaCircle_ft2((foot_t)(double)obj->Diam)));
                                }
                                pat->CompressLastValueAdded();
                            }
                        }
            ...
            */
        {
            ...
            *iter = 1;
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                fullDemand = (double)hyd->NodeDemand[i];
                hyd->NodeDemand[i] = hyd->DemandFlow[i] + hyd->EmitterFlow[i];
                hyd->DemandFlow[i] = (double)fullDemand;
            }
            ...
            */
        {
            ...
            for (i = 1; i <= net->Nlinks; i++)
            {
                ...
                if (hyd->LinkStatus[i] <= CLOSED)
                {
                    hyd->LinkFlow[i] = (cubic_foot_per_second_t)(double)QZERO;
                }
                else if (ABS(hyd->LinkFlow[i]) <= (cubic_foot_per_second_t)(double)QZERO || initflag > 0)
                {
            ...
            */
        {
            ...
            {
                hyd->LinkFlow[i] = (cubic_foot_per_second_t)(double)(k * n->Pump[findpump(n, i)]->Q0);
            }
            else
            {
                hyd->LinkFlow[i] = (cubic_foot_per_second_t)(double)(PI * SQR(link->Diam) / 4.0);
            }
        };
            ...
            */
        {
            ...
            for (i = 1; i <= net->Njuncs; i++)
            {
                ...
                hyd->NodeDemand[i] = (double)sum;

                // Initialize pressure dependent demand
                hyd->DemandFlow[i] = (double)sum;
            }
            ...
            for (n = 1; n <= net->Ntanks; n++)
            {
                ...
                if (tank->A == 0.0_sq_ft)
                {
                    ...
                    {
                        i = tank->Node;
                        hyd->NodeHead[i] = net->Node[i]->El * net->Pattern[j].Pat((u64)time->GetCurrentRealHtime());
                    }
                }
            }
            ...
            */
        {
            ...
            for (j = 1; j <= net->Npumps; j++)
            {
                ...
                k = pump->Link;
                if (hyd->LinkStatus[k] <= CLOSED) continue;
                q = (double)fMAX((cubic_foot_per_second_t)(double)QZERO, ABS(hyd->LinkFlow[k]));
            ...
            */
        {
            ...
            q = (double)ABS(hyd->LinkFlow[k]);
            dh = (double)ABS(hyd->NodeHead[link->N1] - hyd->NodeHead[link->N2]);
            ...
            */
        {
            ...
            for (i = 1; i <= net->Ntanks; i++)
            {
                ...
                if (tank->V + (hyd->NodeDemand[n]*1_s) >= tank->Vmax)
                {
                    tank->V = tank->Vmax;
                }
                else if (tank->V - (hyd->NodeDemand[n] * 1_s) <= tank->Vmin)
                {
                    tank->V = tank->Vmin;
                }
                hyd->NodeHead[n] = (double)(tankgrade(pr, i, (double)tank->V));
            }
        };
              ...
              //loss, d = diam., & L = pipe length
                for (i = 1; i <= net->Nlinks; i++)
                {
                    if (net->Link[i]->Type <= PIPE && ABS(hyd->LinkFlow[i]) > (cubic_foot_per_second_t)(double)TINY)
                    {
                        ...
                        f = 39.725 * h * ::epanet::pow(net->Link[i]->Diam, 5) / net->Link[i]->Len / (SCALER)(double)SQR(hyd->LinkFlow[i]);
                        x[i] = (REAL4)f;
                    }
                    else x[i] = 0.0f;
                }
