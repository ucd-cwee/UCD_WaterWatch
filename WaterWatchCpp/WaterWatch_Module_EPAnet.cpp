                ...
                /* EPAnet optimization */ {
                    {
                        ...
                        lib->add(chaiscript::fun([](cweeSharedPtr<EPAnetProject>& control_project, int numValves){
                            Alternating_OptimizationManagementTool< Random_OptimizationManagementTool<true>, Genetic_OptimizationManagementTool<true>>
                                ramt(numValves, 30 * numValves);
                            ...
                            AUTO linkIsGoodCandidate = [=](::epanet::Plink link) -> bool {
                                if (link) {
                                    ...
                                    AUTO max_flow = flowPat->GetMaxValue();
                                                ...
                                                if (flowPat) {
                                                    ...
                                                    if (avgFlow > units::flowrate::gallon_per_minute_t(0)) { // positive direction
                                                        ...
                                                        if (linkID > 0) {
                                                            ...
                                                            p->epanetProj->network->Link[linkID]->Vertices = p->epanetProj->network->Link[link_index]->Vertices; // the vertices are good as-is
                                                            newLinks.Append(p->epanetProj->network->Link[linkID]);
                                                        }
                                                        else {
                                                        ...
                                                        if (linkID > 0) {
                                                            ...
                                                            if (p->epanetProj->network->Link[link_index]->Vertices) {
                                                                // the vertices need to be reversed to make any sense
                                                                p->epanetProj->network->Link[link_index]->Vertices->Array.Reverse(); 
                                                                // copies the list to the end, then fast-swaps the first with the last items and results 
                                                                // in a reversed list.
                                                                p->epanetProj->network->Link[linkID]->Vertices = p->epanetProj->network->Link[link_index]->Vertices;
                                                            }
                                                            newLinks.Append(p->epanetProj->network->Link[linkID]);
                                                        }
                                                        else {
                            ...
                            AUTO todo = [=](cweeThreadedList<float> const& policy)-> float {                                
                                ...
                                if (!p || newLinks.size() == 0) { // newPolicyStr is unusable in this state                                    
                                    performance = PenaltyValue;
                                ...
                                /* The placement appears to be valid. Let's try it by simulation. */
                                { 
                                    p->epanetProj->times.Dur /= 7.0;
                                    AUTO hyd = p->StartHydraulicSimulation();
                                    while (true) {
                                        hyd->DoSteadyState();
                                ...
                                /* Calculate the MWh */
                                units::energy::megawatt_hour_t tot; 
                                units::energy::megawatt_hour_t tot_pump; {
                                    for (auto& x : p->epanetProj->network->Pump) { if (x) { 
                                        AUTO pat = x->GetValue<_ENERGY_>();
                                        if (pat) {
                                            tot += pat->RombergIntegral(pat->GetMinTime(), pat->GetMaxTime());
                                            tot_pump += pat->RombergIntegral(pat->GetMinTime(), pat->GetMaxTime());
                                        }
                                    } }
                                    for (auto& x : p->epanetProj->network->Valve) {
                                        if (x && x->ProducesElectricity) {
                                            AUTO pat = x->GetValue<_ENERGY_>();
                                            if (pat) {
                                                tot += pat->RombergIntegral(pat->GetMinTime(), pat->GetMaxTime());
                                            }
                                    ...
                                    //if (tot_pump <= units::energy::megawatt_hour_t(0)) {
                                        // probably wrong
                                    //    performance = PenaltyValue;
                                    //}
                                    //else {
                                ...
                                /* Evaluate water customer pressures */ {
                                    ...
                                    else if (this_minCustomerPSI < minCustomerPSI) {
                                        // bad customer pressure
                                        performance += (((minCustomerPSI - this_minCustomerPSI) / units::pressure::pounds_per_square_inch_t(1)) * units::energy::megawatt_hour_t(50))(); // 50MWh penalty per min pressure reduction

                                        fileSystem->ensureDirectoryExists(fileSystem->getDataFolder() + "\\OptimizationFiles");
                                        cweeStr newFp = fileSystem->createFilePath(fileSystem->getDataFolder() + "\\OptimizationFiles", cweeStr(performance) + "_AT_" + newPolicyStr.ReplaceInline(", ", "_"), fileType_t::INP);
                                        p->saveINP(newFp);
                                    }
                                    else {
                                        // all good?
                            ...
                            AUTO isFinishedFunc = [=](sharedObjType& shared, cweeThreadedList<u64>& bestPolicy, u64 bestPerformance) -> bool {
                            ...
                            try {
                                ...
                                bv_final["performance"] = chaiscript::var(shared->results[shared->results.Num() - 1].bestPerformance);
                                ...
                                if (p) {
                                    ...
                                    units::energy::megawatt_hour_t tot_pump;
                                    /* Calculate the MWh */ {
                                        for (auto& x : p->epanetProj->network->Pump) {
                                            if (x) {
                                                AUTO pat = x->GetValue<_ENERGY_>();
                                                if (pat) {
                                                    tot += pat->RombergIntegral(pat->GetMinTime(), pat->GetMaxTime());
                                        ...
                                        for (auto& x : p->epanetProj->network->Valve) {
                                            if (x) {
                                                cweeUnitValues::cweeUnitPattern flow(*x->GetValue<_FLOW_>()); // positive flow
                                                cweeUnitValues::cweeUnitPattern head(*x->GetValue<_HEADLOSS_>()); // positive headloss

                                                AUTO energyPat = (flow * head * units::constants::d * units::constants::g / 1.31);

                                                cweeUnitValues::unit_value v = cweeUnitValues::unit_value(units::energy::megawatt_hour_t());
                                                v = energyPat.RombergIntegral(energyPat.GetMinTime(), energyPat.GetMaxTime());
                                                units::energy::megawatt_hour_t v2 = v();
                                                tot -= v2;
                                            }
                                        }
                                    }
                                    ...
                                    bv_final["Energy Used For Pumping"] = chaiscript::var(cweeUnitValues::unit_value(cweeUnitValues::megawatt_hour(tot_pump)));
                                    bv_final["Energy Generated"] = chaiscript::var(cweeUnitValues::unit_value(cweeUnitValues::megawatt_hour(tot_pump - tot)));
                                    ...
                                    // save result
                                    fileSystem->ensureDirectoryExists(fileSystem->getDataFolder() + "\\OptimizationFiles");
                                    cweeStr newFp = fileSystem->createFilePath(fileSystem->getDataFolder() + "\\OptimizationFiles", cweeStr("result"), fileType_t::INP);
                                    p->saveINP(newFp);
                                }
                        ...
                        lib->add(chaiscript::fun([](cweeSharedPtr<EPAnetProject>& control_project, int numValves) { using namespace cwee_units;
                            units::power::kilowatt_t ERT_minPowerProduction = 10_kW;
                            units::length::foot_t ERT_minHead = 2_m;
                            units::flowrate::gallon_per_minute_t ERT_minFlow = 0.006_cu_m / 1_s;
                            units::pressure::pounds_per_square_inch_t minAllowedCustomerPSI = units::pressure::pounds_per_square_inch_t(40.0);
                            units::pressure::pounds_per_square_inch_t minAllowedPSI = units::pressure::pounds_per_square_inch_t(5.0);
                            Dollar_t ERT_cost = 6000;
                            Dollar_t PRV_cost = 3000;
                            AUTO EnergyPrice = 0.07_USD / 1_kWh;


                            int maxIterations = 1000; // optimization iterations
                            float eps = 0.000001f; // optimization epsilon

                            /* Cached results when re-evaluating the same policy */
                            cweeSharedPtr<cweeThreadedMap<std::string, double>>  cachedResults = make_cwee_shared<cweeThreadedMap<std::string, double>>();

                            /* shared results PTR for the optimization */ using sharedObjType = ChaiscriptOptimizationObj;
                            /* optimization tool */
                            //Alternating_OptimizationManagementTool< 
                            //    Random_OptimizationManagementTool<true>, 
                                // Genetic_OptimizationManagementTool<true>
                                ParticleSwarm_OptimizationManagementTool<true>
                            //>
                                ramt(
                                    numValves // num dimensions
                                    , 20 * numValves // num policies per iteration
                                );
                            const auto PenaltyValue = ramt.default_constraint(); // penalty values decrease proportionatly with the number of dimensions. 
                            ...
                            for (auto& zone : control_network->Zone) {
                                units::pressure::pounds_per_square_inch_t controlZonePressure; int zoneCount = 0;
                                units::pressure::pounds_per_square_inch_t controlZoneCustomerPressure; int zoneCustomerCount = 0;
                                if (zone) {
                                    for (auto& node : zone->Node) {
                                        if (node) {
                                            ...
                                            if (pat) {
                                                ...
                                                if (node->HasWaterDemand()) {
                                                    ...
                                                    cweeMath::rollingAverageRef(controlZoneCustomerPressure, thisPressure, zoneCustomerCount);
                                                }
                                                cweeMath::rollingAverageRef(controlZonePressure, thisPressure, zoneCount);
                                            }
                                        }
                                    }
                                    for (auto& node : zone->Node) {
                            ...
                            /* create a temp INP file that we can duplicate for the future networks */ auto fp = fileSystem->createRandomFile(fileType_t::INP); {
                                control_project->saveINP(fp); 
                            }

                            // FUNCTION: basic check to see if a link is a decent candidate
                            AUTO linkIsGoodCandidate = [=](::epanet::Plink link) -> bool {
                                if (link) {
                                    if (link->Type_p == asset_t::PUMP) { // cannot replace a pump
                                        return false;
                                    ...
                                    if (link->Type >= ::epanet::LinkType::PRV) {
                                        ...
                                        if (link->Kc == ::epanet::MISSING) {
                                            if ((::epanet::StatusType)(double)link->Status(control_project->epanetProj->times.GetSimStartTime()) == ::epanet::OPEN)
                                            {
                                                return true; // INITIALLY OPEN VALVE
                                            }
                                            if ((::epanet::StatusType)(double)link->Status(control_project->epanetProj->times.GetSimStartTime()) == ::epanet::CLOSED)
                                            {
                                                return false; // INITIALLY CLOSED VALVE
                                            }
                                        }
                                        return false; // UNKNOWN
                                    }

                                    // pipe / checkvalve == good options
                                    if ((::epanet::StatusType)(double)link->Status(control_project->epanetProj->times.GetSimStartTime()) == ::epanet::CLOSED) {
                                        return false;
                                    }
                                    else {
                            ...
                            // FUNCTION: walk upstream as far as possible when considering a location
                            AUTO getNextGoodLink = [=](::epanet::Plink link) -> ::epanet::Plink {
                                if (link && linkIsGoodCandidate(link)) {
                                    if (link->GetValue<_FLOW_>()->GetAvgValue() > units::flowrate::gallon_per_minute_t(0)) { // positive direction
                                        AUTO links = control_project->getConnectedLinks(link->N1); // upstream
                            ...
                            /* convert the decision space into a Hilbert curve space */
                            cweeSharedPtr < cweeInterpolatedMatrix<float>> coord_to_candidate_index = make_cwee_shared < cweeInterpolatedMatrix<float> >();
                            cweeSharedPtr < cweeInterpolatedMatrix<float>> coord_to_x = make_cwee_shared < cweeInterpolatedMatrix<float> >();
                            cweeSharedPtr < cweeInterpolatedMatrix<float>> coord_to_y = make_cwee_shared < cweeInterpolatedMatrix<float> >();

                            /* Get the list of all possible candidate sites that could accept new PRVs or ERTs */
                            cweeSharedPtr < cweeList<::epanet::Plink> > candidates = make_cwee_shared < cweeList<::epanet::Plink> >(); {
                                for (int i = 0; i < control_network->Link.Num(); i++) {
                                    AUTO control_link2 = control_network->Link[i];
                                    if (control_link2) {
                                        auto control_link = control_link2;
                                        while (control_link2 && control_link) {
                                            control_link2 = getNextGoodLink(control_link);
                                        ...
                                        if (!candidates->contains(control_link)) {
                                            AUTO x = control_link->X()();
                                            AUTO y = control_link->Y()();

                                            coord_to_candidate_index->AddValue(x, y, candidates->Append(control_link));
                                            coord_to_x->AddValue(x, y, x);
                                            coord_to_y->AddValue(x, y, y);
                                        }
                                    }
                                }
                            }
                            // Note: these candidates have not yet looked at flowrate, zone connectivity, etc. and are therefore candiates in all dimenions
                            ...
                            AUTO createEPAnetProject = [=](cweeThreadedList<float> const& policy, cweeList<::epanet::Plink>& newLinks, cweeStr& policyStr)->cweeSharedPtr<EPAnetProject> {
                                ...
                                cweeThreadedMap<std::string, units::flowrate::gallon_per_minute_t> averageFlowrateMap;
                                for (auto& hilbertPos : policy) {
                                    int candidate_index = cweeMath::Rint(coord_to_candidate_index->HilbertPositionToValue(hilbertPos));
                                    auto newLinkName = "NewLink" + cweeStr(cweeRandomInt(0, 10000));
                                    try {
                                        AUTO control_link = candidates->operator[](candidate_index);

                                        if (control_link) {
                                            // the control_link now determines the link_index;
                                            AUTO link_index = control_project->getlinkIndex(control_link->Name_p);
                            ...
                            AUTO min_search_position = coord_to_candidate_index->MinHilbertPosition();
                            AUTO max_search_position = coord_to_candidate_index->MaxHilbertPosition();

                            cweeSharedPtr < cweeList<cweeList<float>> > candidates_solutions = make_cwee_shared< cweeList<cweeList<float>> >();

                            // Random_OptimizationManagementTool<true> ramt_first(numValves, 10 * numValves);
                            cweeSharedPtr<sharedObjType> shared = make_cwee_shared<sharedObjType>();
                            for (int i = 0; i < numValves; i++) ramt.lower_constraints().Emplace(min_search_position, i);
                            for (int i = 0; i < numValves; i++) ramt.upper_constraints().Emplace(max_search_position, i);
                            ...
                            AUTO evaluate = [=](cweeSharedPtr<EPAnetProject> proj, cweeList<::epanet::Plink> newLinks) -> cweeThreadedMap<std::string, cweeUnitValues::unit_value> {
                                        ...
                                        for (auto& node : zone->Node) {
                                            if (node) {
                                                ...
                                                if (pat) {
                                                    ...
                                                    if (minPressure && thisPressure < *minPressure) {
                                                        ...
                                                        if (node->HasWaterDemand() && thisPressure < minAllowedCustomerPSI) { 
                                                            ...
                                                            penalty += (minAllowedCustomerPSI - thisPressure)();
                                                        }
                                                    }
                                                }
                                            }
                                ...
                                year_t surveyFreq = cweeEng::LeakageModel::IdealSurveyFrequency(valves, 
                                    (92950900.0 / 4.0) / 365,
                                ...
                                AUTO result = cweeEng::LeakageModel::LeakModelResults(valves, surveyFreq, 
                                    (92950900.0 / 4.0) / 365,
                                    controlCustomerPressure,
                                    customerPressure,
                                    1.3, // # infrastructure condition factor
                            ...
                            AUTO todo = [=](cweeThreadedList<float> const& policy)-> float {
                                ...
                                cweeStr policyStr; {
                                    cweeList<int> sortedIndexes1;
                                    for (auto& hilbertPos : policy) {
                                        int link_index = cweeMath::Rint(coord_to_candidate_index->HilbertPositionToValue(hilbertPos));
                                        link_index = control_project->getlinkIndex(candidates->operator[](link_index)->Name_p);
                                        sortedIndexes1.Append(link_index);
                                    }
                                    sortedIndexes1.Sort();
                                    for (auto& link_index : sortedIndexes1) {
                                ...
                                /* The placement appears to be valid! */
                                candidates_solutions.Lock();
                                double candidate_size = candidates_solutions.UnsafeGet()->Append(policy) + 1;
                                candidates_solutions.Unlock();

                                cweeSharedPtr<EPAnetProject> proj = createEPAnetProject(policy, newLinks, policyStr);
                                if (proj) {
                                    AUTO perf = evaluate(proj, newLinks);

                                    performance = perf.TryGetPtr("Performance")->operator()();
                                    ...
                                    /* Evaluate water customer pressures */ {
                                        ...
                                        if (this_minCustomerPSI < units::pressure::pounds_per_square_inch_t(0)) {
                                            ...
                                            // bad customer pressure
                                            performance += (((minCustomerPSI - this_minCustomerPSI) / units::pressure::pounds_per_square_inch_t(1)) * units::energy::megawatt_hour_t(5000))(); // 50MWh penalty per min pressure reduction

                                            //if (*bestPerformance > performance) {
                                            //    bestPerformance.Lock();
                                            //    *bestPerformance.UnsafeGet() = performance;
                                            //    bestPerformance.Unlock();

                                            //    BestPolicy.Lock();
                                            //    *BestPolicy.UnsafeGet() = policy;
                                            //    BestPolicy.Unlock();
                                            //}

                                            fileSystem->ensureDirectoryExists(fileSystem->getDataFolder() + "\\OptimizationFiles2");
                                            cweeStr newFp = fileSystem->createFilePath(fileSystem->getDataFolder() + "\\OptimizationFiles2", cweeStr(performance) + "_AT_" + newPolicyStr.ReplaceInline(", ", "_"), fileType_t::INP);
                                            p->saveINP(newFp);
                                        }
                                        else {
                                            //if (*bestPerformance > performance) {
                                    ...
                                    //fileSystem->ensureDirectoryExists(fileSystem->getDataFolder() + "\\OptimizationFiles2");
                                    //cweeStr newFp = fileSystem->createFilePath(fileSystem->getDataFolder() + "\\OptimizationFiles2", perf.TryGetPtr("Performance")->ToString() + "__" + policyStr.ReplaceInline(", ", "_"), fileType_t::INP);
                                    //proj->saveINP(newFp);
                                    ...
                                    cachedResults->Emplace(policyStr.c_str(), PenaltyValue);
                                    cachedResults->Emplace(newPolicyStr.c_str(), PenaltyValue);
                                }
                            ...
                            AUTO isFinishedFunc = [=](sharedObjType& shared, cweeThreadedList<u64>& bestPolicy, u64 bestPerformance) -> bool {
                                ...
                                if (shared.results.Num() <= 10) return false; // guarrantee at least 10 iterations
                                    ...
                                    if (cweeMath::Fabs(perf_new - perf_old) <= cweeMath::Fabs(bestPerformance * eps)) {
                                        shared.numIterationsFailedImprovement++;
                                        if (shared.numIterationsFailedImprovement > (maxIterations / 10)) // 20 iterations processed and we saw no further improvement. Unlikely to see improvement with another 20.                         
                                            return true;
                                        else
                                            return false;
                                    }
                            ...
                            std::map<std::string, chaiscript::Boxed_Value> bv_final;
                            try {
                                cweeSharedPtr<EPAnetProject> p;
                                int index = shared->results.Num() - 1;
                                while (!p) {
                                    auto& bestPolicy = shared->results[index].bestPolicy;

                                    cweeStr policy_Str;
                                    cweeList<::epanet::Plink> newLinks;
                                    p = createEPAnetProject(bestPolicy, newLinks, policy_Str);
                                    ...
                                    if (p) {
                                        ...
                                        AUTO r = evaluate(p, newLinks);

                                        for (auto& x : r) {
                                            bv_final[x.first] = var(cweeUnitValues::unit_value(*x.second));
                                        }

                                        // save result
                                        fileSystem->ensureDirectoryExists(fileSystem->getDataFolder() + "\\OptimizationFiles2");
                                        cweeStr newFp = fileSystem->createFilePath(fileSystem->getDataFolder() + "\\OptimizationFiles2", cweeStr("result"), fileType_t::INP);
                                        p->saveINP(newFp);

                                        bv_final["Project"] = var(std::move(p));

                                        if (control_project) {
                                            cweeList<::epanet::Plink> newLinks2;
                                            AUTO r = evaluate(control_project, newLinks2);
                                            for (auto& x : r) {
                                                bv_final["Control " + x.first] = var(cweeUnitValues::unit_value(*x.second));
                                            }
                                        }
