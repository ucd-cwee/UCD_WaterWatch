...
// #define AllowGlobalDeclOnScopeZero
...
namespace chaiscript {
    ...
    namespace parser {
        ...
        template<typename Tracer, typename Optimizer, std::size_t Parse_Depth = 1024> // doubled the max depth
        class ChaiScript_Parser final : public ChaiScript_Parser_Base {
            ...
            struct Scope_Counter {
                static const auto max_depth = Parse_Depth;
                Scope_Counter(ChaiScript_Parser* t_parser)
                    : parser(t_parser) {
                    ++parser->m_scope_depth;
                    if (parser->m_scope_depth > max_depth) {
                        throw exception::eval_error("Maximum scope depth exceeded",
                            File_Position(parser->m_position.line, parser->m_position.col),
                            *(parser->m_filename));
                    }
                }
                ~Scope_Counter() noexcept { --parser->m_scope_depth; }
                ChaiScript_Parser* parser;
            };
            ...
            struct Operator_Matches {
                ...
                // std::array<utility::Static_String, 1> m_4{ {/*SS("^"), */SS("..")} }; // RG
                ...
                std::array<utility::Static_String, 6> m_11{ {SS("++"), SS("--"), SS("-"), SS("+"), SS("!"), SS("~")} }; // RG
            ...
            constexpr static std::array<Operator_Precedence, 12> create_operators() noexcept {
                std::array<Operator_Precedence, 12> operators = { {Operator_Precedence::Ternary_Cond,
                                                                  ...
                                                                  // Operator_Precedence::Bitwise_Xor, // RG .. was ON
                    ...
                    Operator_Precedence::Bitwise_Xor, // RG .. was off
            ...
            chaiscript::shared_ptr<std::string> m_filename;
            chaiscript::small_vector<eval::AST_Node_Impl_Ptr<Tracer>> m_match_stack;
            chaiscript::small_vector<eval::AST_Node_Impl_Ptr<Tracer>> m_comment_stack;
        ...
        public:
            ...
            explicit ChaiScript_Parser(const chaiscript::detail::Dispatch_Engine& engine, Tracer tracer = Tracer(), Optimizer optimizer = Optimizer())
                : m_tracer(std::move(tracer))
                , m_optimizer(std::move(optimizer))
                , m_engine(&engine)
            {
            ...
            const chaiscript::detail::Dispatch_Engine* get_engine() noexcept { return m_engine; };
            ...
            void build_match(size_t t_match_start, std::string t_text = "") {
                ...
                chaiscript::small_vector<eval::AST_Node_Impl_Ptr<Tracer>> new_children;
...
#endif
            inline auto Symbol_(const std::string_view& sym) noexcept {
                const auto len = sym.size();
                if (m_position.remaining() >= len) {
                    const char* file_pos = &(*m_position);
                    for (size_t pos = 0; pos < len; ++pos) {
            ...
            bool SkipComment() {
                ...
                if (Symbol_(m_multiline_comment_begin)) {                  
                    ...
                    auto parseLoc = Parse_Location(m_filename, start.line, start.col, m_position.line, m_position.col);
                    m_comment_stack.push_back(chaiscript::make_unique<eval::AST_Node_Impl<Tracer>, eval::Noop_AST_Node<Tracer>>(comment.data(), parseLoc));
...
#pragma GCC diagnostic pop
            ...
            chaiscript::unique_ptr<eval::AST_Node_Impl<Tracer>>
...
#pragma warning(disable : 4307)
                    ...
                    case utility::hash("true"): {
                        ...
                        //m_match_stack[m_match_stack.size() - 1]->potentialReturnType = user_type<bool>();
                    } break;
                    ...
                    case utility::hash("Infinity"): {
                        ...
                        //m_match_stack[m_match_stack.size() - 1]->potentialReturnType = user_type<double>();
                    } break;
                    ...
                    case utility::hash("_"): {
                        m_match_stack.push_back(
                            make_node<eval::Constant_AST_Node<Tracer>>(std::move(text),
                                start.line,
                                start.col,
                                Boxed_Value(chaiscript::make_shared<dispatch::Placeholder_Object>())));
                    } break;
                    default: {
                        auto val = text;
                        if (*start == '`') {
...
#pragma warning(pop)
            ...
            bool Try() {
                ...
                if (Keyword("try")) {
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'do' block", File_Position(m_position.line, m_position.col), *m_filename);
                    }
                    SkipWS(true);
                    if (Keyword("finally")) {
                        const auto finally_stack_top = m_match_stack.size();
                    ...
                    build_match<eval::Do_AST_Node<Tracer>>(prev_stack_top);
                }
                return retval;
            }

            /// Reads a control block from input
            bool ControlBlock() {
                Depth_Counter dc{ this };
                Scope_Counter sc{ this };
                bool retval = false;

                const auto prev_stack_top = m_match_stack.size();
                if (Keyword("__LOCK__")) {
                    retval = true;
                    SkipWS(true); // white space
                    if (Char('{')) {
                        Statements();
                        if (!Char('}')) {
                            throw exception::eval_error("Incomplete 'lock' block", File_Position(m_position.line, m_position.col), *m_filename);
                        }
                        ...
                        build_match<eval::ControlBlock_AST_Node<Tracer>>(prev_stack_top);
                    }
                }
                return retval;
            }
            ...
            bool If() {
                ...
                if (Keyword("if")) {
                    ...
                    SkipWS(true);
                    if (!Char('(')) {
                        throw exception::eval_error("Incomplete 'if' expression: cannot find '('", File_Position(m_position.line, m_position.col), *m_filename);
                    }
                    SkipWS(true);
                    if (!Equation()) {
                        throw exception::eval_error("Incomplete 'if' expression: cannot find equation block", File_Position(m_position.line, m_position.col), *m_filename);
                    }
                    SkipWS(true);
                    ...
                    if (!Block()) {
                        //const auto prev_stack_top_test = m_match_stack.size();
                        //if (!Statement()) {
                            throw exception::eval_error("Incomplete 'if' block", File_Position(m_position.line, m_position.col), *m_filename);
                        //}
                        //else if ((m_match_stack.size() - prev_stack_top_test) != 1) {
                        //    throw exception::eval_error("If statement without curly braces cannot be followed by more than one complete statement.", File_Position(m_position.line, m_position.col), *m_filename);
                        //}
                    }
                    ...
                    while (has_matches) {
                        ...
                        if (Keyword("else")) {
                            ...
                            if (If()) {
                                ...
                                SkipWS(true);
                                if (!Block()) {
                                    //const auto prev_stack_top_test2 = m_match_stack.size();
                                    //if (!Statement()) {
                                        throw exception::eval_error("Incomplete 'else' block", File_Position(m_position.line, m_position.col), *m_filename);
                                    //}
                                    //if ((m_match_stack.size() - prev_stack_top_test2) != 1) {
                                    //    throw exception::eval_error("Else statement without curly braces cannot be followed by more than one complete statement.", File_Position(m_position.line, m_position.col), *m_filename);
                                    //}
                                }     
                                has_matches = true;
                            }
            ...
            /*
            Reads a boolean statement of whether an object is of a specific type(including checking for polymorphism)
            var& x = 100; // int
            if (x is int){ ... }
            ...
            */
            ...
            bool TypeCastOperation() {
                ...
                if (Char('(') && TypeName(typeName_r) && Char(')')) { // (string)100
                    if (Id(true) || Value() || Operator() || Paren_Expression()) {
                        retval = true;
                        build_match<eval::Arg_List_AST_Node<Tracer>>(prev_stack_top + 1);                        
                        build_match<eval::Fun_Call_AST_Node<Tracer>>(prev_stack_top); // Id(fun name), Arg_List()
                    }
                }
                if (!retval) {
...
#ifdef AllowGlobalDeclOnScopeZero
                    if (this->m_scope_depth == 0) {
                        retval = true;
                        if (!(Reference() || Id(true))) {
                            throw exception::eval_error("Incomplete implicit global declaration", File_Position(m_position.line, m_position.col), *m_filename);
                        }
            ...
            bool TypeName(std::string_view& typeName_r) {
                static AUTO empty_type = Type_Info();
                Depth_Counter dc{ this };
                const auto prev_stack_top = m_match_stack.size();
                const auto prev_pos = m_position;
                AUTO engine_p = get_engine();
                if (engine_p && Id(false)) {
                    std::string_view text = Position::str(prev_pos, m_position);
                    if (text[text.size() - 1] == '&' && text.size() > 1) {
                    ...
                    chaiscript::Type_Info type_i = engine_p->get_type(typeName_r, false);
                    if (type_i == empty_type) {
                        while (m_match_stack.size() != prev_stack_top) m_match_stack.pop_back();
                        m_position = prev_pos;
            ...
            bool SpecifiedType_Var_Decl() {
                ...
                bool is_global = this->m_scope_depth == 0;
                std::string_view typeName_r;
                if (TypeName(typeName_r)) { // typename was specified and found         
                    build_match<eval::Arg_List_AST_Node<Tracer>>(prev_stack_top+1); // {no_params}
                    build_match<eval::Fun_Call_AST_Node<Tracer>>(prev_stack_top); // collapse all into a function call (i.e. int({no_params}))
#ifdef AllowGlobalDeclOnScopeZero
                    if (is_global) {
                        if (Reference() || Id(true)) {
                            retval = true;
                            build_match<eval::Global_Decl_AST_Node<Tracer>>(prev_stack_top+1);  // global i;                            
                        }
                    } else {
#endif
            ...
            bool Postfix(bool gotValueAlready) {
                ...
                if (gotValueAlready) {
                    if (Symbol("++")) {
                        build_match<eval::Postfix_AST_Node<Tracer>>(prev_stack_top - 1, "++");
                        return true;
                    }
                    else if (Symbol("--")) {
                        build_match<eval::Postfix_AST_Node<Tracer>>(prev_stack_top - 1, "--");
                        ...
                        if (prev_stack_top > 0 && m_match_stack[prev_stack_top - 1]->text != "" && cweeStr(m_match_stack[prev_stack_top - 1]->text.c_str()).IsNumeric()) {
                            for (const auto& possible_match : this->get_engine()->get_sortedpostfixes()) {
                                if (Symbol(possible_match.second.first)) {
                                    build_match<eval::Postfix_AST_Node<Tracer>>(prev_stack_top - 1, possible_match.second.first);
                                    return true;
                                }
                            }
                        }
                    ...
                    if (Id(true)) {
                        if (Symbol("++")) {
                            build_match<eval::Postfix_AST_Node<Tracer>>(prev_stack_top, "++");
                            return true;
                        }
                        else if (Symbol("--")) {
                            build_match<eval::Postfix_AST_Node<Tracer>>(prev_stack_top, "--");
                    ...
                    if (Operator(t_precedence + 1)) {
                        ...
                        while (Operator_Helper(t_precedence, oper)) {
                            ...
                            case (Operator_Precedence::Ternary_Cond):
                                if (oper == "?=") {
                                    build_match<eval::Equation_AST_Node<Tracer>>(prev_stack_top, oper);
                                }
                                else {
                                    if (Symbol(":")) {
                                        if (!Operator(t_precedence + 1)) {
                        ...
                        //if (!retval) {
                        //    // something went wrong -- bad attempt to convert as operator
                        //    while (m_match_stack.size() != prev_stack_top) m_match_stack.pop_back();
                        //    m_position = prev_pos;
                        //}
                    }
            ...
            bool Statements(const bool t_class_allowed = false) {
                ...
                while (has_more) {
                    const auto start = m_position;
                    if (Def() || Try() || If() || While() || Class(t_class_allowed) || For() || Switch() || ControlBlock()) {
                        if (!saw_eol) {
                            throw exception::eval_error("Two function definitions missing line separator",
                                File_Position(start.line, start.col),
                                *m_filename);
            ...
            bool Statement(const bool t_class_allowed = false) {
                Depth_Counter dc{ this };
                bool retval = false;
            ...
            AST_NodePtr parse(const std::string& t_input, const std::string& t_fname, chaiscript::detail::Dispatch_Engine const& engine) override {
                this->m_engine = &engine;
                ChaiScript_Parser<Tracer, Optimizer> parser(engine, m_tracer, m_optimizer);
            ...
            bool TryParseReturnType(eval::AST_Node_Impl_Ptr<Tracer>& node, AST_Node_Type const& nodeType, const chaiscript::detail::Dispatch_Engine* thisEngine, std::map<std::string, ReturnType>* IdsMap = nullptr) {
                ...
                if (node->identifier == nodeType) {
                                ...
                                if (postfixNodePointer->text != "") {
                                    ...
                                    if (ptr) {
                                        ...
                                        if (funcs.second) {
                                            ...
                                            node->potentialReturnType.Deterministic() = true;
                                        }
                                    }
                                    else {
                                        node->potentialReturnType.Deterministic() = true;
                    ...
                    case AST_Node_Type::Dot_Access: {
                        ...
                        if (dotAccessNodePointer) {
                            if (
                                dotAccessNodePointer->children[0]->potentialReturnType.Type() == Type_Info() 
                                ||
                                dotAccessNodePointer->children[0]->potentialReturnType.Type() == user_type<Boxed_Value>()
                                ) {
                                successfulParse = TryParseReturnType(dotAccessNodePointer->children[0], dotAccessNodePointer->children[0]->identifier, thisEngine, IdsMap); // try parse immediately
                            }
                            std::pair<size_t, chaiscript::shared_ptr<chaiscript::small_vector<Proxy_Function>>> funcs = thisEngine->get_function(dotAccessNodePointer->m_fun_name, 0);
                            if (funcs.second) {
                                // Type_Conversions_State state(thisEngine->conversions(), thisEngine->conversions().conversion_saves());

                                std::multimap<int, Proxy_Function> ordered_funcs;                                
                                for (auto& func : *funcs.second) {
                                    const auto arity = func->get_arity();

                                    if (arity == 0) continue; // could not be used as the function in a dot access. (By definition there will be one input);                                    
                                    else if (arity == -1) { ordered_funcs.insert({ 32, func }); /* unknown, so put it in the back. */ }
                                    else { // 1 <= arity < INF    
                                        const chaiscript::small_vector<Type_Info>& params = func->get_param_types();
                                        if (dotAccessNodePointer->children[0]->potentialReturnType.Type().bare_equal(params[1])) {
                                            ordered_funcs.insert({ 0, func });
                                        }
                                        else {
                                            ordered_funcs.insert({ 1, func });
                                        }
                                    }
                                }

                                for (auto& func : ordered_funcs) {
                                    //if (func->compare_first_type_to_type(dotAccessNodePointer->children[0]->potentialReturnType.Type(), state)) {
                                    //    // can convert
                                    //    ordered_funcs.insert({ 1, func });
                                    //}
                                    //else {
                                    ...
                                    //    continue;
                                    //}
                                    const chaiscript::small_vector<Type_Info>& params = func.second->get_param_types();
                                    if (params.size() > 0) {
                                        // node->potentialReturnType = ReturnType(params[0], node->identifier, true);
                                        //node->children[1]->potentialReturnType.ForwardRef(node->potentialReturnType); // = ReturnType(params[0], node->identifier, true);
                                                                                
                                        node->children[1]->potentialReturnType.Set(ReturnType(params[0], node->identifier, true)); //node->potentialReturnType;
                                        if (node->children[1]->children.size() > 0) {
                                            node->children[1]->children[0]->potentialReturnType.Set(ReturnType(params[0], node->identifier, true));
                                        }
                    ...
                    case AST_Node_Type::Id: {
                        ...
                        if (IdsMap && chaiscript::eval::TryGetVariableName(*node, varName)) {
                            if (IdsMap->count(varName) <= 0) {
                                // we never saw this variable before
                                ...
                                // we have seen this variable before -- BUT has the variable changed definition? 
                                if (node->potentialReturnType.Type().is_undef()) {
                                    node->potentialReturnType.Set(IdsMap->operator[](varName));
                                    for (auto& nodes : node->children) {
                                        nodes->potentialReturnType.Set(IdsMap->operator[](varName));
                                    }
                                }
                                else {
                                    // new definition! Accept it?
                                    IdsMap->erase(varName);
                                    IdsMap->operator[](varName) = node->potentialReturnType;
                                }
                    ...
                    case AST_Node_Type::Array_Call: {
                        chaiscript::eval::Array_Call_AST_Node<Tracer>* arrayAccessNodePointer = dynamic_cast<chaiscript::eval::Array_Call_AST_Node<Tracer>*>(node.get());
                        if (arrayAccessNodePointer) {
                            auto& containerTypes = thisEngine->get_containerValueTypes();

                            ReturnType& potRetType = arrayAccessNodePointer->children[0]->potentialReturnType;
                            if (potRetType.Type().is_container_type()) {
                                AUTO name = std::string(potRetType.Type().name());
                                AUTO def = containerTypes.TryGetPtr(name);
                                if (def) {
                                    arrayAccessNodePointer->potentialReturnType.Set(ReturnType(def->value_type_m, node->identifier, true));
                                    successfulParse = true;
                                }
                            }
                        }
                ...
                for (auto& nodes : node->children) {
                    bool t = successfulParse;
                    successfulParse = TryParseReturnType(nodes, nodeType, thisEngine, IdsMap);
                    successfulParse = successfulParse || t;
                }
            ...
            AST_NodePtr parse_internal(const std::string& t_input, std::string t_fname) {
                ...
                m_filename = chaiscript::make_shared<std::string>(std::move(t_fname));
                ...
                chaiscript::eval::AST_Node_Impl_Ptr<Tracer>& elem = m_match_stack.front();

                chaiscript::unique_ptr<chaiscript::AST_Node> retval = chaiscript::dynamic_unique_ptr_cast<chaiscript::AST_Node>(std::move(m_match_stack.front()));
