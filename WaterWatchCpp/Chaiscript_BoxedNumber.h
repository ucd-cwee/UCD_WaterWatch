    ...
    private:
        ...
        static auto go(Operators::Opers t_oper, const Boxed_Value& t_bv, LHS* t_lhs, const LHS& c_lhs, const RHS& c_rhs) {
            ...
            case Operators::Opers::logical_list: { // << RG           
                ...
                int num = 1;
                for (decltype(type) x = c_lhs; x < c_rhs; ++x)  ++num;
                for (decltype(type) x = c_lhs; x > c_rhs; --x)  ++num;

                chaiscript::shared_ptr<chaiscript::small_vector<chaiscript::Boxed_Value>> out = chaiscript::make_shared<chaiscript::small_vector<chaiscript::Boxed_Value>>();
                out->reserve(num);

                num = 0;
                for (decltype(type) x = c_lhs; x < c_rhs; ++x, num++) {
                    out->emplace_back(x);
                    // out->at(num) = chaiscript::Boxed_Value(x);
                    // num++;
                }
                for (decltype(type) x = c_lhs; x > c_rhs; --x, num++) {
                    out->emplace_back(x); 
                ...
                out->emplace_back((decltype(type))c_rhs);
                // out->at(num) = chaiscript::Boxed_Value((decltype(type))c_rhs);

                return const_var(out);
            }
            default:
                break;
            }
            ...
            if constexpr (!std::is_floating_point<LHS>::value && !std::is_floating_point<RHS>::value) {
                ...
                case Operators::Opers::logical_list: { // << RG
                    ...
                    num = 0;
                    for (decltype(type) x = c_lhs; x < c_rhs; ++x) {
                        out->at(num) = chaiscript::Boxed_Value(x);
                        num++;
                    }
                    for (decltype(type) x = c_lhs; x > c_rhs; --x) {
...
#pragma warning(pop)
        ...
        std::string ltrim(const std::string& s, const char trim) const
        {
            size_t start = s.find_first_not_of(trim);
            return (start == std::string::npos) ? "" : s.substr(start);
        }

        std::string rtrim(const std::string& s, const char trim) const
        {
            size_t end = s.find_last_not_of(trim);
            return (end == std::string::npos) ? "" : s.substr(0, end + 1);
        }

        std::string get_pretty_string(const std::string& a) const {
        ...
        std::string to_string() const {
            ...
            case Common_Types::t_double:
                return get_pretty_string(std::to_string(get_as<double>())); // return to_string_aux<double>(bv);
            case Common_Types::t_float:
                return get_pretty_string(std::to_string(get_as<float>())); //return to_string_aux<float>(bv);
            case Common_Types::t_long_double:
                return get_pretty_string(std::to_string(get_as<long double>())); // return to_string_aux<long double>(bv);
            }
        ...
        static Boxed_Number assign_if_null(Boxed_Number t_lhs, const Boxed_Number& t_rhs) {
            return Boxed_Number(oper(Operators::Opers::assign_if_null, t_lhs.bv, t_rhs.bv));
        }
        ...
        static Boxed_Number logical_list(Boxed_Number t_lhs, const Boxed_Number& t_rhs) {
            return Boxed_Number(oper(Operators::Opers::logical_list, t_lhs.bv, t_rhs.bv));
        }
