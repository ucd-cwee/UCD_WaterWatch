...
#pragma once
#include "../WaterWatchCpp/include/sqlite3.h"
#include "SQLITE.h"
#include "InterlockedValues.h"
#include "cwee_math.h"

INLINE int callbackMIN(void* minCol, int argc, char** argv, char** azColName) {
	time_t* c = (time_t*)minCol;
	*c = atof(argv[0]);
	return 0;
};
INLINE int callback1(void* count, int argc, char** argv, char** azColName) {
	int* c = (int*)count;
	*c = atoi(argv[0]);
	return 0;
};
INLINE int callback2(void* columns, int argc, char** argv, char** azColName) {
	int* c = (int*)columns;
	*c = argc;
	return 0;
};
INLINE int callback3(void* out, int argc, char** argv, char** azColName) {
	char** c = (char**)out;
	c = argv;
	return 0;
};
INLINE void* SQLite_Open(const char* value) noexcept
{
	sqlite3* DB;
	if (sqlite3_open(value, &DB)) sqlite3_close(DB);
	return DB;
};
INLINE void SQLite_Close(void* value) noexcept
{
	sqlite3_close((sqlite3*)value);
};
INLINE void SQLite_Exec(void* value, const char* sql, int (*callback)(void*, int, char**, char**), void* returnArg, char** errmsg) noexcept
{
	sqlite3_exec((sqlite3*)value, sql, callback, returnArg, errmsg);
};
INLINE void SQLite_Prepare(void* value, const char* zSql, int nBytes, void* ppStmt, const char** pzTail) noexcept
{
	sqlite3_prepare((sqlite3*)value, zSql, nBytes, (sqlite3_stmt**)ppStmt, pzTail);
};
INLINE int SQLite_Step(void* pStmt) noexcept
{
	return sqlite3_step((sqlite3_stmt*)pStmt);
};
INLINE const unsigned char* SQLite_Column_Text(void* pStmt, int i) noexcept
{
	return sqlite3_column_text((sqlite3_stmt*)pStmt, i);
};
INLINE int SQLite_Column_Count(void* pStmt) noexcept
{
	return sqlite3_column_count((sqlite3_stmt*)pStmt);
};
INLINE void SQLite_Finalize(void* pStmt) noexcept
{
	sqlite3_finalize((sqlite3_stmt*)pStmt);
};
INLINE void SQLite_GetTable(void* value, const char* sql, char*** pazResult, int* pnRow, int* pnColumn, char** pzErrMsg) noexcept
{
	sqlite3_get_table((sqlite3*)value, sql, pazResult, pnRow, pnColumn, pzErrMsg);
};
INLINE const char* SQLite_GetColumnName(void* pStmt, int N) noexcept
{
...
int SQLite::openDbDirect(cweeStr fullPath) noexcept {
	sqlite3* DB;
	DB = (sqlite3*)SQLite_Open(fullPath.c_str());
...
int SQLite::openDB(cweeStr folderPath, cweeStr fileName) noexcept {
	...
	sqlite3* DB;
	DB = (sqlite3*)SQLite_Open(filePath.c_str());
	auto temp = SQLite_numSQLiteAccess.Increment();
	SQLite_databaseMap.insert(std::make_pair(temp, DB));
	return temp;
}
