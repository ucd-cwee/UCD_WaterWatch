...
namespace chaiscript {
    ...
    public:
        ...
        explicit Boxed_Value(T&& t, bool t_return_value = false) : m_data(Object_Data::get(std::forward<T>(t), t_return_value)) {}

        /// Basic Boxed_Value copy constructor
        template<typename T, typename = std::enable_if_t<!std::is_same_v<Boxed_Value, std::decay_t<T>>>>
        explicit Boxed_Value(const T& t, bool t_return_value = false) : m_data(Object_Data::get(t, t_return_value)) {}
        ...
        /// \endcode
        template<typename T, typename = std::enable_if_t<!std::is_same_v<Boxed_Value, std::decay_t<T>>>>
        explicit operator T() const { // recommended to only do this when the type is explicitely known
            try {
                return chaiscript::boxed_cast<T>(*this, nullptr);
            } catch (...) {}
            return T();
        }
    ...
    private:
        ...
        Boxed_Value(chaiscript::shared_ptr<Data> t_data, Internal_Construction) : m_data(std::move(t_data)) {}
    ...
    /// \sa \ref adding_objects
    template<typename T>
    Boxed_Value const_var(const T& t) { return detail::const_var_impl(t); }

    inline Boxed_Value void_var() { static const auto v = Boxed_Value(Boxed_Value::Void_Type()); return v; }
