...
/*
TO-DO; Add encryptio and unencryption leveraging password protection
*/
...
#define UNZ_CRCERROR                    (-105)
	...
	extern int ZEXPORT unzGetNumFiles OF((unzFile file));


	//extern int unzSetCurrentFileInfoPosition(unzFile file, unsigned long pos);
	extern int ZEXPORT unzSetCurrentFileInfoPosition OF((unzFile file, unsigned long pos));
...
const unsigned int ziblib_BUFSIZE = 2048;
...
public:
	...
	zipper& operator<<(std::string& is) {
		...
		if (isOpenEntry())
		{
			for (i = 0; (i < (is.size() + ziblib_BUFSIZE - 1)) && (err == ZIP_OK); i += (ziblib_BUFSIZE - 1)) {
				j = i + ziblib_BUFSIZE;
				if (j > is.size()) j = is.size();
				{
					l = j - i;
					start = i;
					k = is.size();
...
private:
	zipFile			zipFile_ = nullptr; // was nothing, not specified
	bool			entryOpen_ = false; // was nothing, not specified
};
...
public:
	...
	unsigned int getEntrySize() {
		if (entryOpen_)
		{
			unz_file_info64 oFileInfo64;
			unz_file_info oFileInfo;
			int err = unzGetCurrentFileInfo64(zipFile_, &oFileInfo64, 0, 0, 0, 0, 0, 0);
			err = unzGetCurrentFileInfo(zipFile_, &oFileInfo, 0, 0, 0, 0, 0, 0);
			if (err == UNZ_OK)
			{
				auto sz = (unsigned int)oFileInfo.uncompressed_size;
				auto sz_64 = (unsigned int)oFileInfo64.uncompressed_size;
...
private:
	unzFile			zipFile_ = nullptr; // was nothing
	bool			entryOpen_ = false; // was nothing
