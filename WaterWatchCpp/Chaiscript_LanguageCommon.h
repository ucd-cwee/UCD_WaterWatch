...
namespace chaiscript {
    ...
    struct Parse_Location {
        Parse_Location(std::string t_fname = "", const int t_start_line = 0, const int t_start_col = 0, const int t_end_line = 0, const int t_end_col = 0)
            ...
            , filename(chaiscript::make_shared<std::string>(std::move(t_fname))) {
        }

        Parse_Location(chaiscript::shared_ptr<std::string> t_fname,
        ...
        File_Position start;
        File_Position end;
        chaiscript::shared_ptr<std::string> filename;
    };

    /// \brief Typedef for pointers to AST_Node objects. Used in building of the AST_Node tree
    using AST_NodePtr = chaiscript::unique_ptr<AST_Node>;
    using AST_NodePtr_Const = chaiscript::unique_ptr<const AST_Node>;
    ...
    namespace exception {
        ...
        struct load_module_error : std::runtime_error {
            ...
            load_module_error(const std::string& t_name, const chaiscript::small_vector<load_module_error>& t_errors)
            ...
            static std::string format_error(const std::string& t_name, const chaiscript::small_vector<load_module_error>& t_errors) {
                std::stringstream ss;
        ...
        struct eval_error : std::runtime_error {
            ...
            std::string filename;
            std::string detail;
            chaiscript::small_vector<AST_Node_Trace> call_stack;

            eval_error(const std::string& t_why,
                const File_Position& t_where,
                const std::string& t_fname,
                const chaiscript::small_vector<Boxed_Value>& t_parameters,
                const chaiscript::small_vector<chaiscript::Const_Proxy_Function>& t_functions,
                bool t_dot_notation,
            ...
            explicit eval_error(const std::string& t_why) noexcept
                : std::runtime_error(t_why) // std::runtime_error("Error: \"" + t_why + "\" ")
                , reason(t_why) {
            }
    ...
    public:
        ...
        virtual ~ReturnType_Base() {};

        virtual Type_Info& Type() const noexcept = 0;
        virtual AST_Node_Type& Source() const noexcept = 0;
        virtual bool& Deterministic() const noexcept = 0;

        virtual cweeSharedPtr<ReturnType_Base> Next() const { return nullptr; };

        virtual bool Find(void* p) noexcept = 0;
        virtual void* ThisPtr() noexcept = 0;
        virtual bool SetFinalPtr(cweeSharedPtr<ReturnType_Base> newPtr) noexcept = 0;
    };
    class ReturnType_Impl final : public ReturnType_Base {
    public:
        ReturnType_Impl() noexcept : ReturnType_Base(), deterministic(false), type(), source(AST_Node_Type::Noop) {};
        ReturnType_Impl(Type_Info const& p_type, AST_Node_Type const& p_source, bool p_deterministic) noexcept : ReturnType_Base(), deterministic(p_deterministic), type(p_type), source(p_source) {};
        virtual ~ReturnType_Impl() {};

        mutable bool            deterministic; // set to finished to prevent this from changing
        mutable Type_Info       type;
        mutable AST_Node_Type   source;

        virtual Type_Info& Type() const noexcept {
            return type;
        };
        virtual AST_Node_Type& Source() const noexcept {
            return source;
        };
        virtual bool& Deterministic() const noexcept {
            return deterministic;
        };

        cweeSharedPtr<ReturnType_Base> Next() const override { return nullptr; };

        bool Find(void* p) noexcept override {
            return p == ThisPtr();
        };
        void* ThisPtr() noexcept override { return (void*)this; };
        bool SetFinalPtr(cweeSharedPtr<ReturnType_Base> newPtr) noexcept override { return false; };
    };
    class ReturnType final : public ReturnType_Base {
    public:
        mutable cweeSharedPtr<ReturnType_Base> reference;

    private:
        cweeSharedPtr<ReturnType_Base> Next() const override { return reference; };

    public:
        static cweeSharedPtr<ReturnType_Base> MakePtr(Type_Info const& type, AST_Node_Type const& source, bool deterministic) {
            AUTO ptr1 = make_cwee_shared<ReturnType_Impl>(type, source, deterministic);
            return ptr1.CastReference<ReturnType_Base>();
        };
        static cweeSharedPtr<ReturnType_Base> MakePtr() {
            AUTO ptr1 = make_cwee_shared<ReturnType_Impl>();
            return ptr1.CastReference<ReturnType_Base>();
        };
        ReturnType(Type_Info const& type, AST_Node_Type const& source, bool deterministic) noexcept : ReturnType_Base(), reference(MakePtr(type, source, deterministic)) {};
        ReturnType() noexcept : ReturnType_Base(), reference(MakePtr()) {};
        ReturnType(ReturnType const& other) noexcept : ReturnType_Base(), reference(other.reference) {};
        virtual ~ReturnType() {};

        bool Find(void* p) noexcept override {
            if (p == ThisPtr()) return true;
            return reference->Find(p);
        };
        void* ThisPtr() noexcept override { return (void*)this; };
        bool SetFinalPtr(cweeSharedPtr<ReturnType_Base> newPtr) noexcept override {
            if (Find(newPtr->ThisPtr()) || newPtr->Find(ThisPtr())) { 
                return false; 
            }
            if (reference == newPtr) {
                return false;
            }
            if (reference->SetFinalPtr(newPtr)) {
                return true;
            }
            else {
                if (reference->Next()) {
                    return reference->Next()->SetFinalPtr(newPtr);
                }
                else {
                    // change my next ptr to be this one...
                    reference = newPtr;
                    return true;
                }
            }
    ...
    public:
        ...
        AST_Node_Type& Source() const noexcept override {
        ...
        ReturnType& ForwardRef(ReturnType const& other) {
            if (!Deterministic()) {
                cweeSharedPtr<ReturnType> new_ptr = make_cwee_shared<ReturnType>(other);
                SetFinalPtr(new_ptr.CastReference<ReturnType_Base>());
            }
            return *this;
        };
        ...
        ReturnType& Set(ReturnType const& other) noexcept {
            cweeSharedPtr<ReturnType> new_ptr = make_cwee_shared<ReturnType>(other);
            SetFinalPtr(new_ptr.CastReference<ReturnType_Base>());
            return *this;
        };
    ...
    public:
        ...
        virtual chaiscript::small_vector<std::reference_wrapper<AST_Node>> get_children() const = 0;
    ...
    //inline chaiscript::small_vector<std::reference_wrapper<AST_Node>> AST_Node::get_children() const {
    //    throw(std::exception("Do not call this on the basic AST_Node"));
    //};
    //inline Boxed_Value AST_Node::eval(const chaiscript::detail::Dispatch_State& t_e) const {
    //    throw(std::exception("Do not call this on the basic AST_Node"));
    //};
    ...
    struct AST_Node_Trace {
        ...
        chaiscript::small_vector<AST_Node_Trace> get_children(const AST_Node& node) {
            const auto node_children = node.get_children();
            return chaiscript::small_vector<AST_Node_Trace>(node_children.begin(), node_children.end());
        }
        ...
        chaiscript::small_vector<AST_Node_Trace> children;
    };

    namespace parser {
        class ChaiScript_Parser_Base {
        public:
            virtual AST_NodePtr parse(const std::string& t_input, const std::string& t_fname, chaiscript::detail::Dispatch_Engine const& engine) = 0;
