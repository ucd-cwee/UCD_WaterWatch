...
namespace chaiscript::bootstrap {
    ...
    auto shared_ptr_clone(const chaiscript::shared_ptr<Type>& p) {
        return p;
    }

    /// Specific version of shared_ptr_clone just for Proxy_Functions
    template<typename Type>
    chaiscript::shared_ptr<std::remove_const_t<Type>> shared_ptr_unconst_clone(const chaiscript::shared_ptr<std::add_const_t<Type>>& p) {
        return chaiscript::const_shared_ptr_cast<typename std::remove_const<Type>::type>(p);
    }
    ...
    template<typename Type>
    Boxed_Value ptr_assign(Boxed_Value lhs, const chaiscript::shared_ptr<Type>& rhs) {
    ...
    private:
        ...
        static void opers_arithmetic_pod(Module& m) {
            ...
            m.add(fun(&Boxed_Number::logical_list), "..");
        }
        ...
        static Boxed_Value bind_function(const Function_Params& params) {
            ...
            chaiscript::shared_ptr<dispatch::Bound_Function> shr_ptr = chaiscript::make_shared<dispatch::Bound_Function>(std::move(f), std::vector<Boxed_Value>(params.begin() + 1, params.end()));
            return Boxed_Value(Const_Proxy_Function(shr_ptr));
        }

        static bool has_guard(const Const_Proxy_Function& t_pf) noexcept {
            auto pf = chaiscript::dynamic_shared_ptr_cast<const dispatch::Dynamic_Proxy_Function>(t_pf);
            return pf && pf->has_guard();
        }

        static Const_Proxy_Function get_guard(const Const_Proxy_Function& t_pf) {
            const auto pf = chaiscript::dynamic_shared_ptr_cast<const dispatch::Dynamic_Proxy_Function>(t_pf);
            if (pf && pf->get_guard()) {
                chaiscript::Proxy_Function pf_actual = pf->get_guard();
                return pf_actual;
            }
            else {
        ...
        static chaiscript::small_vector<Boxed_Value> do_return_boxed_value_vector(FunctionType f, const dispatch::Proxy_Function_Base* b) {
            auto v = (b->*f)();

            chaiscript::small_vector<Boxed_Value> vbv;
    ...
    public:
        ...
        static void bootstrap(Module& m) {
            ...
            //m.add(fun([](chaiscript::Const_Proxy_Function cpf) {
            //   
            //}), "get_name");
            m.add(fun(&dispatch::Proxy_Function_Base::get_description), "get_description");
            m.add(fun(&dispatch::Proxy_Function_Base::set_description), "set_description");
            ...
            m.add(fun(&dispatch::Dynamic_Object::remove_attr), "remove_attr");
          ...
          def `==`(Dynamic_Object lhs, Dynamic_Object rhs) : lhs.get_type_name() == rhs.get_type_name()
          {
            ...
            // m.add(fun([](const Type_Info& s) { return std::string(s.name()); }), "to_string");
            m.add(fun([](const Proxy_Function& s) { return s->get_description(); }), "to_string");
            ...
            m.add(fun([](dispatch::Assignable_Proxy_Function& t_lhs, const chaiscript::shared_ptr<const dispatch::Proxy_Function_Base>& t_rhs) {
                t_lhs.assign(t_rhs);
                }),
                "=");
            ...
            //        chaiscript::bootstrap::standard_library::vector_type<chaiscript::small_vector<chaiscript::shared_ptr<chaiscript::AST_Node> >
            //        >("AST_NodeVector", m);
                ...
                { constructor<File_Position()>(), constructor<File_Position(int, int)>() },
                                                                {
                                                                    ...
                                                                    {fun([](const File_Position& line) {
                                                                        return Boxed_Value(std::string(cweeStr::ToString(line.column).c_str()));
                                                                    }), "to_string"},
