...
namespace epanet {
    namespace epanet_postcompiled {
            ...
            */
        {
            ...
            {
            case SCRATCH:
                if (pr->TmpHydFname.Length() > 0) fileSystem->removeFile(pr->TmpHydFname);
                pr->TmpHydFname = fileSystem->createRandomFile(fileType_t::TXT);
            ...
            */
        {
            ...
            if (pr->TmpOutFname.Length() > 0) fileSystem->removeFile(pr->TmpOutFname);
            pr->TmpOutFname = fileSystem->createRandomFile(fileType_t::TXT);
            ...
            if (!errcode)
            {
                ...
                {
                    if (pr->TmpStatFname.Length() > 0) fileSystem->removeFile(pr->TmpStatFname);
                    pr->TmpStatFname = fileSystem->createRandomFile(fileType_t::TXT);
        ...
        bool    combineZones(EN_Project const& pr, 
                    ...
                    if (sect >= 0)
                    {
                        if (targetedSection == sect) {
                            ...
                            if (inperr >= 100)
                            {
                                ...
                                // announce the warning but don't crash
                                inperrmsg(pr, inperr, sect, line); 
                            }
                        }
                    }
            ...
            */
        {
            ...
            // check for additional parms
            int m = 7;
            while (m < n) // 8&9, 10&11, 12&13
            {
                SCALER y;
                if (match(parser->Tok[m], w_PAT)) // Pump As Turbine
                {
                    y = atof(parser->Tok[m+1]);
                    producesElectricity = y != SCALER(0);
                }
                m = m + 2;  // Move to next keyword token
            }
            ...
            valve->ProducesElectricity = producesElectricity;
            ...
            */
        {
            ...
            if ((j = epanet_shared::findnode(net, parser->Tok[0])) == 0) {
                // failed to find the node. This is not great but should not cause the model to crash.
                return setError(parser, 0, 7);
            }
            ...
            //-----------------------------------------------------------
        {
            ...
            net->Nrules--;
            net->Rule.AssureSize(net->Rule.size() - 1);
        };
