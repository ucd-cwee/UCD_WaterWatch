...
void*			operator new(size_t s, memTag_t tag);		// RG
void			operator delete(void* p, memTag_t tag);		// RG
void*			operator new[](size_t s, memTag_t tag);		// RG
void			operator delete[](void* p, memTag_t tag);	// RG
...
#pragma region INTENDED PUBLIC FUCNTIONS AND USES
	/* ALLOC THE UNIONED DATA IN STACK */ cweeUnion() noexcept : data{ 0 } { Alloc(&data[0]); };
	/* ALLOC THE UNIONED DATA FROM PARAMETERS */
	template <typename T, typename = std::enable_if_t<!std::is_same_v<typename std::remove_reference<T>::type, cweeUnion>>, typename... TArgs>
	explicit cweeUnion(T&& b, TArgs&&... a) noexcept : data{ 0 } {
		InitAndSet(std::forward<T>(b), std::forward<TArgs>(a)...);
	};

	/* INIT AND COPY THE UNIONED DATA FROM ANOTHER UNION */ cweeUnion(cweeUnion const& a) noexcept : data{ 0 } { InitAndCopy(a); };
	/* INIT AND TAKE THE UNIONED DATA FROM ANOTHER UNION */ cweeUnion(cweeUnion&& a) noexcept : data{ 0 } { InitAndTake(std::forward<cweeUnion>(a)); };
	/* COPY THE UNIONED DATA FROM ANOTHER UNION */ cweeUnion& operator=(cweeUnion const& a) { Copy(a); return *this; };
	/* TAKE THE UNIONED DATA FROM ANOTHER UNION */ cweeUnion& operator=(cweeUnion&& a) { Take(std::forward<cweeUnion>(a)); return *this; };
	/* DESTROY THE UNION ONCE OUT-OF-SCOPE */ ~cweeUnion() { Delete(); };

	/* GET A REFERENCE TO THE N'th ITEM */ template<int N> constexpr NthTypeOf<N>& get() const noexcept {
		static_assert(N <= num_parameters && N >= 0, "Cannot access parameters beyond the allocated buffer.");
		return *static_cast<NthTypeOf<N>*>(static_cast<void*>((static_cast<byte*>(&const_cast<byte&>(data[SizeOfFirstN<N>()])))));
	};
	/* GET THE SIZE (in bytes) OF THE ENTIRE UNION */ static constexpr size_t size() noexcept { return SizeOfAll(); };
	/* GET THE SIZE (in bytes) OF THE N'th ITEM */ template<int N> static constexpr size_t size() noexcept { return sizeof(NthTypeOf<N>); };
#pragma endregion
private:
#pragma region DATA ARRAY (BYTES)
	byte data[sizeOfArgs];
#pragma endregion
private:
...
public:
	INLINE				cweeBlockAlloc(bool clear = false);
	INLINE				~cweeBlockAlloc();
...
INLINE cweeBlockAlloc<_type_, _blockSize_, memTag>::cweeBlockAlloc(bool clear) :
	blocks(NULL),
	free(NULL),
	total(0),
	active(0),
...
template<class _type_, int _blockSize_, memTag_t memTag>
INLINE cweeBlockAlloc<_type_, _blockSize_, memTag>::~cweeBlockAlloc() {
	Shutdown();
}

template<class _type_, int _blockSize_, memTag_t memTag>
INLINE _type_* cweeBlockAlloc<_type_, _blockSize_, memTag>::Alloc() {
...
template<class _type_, int _blockSize_, memTag_t memTag>
INLINE void cweeBlockAlloc<_type_, _blockSize_, memTag>::Free(_type_* t) {
#ifdef FORCE_DISCRETE_BLOCK_ALLOCS
	// for debugging tools
	delete t;
#else
...
template<class _type_, int _blockSize_, memTag_t memTag>
INLINE void	cweeBlockAlloc<_type_, _blockSize_, memTag>::Reserve(int num) {
#ifdef FORCE_DISCRETE_BLOCK_ALLOCS
	// for debugging tools	
#else
	num -= total;
...
template<class _type_, int _blockSize_, memTag_t memTag>
INLINE void cweeBlockAlloc<_type_, _blockSize_, memTag>::Shutdown() {
	while (blocks != NULL) {
...
template<class _type_, int _blockSize_, memTag_t memTag>
INLINE void cweeBlockAlloc<_type_, _blockSize_, memTag>::SetFixedBlocks(int numBlocks) {
...
template<class _type_, int _blockSize_, memTag_t memTag>
INLINE void cweeBlockAlloc<_type_, _blockSize_, memTag>::AllocNewBlock() {
...
type* cweeDynamicAlloc<type, baseBlockSize, minBlockSize>::Alloc(const int num) {
	...
	return Mem_Alloc16((size_t)(num * sizeof(type)), TAG_BLOCKALLOC);
}
