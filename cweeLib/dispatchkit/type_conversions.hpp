...
namespace chaiscript {
  ...
  public:
    ...
    std::set<chaiscript::shared_ptr<detail::Type_Conversion_Base>> get_conversions() const {
      AUTO shared_guard = SHARED_GUARD();
  ...
  Type_Conversion type_conversion(const Callable &t_function) {
    auto func = [t_function](const Boxed_Value &t_bv) -> Boxed_Value {
      ...
      Type_Conversions_State state(
          GetChai().get_eval_engine().conversions(), 
          GetChai().get_eval_engine().conversions().conversion_saves());
  ...
  Type_Conversion type_conversion() {
    ...
    auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
      ...
      // not even attempting to call boxed_cast so that we don't get caught in some call recursion
      return chaiscript::Boxed_Value(To(detail::Cast_Helper<From>::cast(t_bv, &state))); // nullptr
    };
  ...
  Type_Conversion vector_conversion() {
    auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
      ...
      const chaiscript::small_vector<Boxed_Value> &from_vec = detail::Cast_Helper<const chaiscript::small_vector<Boxed_Value> &>::cast(t_bv, &state); // nullptr

      To vec;
      vec.reserve(from_vec.size());
      for (const Boxed_Value &bv : from_vec) {
        vec.push_back(detail::Cast_Helper<typename To::value_type>::cast(bv, &state)); // nullptr
    ...
    return chaiscript::make_shared<detail::Type_Conversion_Base, detail::Type_Conversion_Impl<decltype(func)>>(user_type<chaiscript::small_vector<Boxed_Value>>(),
                                                                                                               user_type<To>(),
                                                                                                               func);
  }
  ...
  Type_Conversion vector_conversion() {
      auto func = [](const Boxed_Value& t_bv) -> Boxed_Value {          
          ...
          const From& from_vec = detail::Cast_Helper<const From&>::cast(t_bv, &state); // nullptr
  ...
  Type_Conversion map_conversion() {
    auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
      ...
      const std::map<std::string, Boxed_Value> &from_map
          = detail::Cast_Helper<const std::map<std::string, Boxed_Value> &>::cast(t_bv, &state); // nullptr

      To map;
      for (const std::pair<const std::string, Boxed_Value> &p : from_map) {
        map.insert(std::make_pair(p.first, detail::Cast_Helper<typename To::mapped_type>::cast(p.second, &state))); // nullptr
      }
