...
public:
	...
	virtual type		GetAvgValue(const u64& start, const u64& end) const {
		...
		for (int i = IndexForTime(start); i < GetNumValues(); i++) {
			if (TimeForIndex(i) >= start && TimeForIndex(i) <= end) {
				cweeMath::rollingAverageRef(out, (float)ValueForIndex(i), num);
			}
			else {
	...
	virtual u64			GetAvgTime() const {
		...
		Lock();
		for (auto& time : times) {
			cweeMath::rollingAverageRef(out, (float)time, num);
		}
		Unlock();
		return (u64)out;
	};
...
public:
	...
	*/
	...
	virtual void		Clear() {
		...
		this->Lock();
		machineLearn_min = -cweeMath::INF;
		machineLearn_max = cweeMath::INF;
		machineLearn_roundNearest = 0.001;
		machineLearn_MAD = vec2(1, 0);
		machineLearn_features.Clear(); 
		exclusion_list.Clear(); 
		if (learned) delete learned; learned = nullptr; // learned = cweeML_learned_parameters();				
		this->Unlock();
	}
	...
	*/
	cweeStr												Serialize(int percentToRemove = -1)  const {
		...
		this->Lock(); {
			...
			vec3 t = vec3((float)machineLearn_roundNearest, (float)machineLearn_min, (float)machineLearn_max); out.AddToDelimiter(t.ToString(), delim); // 7
		} this->Unlock();
		...
		this->Lock(); {
			...
			out.AddToDelimiter(cweeStr(machineLearn_MAD), delim); // 11
	...
	void												Deserialize(cweeStr& in) {
		...
		if (obj.getNumVars() >= 19) {
			...
			this->Lock(); {
				...
				machineLearn_roundNearest = (float)obj[12];
				machineLearn_min = (float)obj[13];
				machineLearn_max = (float)obj[14];
			} this->Unlock();
			...
			this->Lock(); {
				...
				{
					vec3 t; t.FromString(obj[7]); machineLearn_roundNearest = (float)t[0]; machineLearn_min = (float)t[1]; machineLearn_max = (float)t[2];
				}
			} this->Unlock();
	...
	*/
	virtual void										Copy(const cweePattern_CatmullRomSpline<type>& copy, const u64& timeStart = -std::numeric_limits < u64>::max(), const u64& timeEnd = std::numeric_limits < u64>::max()) {
		...
		machineLearn_roundNearest = copy.machineLearn_roundNearest;
		machineLearn_MAD = copy.machineLearn_MAD;
		machineLearn_min = copy.machineLearn_min;
		machineLearn_max = copy.machineLearn_max;
...
//#pragma loop(hint_parallel(8))
	...
	*/
	...
	virtual void										SwapLearner(const cweePattern_CatmullRomSpline<type>& copy) {
		this->Lock(); copy.Lock();
		machineLearn_min = copy.machineLearn_min;
		machineLearn_max = copy.machineLearn_max;
		machineLearn_roundNearest = copy.machineLearn_roundNearest;
	...
	*/
	...
	virtual vec2										Learn_Automated(const cweeThreadedList<std::pair<cweeStr, vec2>> FeaturePatterns = cweeThreadedList<std::pair<cweeStr, vec2>>(), const cweeUnorderedList< cweePattern_CatmullRomSpline<type> >* Parent = nullptr, const u64& timeStart = 0, const u64& timeEnd = 0);
	virtual vec2										Relearn_Automated(const cweeUnorderedList< cweePattern_CatmullRomSpline<type> >* Parent = nullptr, const u64& timeStart = 0, const u64& timeEnd = 0);
	virtual type										Forecast_Value_Automated(const u64& time, const cweeUnorderedList< cweePattern_CatmullRomSpline<type> >* Parent = nullptr) const;
	virtual cweeThreadedList< std::pair<u64, type> >	Forecast_Series_Automated(const u64& timeStart, const u64& timeEnd, const u64& timeStep, const cweeUnorderedList< cweePattern_CatmullRomSpline<type> >* Parent = nullptr) const;
...
//#pragma loop(hint_parallel(8))
	...
	*/
	...
	virtual void										TemporarilyAdjustMachineLearnForecasts(const vec2& MAD_Adjustment) {
		this->Lock();
		machineLearn_MAD = MAD_Adjustment;
		this->Unlock();
	};
...
protected:
	...
	static bool											DetermineIfExcludeFeature(const cweeThreadedList< std::pair<u64, type> >& data) {
		cweeThreadedList<type> test(data.NumRef() + 2);
		for (auto& x : data)
		{
			test.AddUnique(x.second);
			if (test.NumRef() > 1) return false;
		}		
		return true;
	};
	bool												DetermineIfExcludeFeature(int& check) const {
		bool toReturn = false;	
		this->Lock();
		if (exclusion_list.NumRef() > check) toReturn = exclusion_list[check];		
		this->Unlock();
		check++;
		return toReturn;
	};
...
private:
	mutable float										machineLearn_min = -cweeMath::INF;
	mutable float										machineLearn_max = cweeMath::INF;
	mutable float										machineLearn_roundNearest = 0.001;
	mutable vec2										machineLearn_MAD = vec2(1, 0);
