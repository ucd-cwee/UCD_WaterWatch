#ifndef __INTERPOLATEDMATRIX_H__
#define __INTERPOLATEDMATRIX_H__
...
/*! Effectively a 2-D surface with variable surface height. The height can be set at arbitrary x-y coordinates and the interpolation is automatic for any requested x-y coordinate. */
#if 0

template <typename T> 
class cweeInterpolatedMatrix {
public:
	typedef cweeBalancedCurve<xyContainer<T>> sourceType;
	...
	cweeInterpolatedMatrix(const cweeInterpolatedMatrix<T>& s) {
		...
		s.Lock();
		{
			hilbertContainer = s.hilbertContainer;
	...
	//	Lock();
	//	s.Lock();
	//	sourceType& sourceS = s.UnsafeGetSource();
	//	//for (int i = sourceS.GetNumValues() - 1; i >= 0; ++i) {
	//	//	auto* v_ptr = sourceS.GetValueAddress(i);
	//	//	
	//	//}
	//	s.Unlock();
	//	Unlock();
	...
	T		GetValue(const u64& column, const u64& row) const {
		...
		Lock();
		{
			long long x = column * compressionFactor;
			long long y = row * compressionFactor;
			if (hilbertN > 0) {
				long long pos = xy2d(x - minX, y - minY, hilbertN);
	...
	std::vector<T> GetMatrix(const u64& Left, const u64& Top, const u64& Right, const u64& Bottom, int numColumns, int numRows) const {
		...
		for (R = 0; R < numRows; R++) {
			...
			for (C = 0; C < numColumns; C++) {				
				{
					Lock();
					x = col * compressionFactor;
					y = row * compressionFactor;
					if (hilbertN > 0) {
						pos = xy2d(x - minX, y - minY, hilbertN);
	...
	void	InsertValue(const u64& column, const u64& row, const T& value) {
		...
		if (x < minX || (x - minX) >= hilbertN || y < minY || (y - minY) >= hilbertN) {
			...
			hilbertN = ::Max(next_pow2(maxX - minX), next_pow2(maxY - minY)); // i.e. 1,2,4,16,128,256,1024
		}

		// the current formula holds and can be re-applied
		hilbertContainer.AddUniqueValue(xy2d(x - minX, y - minY, hilbertN), value);
	...
	cweeStr			ToString(void) const {
		...
		cweeStr out; 
		out.AddToDelimiter(hilbertContainer.Serialize(), delim);
		out.AddToDelimiter(minX, delim);
		out.AddToDelimiter(maxX, delim);
		out.AddToDelimiter(minY, delim);
		out.AddToDelimiter(maxY, delim);
		out.AddToDelimiter(cweeStr::ToString(minV), delim);
		out.AddToDelimiter(cweeStr::ToString(maxV), delim);
		out.AddToDelimiter(hilbertN, delim);

		return out;
	};
	cweeInterpolatedMatrix& FromString(const cweeStr& v) {	
		auto inbound = v.Split("::cIM::");
		Lock();
		if (inbound.getNumVars() >= 8) {
			hilbertContainer.Deserialize(inbound[0]);
			minX = (u64)inbound[1];
			maxX = (u64)inbound[2];
			minY = (u64)inbound[3];
			maxY = (u64)inbound[4];
...
protected: // data
	mutable cweeBalancedPattern<T> hilbertContainer; // x-position is the length along the hilbert line 
	...
	constexpr static u64 compressionFactor = 100.0; // 100000.0f;
...
public:
	...
	cweeInterpolatedMatrix(const cweeInterpolatedMatrix<T>& s) {
		...
		sD.Lock();
		for (cweeBalancedTreeNode<xyContainer<T>, u64>* ptr : sD.UnsafeGetKnotSeries()) {
			if (ptr && ptr->object) {
...
private: // private member methods
	void UnsafeValidateData() const {
		if (invalidated) {
			...
			source.Lock();
			for (cweeBalancedTreeNode<xyContainer<T>, u64>* ptr : source.UnsafeGetKnotSeries()) {
				if (ptr && ptr->object) {
