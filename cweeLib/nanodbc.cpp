...
#include "nanodbc.h" // shouldn't need this since it's being included in the precompiled.h > lib.h sequence
...
public:
    ...
    virtual nanodbc::connection CreateConnection(cweeStr Server, cweeStr UserID, cweeStr Password, cweeStr Driver) {
        ...
        if (fileSystem->checkFileExists(Server) || ext=="db") {
            ...
            connection.SQLiteHandle(handle);
        }
        else {
    ...
    virtual bool IsConnected(nanodbc::connection con) {
        return ((con.SQLiteHandle() >= 0 || con.connected()));
    };

    virtual void EndConnection(nanodbc::connection con) {
        if (IsConnected(con)) con.disconnect();
    };

    virtual nanodbc::result Query(nanodbc::connection con, const cweeStr& query, int batchSize) {
        if (con.SQLiteHandle() >= 0) {
            SQLite::createStatement(con.SQLiteHandle(), query);
        }
        ...
        result.SQLiteHandle(con.SQLiteHandle());

        return result;
    };

    virtual cweeThreadedList<cweeStr> GetNextRow(nanodbc::result& results) {
        cweeThreadedList<cweeStr> out;
        GetNextRow(results, out);
        return out;
    };

    virtual bool GetNextRow(nanodbc::result& results, cweeThreadedList<cweeStr>& out) {
        if (results.SQLiteHandle() >= 0) {
            out = SQLite::getNextRow(results.SQLiteHandle());
            return (out.Num() > 0);
        }
        else {
    ...
    virtual cweeThreadedList < cweeThreadedList<cweeStr> > GetResults(nanodbc::result& results) {
    ...
    virtual cweeThreadedList < cweeThreadedList<cweeStr> > GetResults(nanodbc::connection con, const cweeStr& query, int batchSize) {
        nanodbc::result results = odbc->Query(con, query, batchSize);
        return odbc->GetResults(results);
    };

    virtual void GetResults(nanodbc::connection con, const cweeStr& query, cweeThreadedList < cweeThreadedList<cweeStr> >& out, int batchSize) {
        nanodbc::result results = odbc->Query(con, query, batchSize);
        ...
        if (results.SQLiteHandle() >= 0) {
            ...
            cweeThreadedList<cweeStr> Row = SQLite::getNextRow(results.SQLiteHandle());
            while (Row.Num() > 0) {
                out.Append(Row);
                Row = SQLite::getNextRow(results.SQLiteHandle());
    ...
    virtual cweeThreadedList<cweeStr> GetDatabaseNames(nanodbc::connection con) {
        cweeThreadedList<cweeStr> out;
        if (con.SQLiteHandle() >= 0) {
            ...
            if (cweeStr(con.DriverName().c_str()).Find("MYSQL", false) < 0 && cweeStr(con.DriverName().c_str()).Find("myodbc", false) < 0) {
                ...
                auto queryResult = odbc->GetResults(con, "SELECT name FROM sys.databases;", 100);
                ...
                auto queryResult = odbc->GetResults(con, "SHOW SCHEMAS;", 100);
                for (auto& row : queryResult) {
                    for (auto& col : row) {
                        if (!col.IsEmpty())
                            out.AddUnique(col);
    ...
    virtual cweeThreadedList<cweeStr> GetTableNames(nanodbc::connection con, const cweeStr& databaseName = "") {
        if (con.SQLiteHandle() >= 0) {
            nanodbc::result result = odbc->Query(con, "Select name From sqlite_master where type='table';", 100);
            auto results = odbc->GetResults(result);
            ...
            if (cweeStr(con.DriverName().c_str()).Find("MYSQL", false) < 0 && cweeStr(con.DriverName().c_str()).Find("myodbc", false) < 0) {
                ...
                if (databaseName.IsEmpty()) {
                    ...
                    if (listDB.Num() > 0) {
                        for (auto& x : listDB) {
                            for (auto& y : odbc->GetResults(con, cweeStr::printf("SELECT TABLE_NAME FROM %s.information_schema.tables;", x.c_str()), 1)) {
                    ...
                    queryResult = odbc->GetResults(con, cweeStr::printf("SELECT TABLE_NAME FROM %s.information_schema.tables;", databaseName.c_str()), 1);

                if (queryResult.Num() <= 0) {
                    queryResult = odbc->GetResults(con, "SELECT TABLE_NAME FROM information_schema.tables;", 1);
                }
                ...
                if (databaseName.IsEmpty()) {
                    ...
                    if (listDB.Num() > 0) {
                        for (auto& x : listDB) {
                            for (auto& y : odbc->GetResults(con, cweeStr::printf("SELECT table_name FROM information_schema.tables WHERE table_schema = '%s';", x.c_str()), 1)) {
                                queryResult.Append(y);
                            }
                        }
                    }
                    ...
                    queryResult = odbc->GetResults(con, cweeStr::printf("SELECT table_name FROM information_schema.tables WHERE table_schema = '%s';", databaseName.c_str()), 1);
    ...
    virtual cweeThreadedList<cweeStr> GetColumnNames(nanodbc::connection con, cweeStr tableName, const cweeStr& databaseName = "") {
        if (con.SQLiteHandle() >= 0) {

            int numColumns = SQLite::getNumColumns(con.SQLiteHandle(), tableName);
            if (numColumns > 32767) numColumns = 1;
            cweeThreadedList<cweeStr> out(numColumns + 16);
            for (int i = 0; i < numColumns; i++) out.Append(SQLite::getColumnName(con.SQLiteHandle(), i, tableName));
            ...
            if (databaseName.IsEmpty()) {
                ...
                auto queryResult = odbc->GetResults(con, query.c_str(), 100);
                for (auto& row : queryResult) {
                    for (auto& col : row) {
                        out.Append(col);
                    }
                ...
                if (out.Num() <= 0) {
                    query = cweeStr::printf("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '%s';", tableName.c_str());
                    queryResult = odbc->GetResults(con, query.c_str(), 100);
                    for (auto& row : queryResult) {
                        for (auto& col : row) {
                            out.Append(col);
                        }
    ...
    virtual cweeThreadedList<cweeStr> FirstColumnOnly(const cweeThreadedList < cweeThreadedList<cweeStr> >& results) {
    ...
    virtual cweeStr SafeString(const cweeStr& in, nanodbc::connection* optionalConnectionForTableSchemaCheck = nullptr) {
        /* use case examples:
    ...
    virtual cweeStr PartialTableNameToFullTableName(nanodbc::connection con, const cweeStr& in) {
        /* use case examples:
            EBMUD.SRCE\srce-rtygood.PERSONS     -->     EBMUD.[SRCE\srce-rtygood].PERSONS
            EBMUD.ebmud.ami_usage               -->     EBMUD.ebmud.ami_usage
    ...
    virtual cweeThreadedList < cweeStr > GetTableSchema(nanodbc::connection con, const cweeStr& tableName, const cweeStr& databaseName = "") {
        if (con.SQLiteHandle() >= 0) {
            ...
            if (databaseName.IsEmpty())
                queryResult = odbc->GetResults(con, cweeStr::printf("SELECT TABLE_SCHEMA FROM information_schema.tables WHERE TABLE_NAME = '%s';", tableName.c_str()), 100);
            else
                queryResult = odbc->GetResults(con, cweeStr::printf("SELECT TABLE_SCHEMA FROM %s.information_schema.tables WHERE TABLE_NAME = '%s';", databaseName.c_str(), tableName.c_str()), 100);
    ...
    virtual bool CreateTable(nanodbc::connection con, cweeStr tableName, const cweeThreadedList<cweeStr>& columnNames, cweeStr databaseName = "") {
    ...
    virtual cweeStr GetDatabaseName(nanodbc::connection con, cweeStr tableName) {        
        if (con.SQLiteHandle() >= 0) {
            return "";
        }
        else {
            for (auto& x : GetDatabaseNames(con)) {
                for (auto& y : odbc->GetResults(con, cweeStr::printf("SELECT TABLE_NAME FROM %s.information_schema.tables;", x.c_str()), 100)) {                    
                    if (y == tableName) {
                        return x;
                    }
                }
    ...
    virtual bool TableExists(nanodbc::connection con, cweeStr tableName) {
    ...
    virtual void InsertRow(nanodbc::connection con, cweeStr tableFullPath, const cweeThreadedList<cweeStr>& values) {
        cweeStr query;
        if (con.SQLiteHandle() >= 0) {
            // database name is not used here. 
            {
...
*/
cweeOdbcLocal	odbcLocal;
/*!
geocoding and mapping services
*/
cweeODBC* odbc = &odbcLocal;