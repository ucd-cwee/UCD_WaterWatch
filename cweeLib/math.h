...
static cweeSharedPtr < cwee_rand >	sharedCweeRandomGenerator = nullptr;

cwee_rand* GetCweeRandomGenerator();
int cweeRandomInt();
int cweeRandomInt(int max);
int cweeRandomInt(int min, int max);
float cweeRandomFloat(float min, float max);
float cweeRandomFloat(float max);
float cweeRandomFloat();

// ORIGINAL AUTHOR: https://arvid.io/2018/07/02/better-cxx-prng/#fnref2
class splitmix
{
public:
	using result_type = uint32_t;
...
INLINE cwee_rand* GetCweeRandomGenerator() {
	sharedCweeRandomGenerator.Lock();
	cwee_rand* p = sharedCweeRandomGenerator.UnsafeGet(); {
		if (!p) {
			p = new cwee_rand();
			sharedCweeRandomGenerator.UnsafeSet(p);
		}
	}
	sharedCweeRandomGenerator.Unlock();
	return p;
}
...
*/
INLINE float cweeRandomFloat() {
	return GetCweeRandomGenerator()->Random(0.0f, 1.0f);
...
*/
INLINE float cweeRandomFloat(float max) {
	return GetCweeRandomGenerator()->Random(0.0f, max);
	//std::random_device rd;
	//pcg rand(rd);
	//std::uniform_real_distribution<> u(0, max);
	//return u(rand);
}
...
*/
INLINE float cweeRandomFloat(float min, float max) {
	return GetCweeRandomGenerator()->Random(min, max);
	//std::random_device rd;
	//pcg rand(rd);
...
/*!
random int between 0 and cweeMath::INF
*/
INLINE int cweeRandomInt() {
	return GetCweeRandomGenerator()->Random(0, std::numeric_limits<int>::max());
	//std::random_device rd;
	//pcg rand(rd);
...
*/
INLINE int cweeRandomInt(int max) {
	return GetCweeRandomGenerator()->Random(0, max);
	//std::random_device rd;
	//pcg rand(rd);
...
*/
INLINE int cweeRandomInt(int min, int max) {
	return GetCweeRandomGenerator()->Random(min, max);
...
//	};
	...
	*/
	//static void						rollingAverageRef(float& currentAverage, float newSample, int& numSamples);
	//static void						rollingAverageRef(double& currentAverage, double newSample, int& numSamples);
	template<typename T> static void	rollingAverageRef(T& currentAverage, T newSample, int& numSamples) {
		numSamples++;
		currentAverage -= currentAverage / numSamples;
		currentAverage += newSample / numSamples;
	};
	...
	*/
	...
	static const float				AngRad;						// number of radians in a degree
	static const float				G;							// gravity at Earth's surface
	...
	static const float 				EPSILON;					// smallest positive number such that 1.0+FLT_EPSILON != 1.0
...
#pragma region OPTIM_HELPER_FUNCTIONS
		...
		// return a seed from a hardcoded vector on the stack //
		float static return_seed_number_by_index(int seed_index_input) {
			cweeThreadedList<float> seed_random({
				0.659925939,0.851198612,0.888958499,0.043649046,
				0.294554541,0.640250542,0.145175261,0.420959607,
				0.586797604,0.601317432,0.448056309,0.016455733,
				0.101877904,0.429179555,0.938000942,0.698886156,
				0.526609548,0.992171241,0.349015878,0.821558028,
				0.325748197,0.078570883,0.799374681,0.807721956,
				0.740827815,0.188279933,0.36604604,0.66813877,
				0.63490214,0.661127945,0.173588857,0.143878525,
				0.852131654,0.603966601,0.986792033,0.657933689,
				0.929853487,0.513606515,0.887513836,0.113635684,
				0.557319428,0.841989861,0.538303689,0.377089609,
				0.945879012,0.243048723,0.453836479,0.84905436,
				0.323172588,0.812474094,0.247981833,0.660809508,
				0.724984132,0.879307406,0.037026773,0.33638835,
				0.065285444,0.016726875,0.755269346,0.169076944,
				0.591315259,0.972961471,0.198293728,0.02167758,
				0.03626392,0.85420732,0.974805023,0.760714424,
				0.158645077,0.102183114,0.829561346,0.620496414,
				0.081554207,0.534407724,0.761577324,0.42248031,
				0.900893299,0.833479745,0.122303218,0.593739882,
				0.835224414,0.784897439,0.765540037,0.993217601,
				0.066809805,0.339474803,0.240379012,0.729760651,
				0.236385284,0.253581962,0.205718302,0.071748121,
				0.344392226,0.204947191,0.069789143,0.755642384,
				0.322453522,0.362760427,0.762985558,0.002876362,
				0.087346772,0.994316363,0.218594476,0.806556734,
				0.648064559,0.20430378,0.642914062,0.11590413,
				0.377293893,0.582699633,0.443162141,0.901914037,
				0.368667864,0.46030927,0.156586215,0.926883514,
				 0.686857341,0.283784332,0.071738084,0.88662258,
				0.979817964,0.401688014,0.212762254,0.096936917,
				0.460133485,0.685973249,0.285663143,0.680771539,
				0.273315388,0.974410534,0.652995281,0.695988552,
				0.82325711,0.722614271,0.689250321,0.633165903,
				0.318409661,0.986023829,0.878222603,0.234726491,
				0.567826379,0.484513743,0.053516355,0.339680419,
				0.076070049,0.113946325,0.909807302,0.759485123,
				0.534926527,0.057556357,0.984570052,0.675046447,
				0.961530396,0.713093126,0.15522367,0.72779377,
				0.441174245,0.14570856,0.112989576,0.561240057,
				0.932932348,0.492542614,0.063505589,0.022325076,
				0.423494528,0.633370331,0.691051703,0.44527093,
				0.910517861,0.498053858,0.851611132,0.959553755,
				0.728271649,0.288946722,0.343998244,0.777555745,
				0.731882404,0.726313184,0.83224879,0.562043962,
				0.314873564,0.673314267,0.78561582,0.014405957,
				0.2984526,0.83444698,0.608582839,0.865525944,
				0.171799611,0.129201437,0.71399942,0.490240112,
				0.962389133,0.438019505,0.798042963,0.917171467,
				0.504973952,0.351176713,0.258469352,0.445157832 }); // create a seed of random numbers (n=200)
			return seed_random[(seed_index_input % seed_random.Num())];
		};
		...
		float static NormalRandomGenerator(float mean, float stddev, bool random = true, non_random_type_t type = __NONE_NON_RANDOM__, int seed_number = 0) {
			...
			else if (type == __SEED_NONRANDOM__) {
				return mean + 6 * (return_seed_number_by_index(seed_number) - 0.5) * stddev; // this isn't actually normal. It's not supposed to be (testing only)
			}
			...
			for (int index_i = 0; index_i < upper_bound.Num(); index_i++) {				 // random vector //
				if (random) {
					...
					if (type == __LOW_LIMIT__) {
						...
						sampled_vector.Append(lower_bound[index_i] + (upper_bound[index_i] - lower_bound[index_i]) * return_seed_number_by_index(seed_non_random_counter));
						seed_non_random_counter++;
					}
					else {
...
*/
		...
		public:
			...
			int num_elites								= 2;
			float percent_recombination					= 0.4;
			float percent_dim_for_normal_variance		= 0.4; // only used for normal distribuition variance // 
