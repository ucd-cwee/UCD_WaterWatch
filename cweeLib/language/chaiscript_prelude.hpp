...
def MarkMapNodeDepth(Map themap, int depth){
    themap["depth"] = depth;
    if (themap.contains("children")){
        for (j : themap["children"]){
            MarkMapNodeDepth(j, depth + 1);
        }
    }
}
def MapNodes(AST_Node node) {
    ...
    if (node.children().size() > 0){
        out["children"] := Vector();
        for (j : node.children()){  
            out["children"].push_back_ref(MapNodes(j));
        }
    }
    if (node.identifier == AST_Node_Type().Constant()){
        try{
            var& t = eval("(`${node.text}`).type_name()");
            out["type"] := t;
        }catch(e){
            try{
                var& t = eval("(${node.text}).type_name()");
        ...
        try{
            var& t = eval("(`${node.text}`).type_name()");
            try{
                if (t == "Function" && eval("`${node.text}`").IsAttribute()){                 
                    out["type"] := "Attribute";
                }else{
                    out["type"] := t;
                }
            ...
            try{
                var& tn = eval("(${node.text}).type_name()");
                out["type"] := tn;
            }catch(e2){
                out["type"] := "Object";
            }   
