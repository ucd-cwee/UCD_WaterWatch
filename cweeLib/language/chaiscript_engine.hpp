...
namespace chaiscript {
  ...
  class ChaiScript_Basic {
    ...
    void build_eval_system(const ModulePtr &t_lib, const chaiscript::small_vector<Options> &t_opts) {
      if (t_lib) {
        add(t_lib);
      }
      ...
      m_engine.add(fun([this](Boxed_Value& t_bv) { return m_engine.Delete(t_bv); t_bv.assign(Boxed_Value()); }), "delete");
  ...
  public:
    ...
    std::string to_string(const Boxed_Value& bv) {
        auto t_ss = chaiscript::detail::Dispatch_State(m_engine);
