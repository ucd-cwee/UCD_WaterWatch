...
namespace chaiscript::detail::threading {
  ...
  public:
      ...
      inline const T* operator->() const noexcept { return t()[this].Get(); }

      inline const T& operator*() const noexcept { return *t()[this].Get(); }

      inline T* operator->() noexcept { return t()[this].Get(); }
  ...
  private:
      /// todo: is it valid to make this noexcept? The allocation could fail, but if it does there is no possible way to recover
