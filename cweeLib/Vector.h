...
//===============================================================
//	measurement - 2D vector of time_t/float pairs
//===============================================================
class measurement {
public:
	time_t					time;
	float					value;
							...
							measurement();
	explicit				measurement(const time_t time, const float value);

	void 					Set(const time_t time, const float value);
	void					Zero();

	time_t					first() { return time; };
	float					second() { return value; };


	measurement				operator-() const;
	float					operator*(const measurement& a) const;
	measurement				operator*(const float a) const;
	measurement				operator/(const float a) const;
	...
	measurement&			operator-=(const measurement& a);
	measurement&			operator/=(const measurement& a);
	measurement&			operator/=(const float a);
	measurement&			operator*=(const float a);
	friend measurement		operator*(const float a, const measurement b);

	int						GetDimension() const { return 2; };

	bool					cmp(const measurement& a) const { return Compare(a); };			// exact compare, no epsilon
	bool					Compare(const measurement& a) const;						// exact compare, no epsilon
	bool					operator==(const measurement& a) const;						// exact compare, no epsilon
	bool					operator!=(const measurement& a) const;						// exact compare, no epsilon

	void					Lerp(const measurement& v1, const measurement& v2, const float l);
};

INLINE measurement::measurement() {
}

INLINE measurement::measurement(const time_t time, const float value) {
	this->time = time;
	this->value = value;
}

INLINE void measurement::Set(const time_t time, const float value) {
	this->time = time;
	this->value = value;
}

INLINE void measurement::Zero() {
	time = 0;
	value = 0;
}

INLINE bool measurement::Compare(const measurement& a) const {
	return ((time == a.time) && (value == a.value));
}

INLINE void measurement::Lerp(const measurement& v1, const measurement& v2, const float l) {
	if (l <= 0.0f) {
		(*this) = v1;
	}
	else if (l >= 1.0f) {
...
INLINE bool measurement::operator==(const measurement& a) const {
	return Compare(a);
}
...
INLINE measurement measurement::operator-() const {
	return measurement(time, -value);
}

INLINE measurement measurement::operator-(const measurement& a) const {
	return measurement(time - a.time, value - a.value);
}

INLINE float measurement::operator*(const measurement& a) const {
	return time * a.time + value * a.value;
}

INLINE measurement measurement::operator*(const float a) const {
	return measurement(time * a, value * a);
}

INLINE measurement measurement::operator/(const float a) const {
	float inva = 1.0f / a;
	return measurement(time * inva, value * inva);
}

INLINE measurement operator*(const float a, const measurement b) {
	return measurement(b.time * a, b.value * a);
}
...
INLINE measurement& measurement::operator+=(const measurement& a) {
	time += a.time;
	value += a.value;
...
INLINE measurement& measurement::operator/=(const float a) {
	float inva = 1.0f / a;
	time *= inva;
	value *= inva;
...
extern vec2d vec2d_origin;
#define vec2d_zero vec2d_origin
