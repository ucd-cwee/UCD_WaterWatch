...
// An important component of this system is that when a job waiter "waits", it actually leverages a fiber that was made with the TaskScheduler object to leave the current fiber instance and go work elsewhere.
// If that TaskScheduler is made on thread "A" and then thread "B" adds work to it, however, then 1) thread A *must* be alive, and 2) thread B CANNOT wait, since it does not exist in the fiber pool and it will fail.
