...
#pragma warning(disable : 4267) // suppressing warning on casting size_t to unsigned int, etc.
    ...
    template <typename T> __forceinline GL::any GL::impl::instance_funcs<T>::instance_by_value(GL::any const& from) {
        ...
        else if (GL::value* p = from.cast<GL::value*>()) {
            if constexpr (std::is_constructible_v<base_type, GL::value&>) {
                GL::any out(static_cast<base_type>(*p));
                out |= GL::type::Qualifiers::Temporary;
                return out;
            } else if constexpr (std::is_constructible_v<base_type, float&>) {
                GL::any out(static_cast<base_type>((float)*p));
                out |= GL::type::Qualifiers::Temporary;
                return out;
            }
        }
