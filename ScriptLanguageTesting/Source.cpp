    ...
    namespace ABA_Problem {
        ...
        public:
            ...
            void RunGC() {
                ...
                long long curr_epoch{ GoodLang::EpochGarbageCollectorImpl::ThreadManager::GetCurrentEpoch() };
                ...
                if (((curr_epoch - previous_epoch) > duration_ms) && (InterlockedCompareExchange64(reinterpret_cast<volatile long long*>(&_lastGC), curr_epoch, previous_epoch) == previous_epoch)) {
                    // if ((curr_epoch - _lastGC) > duration_ms) {
                        // InterlockedExchange64(reinterpret_cast<volatile long long*>(&_lastGC), curr_epoch);
        ...
        public:
            ...
            EpochAllocator()
                ...
                , _delete_list{}
                , _TLS{}
                , _lastGC{ GoodLang::EpochGarbageCollectorImpl::ThreadManager::GetCurrentEpoch() }
            {};
...
namespace utilities {
    ...
    public:
        struct aux {
            ...
            virtual std::shared_ptr<void> protect_aux() = 0;
        ...
        static __declspec(noinline) aux* inc_strong(GoodLang::atomic_ptr<aux> const& pa) {
            ...
            std::shared_ptr<void> protect;
            if (pa_ptr = pa.load()) {
                protect = pa_ptr->protect_aux();
            }
    ...
    protected:
        ...
        struct aux_default final : public aux {
            ...
            virtual std::shared_ptr<void> protect_aux() override {
                using guardType = typename utilities::ABA_Problem::EpochAllocator<aux_default<U>>::GuardType;
                return std::static_pointer_cast<void>(std::make_shared<guardType>(shared_ptr<U>::aux_default_allocator().ProtectCurrentEpoch()));
            };
        ...
        struct aux_deleter final : public aux {
            ...
            virtual utilities::type const& type() const override { return utilities::type_of<U>(); };
            virtual std::shared_ptr<void> protect_aux() override {
                using guardType = typename utilities::ABA_Problem::EpochAllocator<aux_deleter<U>>::GuardType;
                return std::static_pointer_cast<void>(std::make_shared<guardType>(shared_ptr<U>::aux_deleter_allocator().ProtectCurrentEpoch()));
            };
            virtual void destroy_aux() override {
        ...
        static T* get(shared_ptr const& p) {
            ...
            if (out = p.ptr) return out;

            std::shared_ptr<void> protect;
            while (auto* pa_ptr = shared_ptr_base::inc_strong(p.GetPaux())) {
                protect = pa_ptr->protect_aux();
                read = pa_ptr->Strong_Weak_Destroy_Delete;
                if (!reinterpret_cast<long*>(&read)[1]) { // if NOT being destroyed or deleted...
                    out = static_cast<T*>(pa_ptr->ptr());
                    shared_ptr_base::dec_strong(pa_ptr);
                    return out;
                }
                else {
                    shared_ptr_base::dec_strong(pa_ptr);
        ...
        */
        ...
        protected:
            static function_signature CreateSignature(std::vector<any> const& defaults = {}) {
                ...
                types Types;
#define argT(NN) if constexpr (numArgs > NN) { Types += type_of<typename std::tuple_element_t<NN, argType>>(); }
                argT(0);
                argT(1);
                argT(2);
                argT(3);
...
#undef argT                
                ...
                if constexpr (std::is_reference< returnType>::value) {
                    ...
                    if constexpr (numArgs == 16) {
                        out = ptrType(&F_m(
                            ...
                            throw GoodLang::exception::arity_error(parents.size(), numArgs, __LINE__);
                        } });
                    }
        ...
        */
        ...
        protected:
            ...
            any do_call_impl(std::shared_ptr<Class> const& o) const {
...
namespace utilities {
    ...
    public:
        ...
        virtual utilities::any convert(const utilities::any& from) const = 0;
    ...
    public:
        ...
        utilities::any convert(const utilities::any& from_p) const override final {
            if (from_p.current_type().is_void()) {
                throw std::runtime_error("Cannot cast from void");
            }
            ...
            if (can_free_cast) {
                ...
                return (To)(from_p.cast<From const&>());
            }
        };
    ...
    public:
        ...
        utilities::any convert(const utilities::any& from_p) const override final {
            return from_p.polymorphic_cast<From>(to());
        };
        double cost() const noexcept override final {
            return 0;
        };
    ...
    public:
        typedef typename GoodLang::utilities::function_traits< typename decltype(std::function(std::declval<Callable>())) >::result_type
            ReturnType;
        typedef typename std::tuple_element_t<0, typename GoodLang::utilities::function_traits< typename decltype(std::function(std::declval<Callable>())) >::arguments>
            InputType;

    public:
        Custom_Type_Conversion_Impl(Callable t_func, type const& inboundType, type const& outboundType)
            : type_conversion_base()
            , m_func(std::move(t_func))
            , inboundType_m(inboundType)
            , outboundType_m(outboundType)
        {
            can_free_cast = from().can_free_cast(to());
        };
        Custom_Type_Conversion_Impl(Callable t_func) : Custom_Type_Conversion_Impl(std::move(t_func), type_of<InputType>(), type_of<ReturnType>()) {};

        const type& to() const noexcept override final {
            return outboundType_m;
        };
        const type& from() const noexcept override final {
            return inboundType_m;
        };
        utilities::any convert(const utilities::any& from_p) const override final {
            if (from_p.current_type().is_void()) {
                throw std::runtime_error("Cannot cast from void");
            ...
            if (can_free_cast) {
                ...
                if constexpr (std::is_convertible<decltype(from_p), InputType>::value) {
                    return m_func(from_p);
                }
                else {
                    return m_func(from_p.cast());
                }
            }
        };
        double cost() const noexcept override final { return baseline; };
        bool is_polymorphic() const noexcept override final { return false; } // is polymorphic conversion?
    ...
    template<class Callable> __forceinline std::shared_ptr<type_conversion_base> make_converter(Callable func) {
        typedef decltype(std::function(std::declval<Callable>())) CallableTypeAsStdFunc;
        typedef typename GoodLang::utilities::function_traits< CallableTypeAsStdFunc >::arguments CallableArguments;
        if constexpr (std::tuple_size_v< CallableArguments > != 1) {
            return nullptr;
        }
        else {
            typedef typename std::tuple_element_t<0, CallableArguments> From;
            typedef typename GoodLang::utilities::function_traits< CallableTypeAsStdFunc >::result_type To;
    ...
    template<typename From, typename To> __forceinline std::shared_ptr<type_conversion_base> make_converter() {
        constexpr static bool is_convertable = impl::is_explicitly_convertible_to<From, To>::value;
        constexpr static bool is_bidir_convertable = impl::is_explicitly_convertible_to<To, From>::value;
        constexpr static bool is_polymorphic = std::is_base_of<To, From>::value;
...
int main2() {
    ...
    if (1) {
        any out;
        auto converter = utilities::make_converter<double, int>();
        sw.Start();
        GoodLang::parallel::For(0, 1000000, [&](int j) {
            any out2 = converter->convert(100.0);
            // out = 100;
            //out = out2;
    ...
    if (1) {
        any out;
        auto converter = utilities::make_converter([](double const& from) -> int { return from; });
        sw.Start();
        GoodLang::parallel::For(0, 1000000, [&](int j) {
            //converter->convert(100.0, out);
            out = 100;
            if (out) {
                if (int* p = out.cast<int*>()) {
                    EXPECT_EQ(100, *p);
                }
                else {
                    print("BAD PTR");
    ...
    if (1) {
        any out = utilities::Static_Type_Conversion_Impl<double, int>().convert(100.0);
        EXPECT_EQ(100, (int)out.cast<int>());
    }
    if (1) {
        any out;
        ...
        out = down_converter.convert(from); // out should be a value
        EXPECT_EQ(false, out.empty());
        EXPECT_EQ(100, (int)out.cast<GoodLang::Units::value>()());
        out = up_converter_1.convert(out); // out should be a foot
        EXPECT_EQ(false, out.empty());
        EXPECT_EQ(100, (int)out.cast<GoodLang::Units::foot>()());
        out = down_converter.convert(out); // out should be a value
        EXPECT_EQ(false, out.empty());
        EXPECT_EQ(100, (int)out.cast<GoodLang::Units::value>()());
        out = up_converter_2.convert(out); // out will be void (empty) because the cast failed. 
    ...
    while (true) {
        ...
        if (1) {
            ...
            for (int i = 0; i < 1000000; ++i) {
                delete (new utilities::string(GoodLang::ToString(i)));
            }
            print(GoodLang::ToString(GoodLang::Units::second(sw.Stop_s())) + " @ " + GoodLang::ToString(__LINE__));
            sw.Start();
            for (int i = 0; i < 1000000; ++i) {
