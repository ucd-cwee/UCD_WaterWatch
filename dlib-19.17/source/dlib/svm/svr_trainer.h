...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        template <
            ...
            typename in_scalar_vector_type
        >
            const decision_function<kernel_type> train_debug(
                const in_sample_vector_type& x,
                const in_scalar_vector_type& y
            ) const
        {
            return do_train_debug(mat(x), mat(y));
        }
    ...
    // ------------------------------------------------------------------------------------
        ...
        ) const
        {
            ...
            int numIterations = solver(
                symmetric_matrix_cache<float>(make_quad(kernel_matrix(kernel_function,x)), cache_size),  // Q
                uniform_matrix<scalar_type>(2*x.size(),1, eps_insensitivity) + join_cols(y,-y), // p
                join_cols(uniform_matrix<scalar_type>(x.size(),1,1), uniform_matrix<scalar_type>(x.size(),1,-1)),  // y
                0, // B
                C, // Cp
                C, //Cn
                alpha, // alpha
        ...
        template <
            ...
            typename in_scalar_vector_type
        >
            const decision_function<kernel_type> do_train_debug(
                const in_sample_vector_type& x,
                const in_scalar_vector_type& y
            ) const
        {
            ...
            scalar_vector_type alpha;
            solve_qp3_using_smo_debug<scalar_vector_type> solver;
            int numIterations = solver(
                symmetric_matrix_cache<float>(make_quad(kernel_matrix(kernel_function, x)), cache_size),  // Q
                uniform_matrix<scalar_type>(2 * x.size(), 1, eps_insensitivity) + join_cols(y, -y), // p
                join_cols(uniform_matrix<scalar_type>(x.size(), 1, 1), uniform_matrix<scalar_type>(x.size(), 1, -1)),  // y
