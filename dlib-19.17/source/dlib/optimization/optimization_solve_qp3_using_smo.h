...
// ----------------------------------------------------------------------------------------
    ...
    public:
                ...
                )
        {
            ...
            matrix_exp<EXP1> _Q = Q;
            matrix_exp<EXP2> _p = p;
            matrix_exp<EXP3> _y = y;
            scalar_type _B = B;
            scalar_type _Cp = Cp;
            scalar_type _Cn = Cn;
            matrix<scalar_type, NR, 1, mem_manager_type, layout_type> _alpha = alpha;
            scalar_type _eps = eps;
            column_matrix _df = df;

            count = 0;            
            unsigned long long timeStart1 = std::chrono::duration_cast<std::chrono::milliseconds>(m_clock.now().time_since_epoch()).count();
            if (1) {
                ...
                while (find_working_group_early_exit(y, alpha, Q, df, Cp, Cn, tau, eps, i, j, orig_eps))
                {
                    // early exit if bad performance
                    {
                        ...
                        if (eps_history.size() > 1) { // review every X attempts, and non-zero attempts were made                    
                            scalar_type avg_eps = 0;
                            for (auto& x : eps_history) avg_eps += x;
                            avg_eps /= eps_history.size();

                            if ((prev_avg_eps - avg_eps) < orig_eps) {
                                ++numFails;
                                // do not update the prev_avg_eps, as it still needs to be beat.
                            }
                            else {
#if 0
                                // Unlock this if you'd like the early-exit system to be more forgiving.
                                // Leave locked if you'd like the early-exit system to early-exit sooner (will result in inferior ML attempts, but could be improved with greater search-speed for Gamma & c combos.)
                                numFails = (numFails - 1 > 0 ? numFails - 1 : 0);
#endif
                                prev_avg_eps = avg_eps;
                            }
                    ...
                    // update the df vector now that we have modified alpha(i) and alpha(j)
                    {
                        ...
                        for (long k = 0; k < df.nr(); ++k) df(k) += Q_i(k) * delta_alpha_i + Q_j(k) * delta_alpha_j;
                    }
            ...
            unsigned long long timeEnd1 = std::chrono::duration_cast<std::chrono::milliseconds>(m_clock.now().time_since_epoch()).count();

            count = 0;
            unsigned long long timeStart2 = std::chrono::duration_cast<std::chrono::milliseconds>(m_clock.now().time_since_epoch()).count();
            if (1) {

                const scalar_type tau = 1e-12;
                typedef typename colm_exp<EXP1>::type col_type;
                long i = 0, j = 0;
                scalar_type orig_eps = _eps;
                std::vector<scalar_type> eps_history;
                scalar_type prev_avg_eps = std::numeric_limits<float>::max();
                int numFails = 0;
                ...
                // initial alpha
                {
                    set_initial_alpha(_y, _B, _Cp, _Cn, _alpha);
                }

                // initialize df. Compute df = Q*alpha + p
                {
                    _df = _p;
                    for (long r = 0; r < _df.nr(); ++r)
                    {
                        if (_alpha(r) != 0)
                        {
                            _df += _alpha(r) * matrix_cast<scalar_type>(colm(_Q, r));
                        }
                    }
                }

                // now perform the actual optimization of alpha
                while (find_working_group_early_exit(_y, _alpha, _Q, _df, _Cp, _Cn, tau, _eps, i, j, orig_eps))
                {
                    // early exit if bad performance
                    {
                        // allow for early exit if the performance (eps) is not satisfactorily improving. 
                        // It is allowed to stuggle then improve again, which will eventually reset the fail counter. 
                        // But too many failures vs. number of improvements may result in an early exit as it appears to be spinning its wheels. 
                        eps_history.push_back(_eps);
                        if (eps_history.size() > 1 && eps_history.size() % 10 == 0) { // review every X attempts, and non-zero attempts were made                    
                            scalar_type avg_eps = 0;
                            for (auto& x : eps_history) avg_eps += x;
                            avg_eps /= eps_history.size();
                    ...
                    const scalar_type old_alpha_i = _alpha(i);
                    const scalar_type old_alpha_j = _alpha(j);
                    optimize_working_pair(_alpha, _Q, _y, _df, tau, i, j, _Cp, _Cn);

                    // update the df vector now that we have modified alpha(i) and alpha(j)
                    {
                        scalar_type delta_alpha_i = _alpha(i) - old_alpha_i;
                        scalar_type delta_alpha_j = _alpha(j) - old_alpha_j;
                        col_type Q_i = colm(_Q, i);
                        col_type Q_j = colm(_Q, j);
                        for (long k = 0; k < _df.nr(); ++k) _df(k) += Q_i(k) * delta_alpha_i + Q_j(k) * delta_alpha_j;
                    }
            ...
            unsigned long long timeEnd2 = std::chrono::duration_cast<std::chrono::milliseconds>(m_clock.now().time_since_epoch()).count();

            auto timeDiff1 = timeEnd1 - timeStart1;
            auto timeDiff2 = timeEnd2 - timeStart2;
            std::cout << "diff1: " << timeDiff1 << ", diff2: " << timeDiff2 << std::endl;
    ...
    // ------------------------------------------------------------------------------------
        ...
        template <
            ...
            typename U, typename V
        >
        inline bool find_working_group_early_exit(
            const V& y,
            const U& alpha,
            const matrix_exp<EXP>& Q,
            const scalar_vector_type& df,
    ...
    template <
        typename matrix_type
    >
    class solve_qp3_using_smo_debug
    {
    public:
        typedef typename matrix_type::mem_manager_type mem_manager_type;
        typedef typename matrix_type::type scalar_type;
    ...
    private:
            ...
            ) const
        {
            ...
            long NR = alpha.nr();
            for (long i = 0; i < NR; ++i) // alpha.nr(); ++i)
            {
                if (y(i) == 1)
                {
                    if (alpha(i) < Cp)
            ...
            // now we need to find the minimum jp indices
            scalar_type b; scalar_type a; scalar_type temp;
            NR = alpha.nr();
            for (long j = 0; j < NR; ++j) // alpha.nr(); ++j)
            {
                if (y(j) == 1)
                {
                    if (alpha(j) > 0.0)
